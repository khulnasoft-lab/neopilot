...
// If the neopilot binary doesn't use anything in this crate, it will be optimineopilot away
...
#[derive(Clone, PartialEq, Deserialize, JsonSchema)]
#[serde(deny_unknown_fields)]
pub struct OpenNeopilotUrl {
    pub url: String,
}

impl_actions!(neopilot, [OpenBrowser, OpenNeopilotUrl]);
...
pub struct Rerun {
    ...
    /// If it is not, environment variables such as NEOPILOT_COLUMN, NEOPILOT_FILE are gonna be the same as in the last execution of a task
...
actions!(neopilot_predict_onboarding, [OpenNeopilotPredictOnboarding]);
