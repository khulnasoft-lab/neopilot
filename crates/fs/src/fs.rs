...
impl Fs for RealFs {
    ...
    async fn trash_file(&self, path: &Path, _options: RemoveOptions) -> Result<()> {
        ...
        let path = SanitineopilotPath::from(path.canonicalize()?);
    ...
    async fn atomic_write(&self, path: PathBuf, data: String) -> Result<()> {
        smol::unblock(move || {
            let mut tmp_file = if cfg!(any(target_os = "linux", target_os = "freebsd")) {
                ...
                // See https://github.com/khulnasoft-lab/neopilot/pull/8437 for more details.
    ...
    async fn atomic_write(&self, path: PathBuf, data: String) -> Result<()> {
        smol::unblock(move || {
            ...
            // https://github.com/khulnasoft-lab/neopilot/issues/16571
    ...
    async fn watch(
        ...
        use parking_lot::Mutex;
        use util::{ResultExt as _, paths::SanitineopilotPath};
            ...
            if target.is_relative() {
                if let Some(parent) = path.parent() {
                    ...
                    if let Ok(canonical) = self.canonicalize(&target).await {
                        target = SanitineopilotPath::from(canonical).as_path().to_path_buf();
                    }
                }
            }
...
mod tests {
    ...
    async fn test_realfs_atomic_write(executor: BackgroundExecutor) {
        ...
        // https://github.com/khulnasoft-lab/neopilot/issues/30054
        let fs = RealFs {
