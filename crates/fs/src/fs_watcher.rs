...
impl Watcher for FsWatcher {
    fn add(&self, path: &std::path::Path) -> anyhow::Result<()> {
        let root_path = SanitineopilotPath::from(path);
                ...
                g.add(move |event: &notify::Event| {
                    ...
                    let mut path_events = event
                        ...
                        .filter_map(|event_path| {
                            let event_path = SanitineopilotPath::from(event_path);
...
fn handle_event(event: Result<notify::Event, notify::Error>) {
    ...
    // https://github.com/khulnasoft-lab/neopilot/actions/runs/14085230504/job/39449448832
