...
use neopilot_actions::feedback::GiveFeedback;
use ui::{IconPosition, prelude::*};
use workspace::{ModalView, Workspace};

use crate::{EmailNeopilot, FileBugReport, OpenNeopilotRepo, RequestFeature};
...
impl Render for FeedbackModal {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let open_neopilot_repo =
            cx.listener(|_, _, window, cx| window.dispatch_action(Box::new(OpenNeopilotRepo), cx));
        ...
        v_flex()
            ...
            .child(Label::new("Thanks for using Neopilot! To share your experience with us, reach for the channel that's the most appropriate:"))
            .child(
            ...
            .child(
                Button::new("send-us_an-email", "Send an Email")
                    ...
                    .icon_position(IconPosition::Start)
                    .on_click(cx.listener(|_, _, window, cx| {
                        window.dispatch_action(Box::new(EmailNeopilot), cx);
                    })),
            )
            .child(
                Button::new("neopilot_repository", "GitHub Repository")
                    .full_width()
                    .icon(IconName::Github)
                    .icon_size(IconSize::XSmall)
                    .icon_color(Color::Muted)
                    .icon_position(IconPosition::Start)
                    .on_click(open_neopilot_repo),
            )
    }
}
