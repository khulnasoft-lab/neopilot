...
use client::{UserStore, neopilot_urls};
...
use language_model::{
    LanguageModelProviderTosView, LanguageModelRegistry, NEOPILOT_CLOUD_PROVIDER_ID, RequestUsage,
};
use neopilot_actions::agent::{OpenConfiguration, OpenOnboardingModal, ResetOnboarding};
use neopilot_actions::assistant::{OpenRulesLibrary, ToggleFocus};
use neopilot_actions::{DecreaseBufferFontSize, IncreaseBufferFontSize, ResetBufferFontSize};
use neopilot_llm_client::UsageLimit;
...
struct SerialineopilotAgentPanel {
    width: Option<Pixels>,
}
...
impl AgentPanel {
        ...
        self.pending_serialization = Some(cx.background_spawn(async move {
            KEY_VALUE_STORE
                .write_kvp(
                    ...
                    serde_json::to_string(&SerialineopilotAgentPanel { width })?,
                )
                .await?;
            anyhow::Ok(())
        }));
    ...
    pub fn load(
        ...
        cx.spawn(async move |cx| {
            ...
            let serialineopilot_panel = if let Some(panel) = cx
                .background_spawn(async move { KEY_VALUE_STORE.read_kvp(AGENT_PANEL_KEY) })
                .await
                .log_err()
                .flatten()
            {
                Some(serde_json::from_str::<SerialineopilotAgentPanel>(&panel)?)
            } else {
                None
            };
            ...
            let panel = workspace.update_in(cx, |workspace, window, cx| {
                ...
                if let Some(serialineopilot_panel) = serialineopilot_panel {
                    panel.update(cx, |panel, cx| {
                        panel.width = serialineopilot_panel.width.map(|w| w.round());
                        cx.notify();
                    });
                }
                panel
...
impl Panel for AgentPanel {
    ...
    fn icon(&self, _window: &Window, cx: &App) -> Option<IconName> {
        (self.enabled(cx) && AgentSettings::get_global(cx).button)
            .then_some(IconName::NeopilotAssistant)
    }
...
impl AgentPanel {
    ...
    fn render_toolbar(&self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
                ...
                Some(RequestUsage {
                    ...
                    limit: match limit {
                        proto::usage_limit::Variant::Limited(limited) => {
                            neopilot_llm_client::UsageLimit::Limited(limited.limit as i32)
                        }
                        proto::usage_limit::Variant::Unlimited(_) => {
                            neopilot_llm_client::UsageLimit::Unlimited
                        }
                    },
                })
            })
        ...
        let account_url = neopilot_urls::account_url(cx);
                ...
                Some(ContextMenu::build(window, cx, |mut menu, _window, _cx| {
                    ...
                    menu = menu
                        ...
                        .action(
                            ...
                            Box::new(neopilot_actions::Extensions {
                                category_filter: Some(
                                    neopilot_actions::ExtensionCategoryFilter::ContextServers,
                                ),
                            }),
                        )
                    ...
                    if let Some(usage) = last_usage {
                        menu = menu
                            ...
                            .custom_entry(
                                ...
                                move |_, cx| cx.open_url(&neopilot_urls::account_url(cx)),
                            )
                            .separator()
                    }
    ...
    fn should_render_upsell(&self, cx: &mut Context<Self>) -> bool {
        ...
        let is_using_neopilot_provider = self
            .thread
            .read(cx)
            .thread()
            .read(cx)
            .configured_model()
            .map_or(false, |model| {
                model.provider.id().0 == NEOPILOT_CLOUD_PROVIDER_ID
            });
        if !is_using_neopilot_provider {
            return false;
        }

        let plan = self.user_store.read(cx).current_plan();
        if matches!(plan, Some(Plan::NeopilotPro | Plan::NeopilotProTrial)) {
            return false;
        }
    ...
    fn render_young_account_upsell(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let contents = div()
            ...
            .flex()
            .flex_col()
            .child(Headline::new("Build better with Neopilot Pro").size(HeadlineSize::Small))
            .child(
            ...
            .child(
                Label::new(
                    "Use your own API keys, upgrade to Neopilot Pro or send an email to billing-support@neopilot.dev.",
                )
                .color(Color::Muted),
            )
            .child(
                h_flex()
                    ...
                    .child(
                        h_flex()
                            ...
                            .child(
                                Button::new("cta-button", "Upgrade to Neopilot Pro")
                                    .style(ButtonStyle::Transparent)
                                    .on_click(|_, _, cx| cx.open_url(&neopilot_urls::account_url(cx))),
                            ),
                    ),
            );
    ...
    fn render_trial_upsell(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let contents = div()
            ...
            .child(Headline::new("Build better with Neopilot Pro").size(HeadlineSize::Small))
            .child(
                Label::new("Try Neopilot Pro for free for 14 days - no credit card required.")
                    .size(LabelSize::Small),
            )
            .child(
                Label::new(
        ...
        Some(
            self.render_upsell_container(
                ...
                div()
                    ...
                    .flex_col()
                    .child(
                        Headline::new("Your Neopilot Pro trial has expired.")
    ...
    fn render_payment_required_error(&self, cx: &mut Context<Self>) -> AnyElement {
        const ERROR_MESSAGE: &str = "Free tier exceeded. Subscribe and add payment to continue using Neopilot LLMs. You'll be billed at cost for tokens used.";
    ...
    fn render_model_request_limit_reached_error(
        ...
        let error_message = match plan {
            ...
            Plan::NeopilotProTrial => {
                "Model request limit reached. Upgrade to Neopilot Pro for more requests."
            }
            Plan::Free => "Model request limit reached. Upgrade to Neopilot Pro for more requests.",
        };
        let call_to_action = match plan {
            Plan::NeopilotPro => "Upgrade to usage-based billing",
            Plan::NeopilotProTrial => "Upgrade to Neopilot Pro",
            Plan::Free => "Upgrade to Neopilot Pro",
        };
