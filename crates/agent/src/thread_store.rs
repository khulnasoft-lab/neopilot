...
pub struct ThreadStore {
    ...
    threads: Vec<SerialineopilotThreadMetadata>,
...
impl ThreadStore {
    ...
    pub fn unordered_threads(&self) -> impl Iterator<Item = &SerialineopilotThreadMetadata> {
        self.threads.iter()
    }

    pub fn reverse_chronological_threads(&self) -> Vec<SerialineopilotThreadMetadata> {
    ...
    pub fn create_thread_from_serialineopilot(
        &mut self,
        serialineopilot: SerialineopilotThread,
        cx: &mut Context<Self>,
    ) -> Entity<Thread> {
        cx.new(|cx| {
            Thread::deserialize(
    ...
    pub fn save_thread(&self, thread: &Entity<Thread>, cx: &mut Context<Self>) -> Task<Result<()>> {
        let (metadata, serialineopilot_thread) =
            thread.update(cx, |thread, cx| (thread.id().clone(), thread.serialize(cx)));

        let database_future = ThreadsDatabase::global_future(cx);
        cx.spawn(async move |this, cx| {
            let serialineopilot_thread = serialineopilot_thread.await?;
            let database = database_future.await.map_err(|err| anyhow!(err))?;
            database
                .save_thread(metadata, serialineopilot_thread)
                .await?;
...
pub struct SerialineopilotThreadMetadata {
    pub id: ThreadId,
...
pub struct SerialineopilotThread {
    pub version: String,
    pub summary: SharedString,
    pub updated_at: DateTime<Utc>,
    pub messages: Vec<SerialineopilotMessage>,
    #[serde(default)]
    ...
    #[serde(default)]
    pub model: Option<SerialineopilotLanguageModel>,
    #[serde(default)]
...
#[derive(Serialize, Deserialize, Debug)]
pub struct SerialineopilotLanguageModel {
    pub provider: String,
    pub model: String,
}

impl SerialineopilotThread {
    ...
    pub fn from_json(json: &[u8]) -> Result<Self> {
        ...
        match saved_thread_json.get("version") {
            Some(serde_json::Value::String(version)) => match version.as_str() {
                SerialineopilotThreadV0_1_0::VERSION => {
                    let saved_thread =
                        serde_json::from_value::<SerialineopilotThreadV0_1_0>(saved_thread_json)?;
                    Ok(saved_thread.upgrade())
                }
                SerialineopilotThread::VERSION => Ok(serde_json::from_value::<
                    SerialineopilotThread,
                >(saved_thread_json)?),
                _ => anyhow::bail!("unrecognineopilot serialineopilot thread version: {version:?}"),
            },
            None => {
                let saved_thread =
                    serde_json::from_value::<LegacySerialineopilotThread>(saved_thread_json)?;
                Ok(saved_thread.upgrade())
            }
            version => {
                anyhow::bail!("unrecognineopilot serialineopilot thread version: {version:?}")
            }
        }
    }
}
...
#[derive(Serialize, Deserialize, Debug)]
pub struct SerialineopilotThreadV0_1_0(
    // The structure did not change, so we are reusing the latest SerialineopilotThread.
    // When making the next version, make sure this points to SerialineopilotThreadV0_2_0
    SerialineopilotThread,
);

impl SerialineopilotThreadV0_1_0 {
    pub const VERSION: &'static str = "0.1.0";

    pub fn upgrade(self) -> SerialineopilotThread {
        debug_assert_eq!(SerialineopilotThread::VERSION, "0.2.0");

        let mut messages: Vec<SerialineopilotMessage> = Vec::with_capacity(self.0.messages.len());
        ...
        SerialineopilotThread { messages, ..self.0 }
    }
}

#[derive(Debug, Serialize, Deserialize)]
pub struct SerialineopilotMessage {
    pub id: MessageId,
    pub role: Role,
    #[serde(default)]
    pub segments: Vec<SerialineopilotMessageSegment>,
    #[serde(default)]
    pub tool_uses: Vec<SerialineopilotToolUse>,
    #[serde(default)]
    pub tool_results: Vec<SerialineopilotToolResult>,
    #[serde(default)]
    pub context: String,
    #[serde(default)]
    pub creases: Vec<SerialineopilotCrease>,
    #[serde(default)]
    pub is_hidden: bool,
}
...
#[derive(Debug, Serialize, Deserialize)]
#[serde(tag = "type")]
pub enum SerialineopilotMessageSegment {
    #[serde(rename = "text")]
    Text {
        text: String,
    },
...
#[derive(Debug, Serialize, Deserialize)]
pub struct SerialineopilotToolUse {
...
#[derive(Debug, Serialize, Deserialize)]
pub struct SerialineopilotToolResult {
...
struct LegacySerialineopilotThread {
    pub summary: SharedString,
    pub updated_at: DateTime<Utc>,
    pub messages: Vec<LegacySerialineopilotMessage>,
    #[serde(default)]
    pub initial_project_snapshot: Option<Arc<ProjectSnapshot>>,
}

impl LegacySerialineopilotThread {
    pub fn upgrade(self) -> SerialineopilotThread {
        SerialineopilotThread {
            version: SerialineopilotThread::VERSION.to_string(),
...
#[derive(Debug, Serialize, Deserialize)]
struct LegacySerialineopilotMessage {
    pub id: MessageId,
    pub role: Role,
    pub text: String,
    #[serde(default)]
...
impl LegacySerialineopilotMessage {
    fn upgrade(self) -> SerialineopilotMessage {
        SerialineopilotMessage {
            id: self.id,
            role: self.role,
            segments: vec![SerialineopilotMessageSegment::Text { text: self.text }],
...
#[derive(Debug, Serialize, Deserialize)]
pub struct SerialineopilotCrease {
    pub start: usize,
    pub end: usize,
...
pub(crate) struct ThreadsDatabase {
    ...
    env: heed::Env,
    threads: Database<SerdeBincode<ThreadId>, SerialineopilotThread>,
}

impl heed::BytesEncode<'_> for SerialineopilotThread {
    type EItem = SerialineopilotThread;
...
impl<'a> heed::BytesDecode<'a> for SerialineopilotThread {
    type DItem = SerialineopilotThread;

    fn bytes_decode(bytes: &'a [u8]) -> Result<Self::DItem, heed::BoxedError> {
        // We implement this type manually because we want to call `SerialineopilotThread::from_json`,
        // instead of the Deserialize trait implementation for `SerialineopilotThread`.
        SerialineopilotThread::from_json(bytes).map_err(Into::into)
    }
}
...
impl ThreadsDatabase {
    ...
    pub fn list_threads(&self) -> Task<Result<Vec<SerialineopilotThreadMetadata>>> {
        ...
        self.executor.spawn(async move {
            ...
            while let Some((key, value)) = iter.next().transpose()? {
                threads.push(SerialineopilotThreadMetadata {
                    id: key,
    ...
    pub fn try_find_thread(&self, id: ThreadId) -> Task<Result<Option<SerialineopilotThread>>> {
        let env = self.env.clone();
        let threads = self.threads;
    ...
    pub fn save_thread(&self, id: ThreadId, thread: SerialineopilotThread) -> Task<Result<()>> {
        let env = self.env.clone();
        let threads = self.threads;
