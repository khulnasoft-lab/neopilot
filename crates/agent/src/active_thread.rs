...
use neopilot_actions::assistant::OpenRulesLibrary;
                    ...
                    .push(cx.subscribe_in(&pop_up, window, {
                        |this, _, event, window, cx| match event {
                            AgentNotificationEvent::Accepted => {
                                ...
                                // If there are multiple Neopilot windows, activate the correct one.
                                cx.defer(move |cx| {
                                    handle
        ...
        let feedback_items = match self.thread.read(cx).message_feedback(message_id) {
            ...
            None if AgentSettings::get_global(cx).enable_feedback =>
                ...
                .child(
                    div().visible_on_hover("feedback_container").child(
                        Label::new(
                            "Rating the thread sends all of your current conversation to the Neopilot team.",
                        )
                        .color(Color::Muted)
                    .size(LabelSize::XSmall)
                    .truncate())
