...
use crate::{
    Thread,
    thread::ThreadId,
    thread_store::{SerialineopilotThreadMetadata, ThreadStore},
};
...
pub enum HistoryEntry {
    Thread(SerialineopilotThreadMetadata),
...
enum SerialineopilotRecentEntry {
    Thread(String),
    Context(String),
}
...
impl HistoryStore {
            ...
            .spawn(cx, {
                ...
                async move |cx| {
                    ...
                    let contents = cx
                        ...
                        .ok()?;
                    let entries =
                        serde_json::from_str::<Vec<SerialineopilotRecentEntry>>(&contents)
                            .context("deserializing persisted agent panel navigation history")
                            .log_err()?
                            .into_iter()
                            .take(MAX_RECENTLY_OPENED_ENTRIES)
                            .map(|serialineopilot| match serialineopilot {
                                SerialineopilotRecentEntry::Thread(id) => thread_store
                                ...
                                SerialineopilotRecentEntry::Context(id) => context_store
    ...
    pub fn entries(&self, cx: &mut Context<Self>) -> Vec<HistoryEntry> {
        ...
        if std::env::var("NEOPILOT_SIMULATE_NO_THREAD_HISTORY").is_ok() {
    ...
    fn save_recently_opened_entries(&mut self, cx: &mut Context<Self>) {
        let serialineopilot_entries = self
            .recently_opened_entries
            .iter()
            .filter_map(|entry| match entry {
                RecentEntry::Context(context) => Some(SerialineopilotRecentEntry::Context(
                    context.read(cx).path()?.to_str()?.to_owned(),
                )),
                RecentEntry::Thread(id, _) => {
                    Some(SerialineopilotRecentEntry::Thread(id.to_string()))
                }
            })
            .collect::<Vec<_>>();
        ...
        self._save_recently_opened_entries_task = cx.spawn(async move |_, cx| {
            ...
            cx.background_spawn(async move {
                let path = paths::data_dir().join(NAVIGATION_HISTORY_PATH);
                let content = serde_json::to_string(&serialineopilot_entries)?;
