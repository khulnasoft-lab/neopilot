...
use feature_flags::{FeatureFlagAppExt as _, NeopilotProFeatureFlag};
use fs::Fs;
use gpui::{
                        ...
                        WithRemSize::new(ui_font_size)
                            ...
                            .map(|el| {
                                ...
                                if error.error_code() == proto::ErrorCode::RateLimitExceeded
                                    && cx.has_flag::<NeopilotProFeatureFlag>()
                                {
                                    el.child(
                                        v_flex()
                                            .child(
...
impl<T: 'static> PromptEditor<T> {
    ...
    pub fn placeholder_text(mode: &PromptEditorMode, window: &mut Window, cx: &mut App) -> String {
        ...
        let agent_panel_keybinding =
            ui::text_for_action(&neopilot_actions::assistant::ToggleFocus, window, cx)
    ...
    fn render_rate_limit_notice(&self, cx: &mut Context<Self>) -> impl IntoElement {
        Popover::new().child(
            v_flex()
                ...
                .child(Label::new(
                    "Try Neopilot Pro for higher limits, a wider range of models, and more.",
                ))
                .child(
                    h_flex()
                        .justify_between()
    ...
    fn handle_codegen_changed(
        ...
        match self.codegen_status(cx) {
            ...
            CodegenStatus::Error(error) => {
                if cx.has_flag::<NeopilotProFeatureFlag>()
