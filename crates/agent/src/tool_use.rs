...
use crate::thread_store::SerialineopilotMessage;
...
impl ToolUseState {
    ...
    /// Constructs a [`ToolUseState`] from the given list of [`SerialineopilotMessage`]s.
    ///
    /// Accepts a function to filter the tools that should be used to populate the state.
    ///
    /// If `window` is `None` (e.g., when in headless mode or when running evals),
    /// tool cards won't be deserialineopilot
    pub fn from_serialineopilot_messages(
        tools: Entity<ToolWorkingSet>,
        messages: &[SerialineopilotMessage],
