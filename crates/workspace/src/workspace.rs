...
pub use pane_group::*;
use persistence::{
    DB, SerialineopilotWindowBounds,
    model::{SerialineopilotSshProject, SerialineopilotWorkspace},
};
pub use persistence::{
    DB as WORKSPACE_DB, WorkspaceDb, delete_unloaded_items,
    model::{ItemId, LocalPaths, SerialineopilotWorkspaceLocation},
};
...
pub use ui;
use ui::prelude::*;
use util::{ResultExt, TryFutureExt, paths::SanitineopilotPath, serde::default_true};
use uuid::Uuid;
...
use crate::persistence::{
    SerialineopilotAxis,
    model::{
        DockData, DockStructure, SerialineopilotItem, SerialineopilotPane, SerialineopilotPaneGroup,
    },
};

pub const SERIALIZATION_THROTTLE_TIME: Duration = Duration::from_millis(200);

static NEOPILOT_WINDOW_SIZE: LazyLock<Option<Size<Pixels>>> = LazyLock::new(|| {
    env::var("NEOPILOT_WINDOW_SIZE")
        .ok()
        .as_deref()
        .and_then(parse_pixel_size_env_var)
});

static NEOPILOT_WINDOW_POSITION: LazyLock<Option<Point<Pixels>>> = LazyLock::new(|| {
    env::var("NEOPILOT_WINDOW_POSITION")
        .ok()
        .as_deref()
...
pub fn register_serializable_item<I: SerializableItem>(cx: &mut App) {
    let serialineopilot_item_kind = I::serialineopilot_item_kind();
    ...
    registry
        .descriptors_by_kind
        .insert(Arc::from(serialineopilot_item_kind), descriptor);
    registry
        .descriptors_by_type
...
pub struct Workspace {
    ...
    serialineopilot_ssh_project: Option<SerialineopilotSshProject>,
...
impl Workspace {
    ...
    pub fn new(
                                ...
                                if let Some(database_id) = workspace_id {
                                    cx.background_executor()
                                        .spawn(DB.set_window_open_status(
                                            database_id,
                                            SerialineopilotWindowBounds(window_bounds),
                                            display_uuid,
                                        ))
        ...
        Workspace {
            ...
            serialineopilot_ssh_project: None,
        }
    }
    ...
    > {
        ...
        cx.spawn(async move |cx| {
            ...
            let serialineopilot_workspace =
                persistence::DB.workspace_for_roots(paths_to_open.as_slice());

            let workspace_location = serialineopilot_workspace
                .as_ref()
                .map(|ws| &ws.location)
                .and_then(|loc| match loc {
                    SerialineopilotWorkspaceLocation::Local(_, order) => {
            ...
            let workspace_id =
                if let Some(serialineopilot_workspace) = serialineopilot_workspace.as_ref() {
                    serialineopilot_workspace.id
                } else {
            ...
            let window = if let Some(window) = requesting_window {
                ...
                let (window_bounds, display) = if let Some(bounds) = window_bounds_override {
                    ...
                    let restorable_bounds = serialineopilot_workspace
                        .as_ref()
                    ...
                    if let Some((serialineopilot_display, serialineopilot_status)) =
                        restorable_bounds
                    {
                        (
                            Some(serialineopilot_status.0),
                            Some(serialineopilot_display),
                        )
                    } else {
                        (None, None)
                    }
                ...
                // Use the serialineopilot workspace to construct the new window
                let mut options = cx.update(|cx| (app_state.build_window_options)(display, cx))?;
                options.window_bounds = window_bounds;
                let centered_layout = serialineopilot_workspace
                    .as_ref()
            ...
            let opened_items = window
                .update(cx, |_workspace, window, cx| {
                    open_items(serialineopilot_workspace, project_paths, window, cx)
                })?
                .await
                .unwrap_or_default();
    ...
    pub fn serialineopilot_ssh_project(&self) -> Option<SerialineopilotSshProject> {
        self.serialineopilot_ssh_project.clone()
    }

    pub fn set_serialineopilot_ssh_project(
        &mut self,
        serialineopilot_ssh_project: SerialineopilotSshProject,
    ) {
        self.serialineopilot_ssh_project = Some(serialineopilot_ssh_project);
    }
    ...
    pub fn open_paths(
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            for abs_path in &abs_paths {
                ...
                let this = this.clone();
                let abs_path: Arc<Path> = SanitineopilotPath::from(abs_path.clone()).into();
                let fs = fs.clone();
                let pane = pane.clone();
    ...
    async fn add_view_from_leader(
        ...
        let item = if let Some(existing_item) = existing_item {
            ...
            let Some(task) = task else {
                anyhow::bail!(
                    "failed to construct view from leader (maybe from a different version of neopilot?)"
                );
            };
    ...
    fn serialize_workspace_internal(&self, window: &mut Window, cx: &mut App) -> Task<()> {
        ...
        fn serialize_pane_handle(
                ...
                (
                    pane.items()
                        .filter_map(|handle| {
                            ...
                            Some(SerialineopilotItem {
                                kind: Arc::from(handle.serialineopilot_item_kind()),
            ...
            SerialineopilotPane::new(items, active, pinned_count)
        }

        fn build_serialineopilot_pane_group(
            pane_group: &Member,
            window: &mut Window,
            cx: &mut App,
        ) -> SerialineopilotPaneGroup {
            match pane_group {
                Member::Axis(PaneAxis {
                    ...
                    children: members
                        .iter()
                        .map(|member| build_serialineopilot_pane_group(member, window, cx))
                        .collect::<Vec<_>>(),
                    flexes: Some(flexes.lock().clone()),
                },
                Member::Pane(pane_handle) => {
                    SerialineopilotPaneGroup::Pane(serialize_pane_handle(pane_handle, window, cx))
                }
            }
        }

        fn build_serialineopilot_docks(
            this: &Workspace,
            window: &mut Window,
            cx: &mut App,
        ) -> DockStructure {
        ...
        if let Some(location) = self.serialize_workspace_location(cx) {
            ...
            let center_group = build_serialineopilot_pane_group(&self.center.root, window, cx);
            let docks = build_serialineopilot_docks(self, window, cx);
            let window_bounds = Some(SerialineopilotWindowBounds(window.window_bounds()));
            let serialineopilot_workspace = SerialineopilotWorkspace {
                id: database_id,
                location,
                center_group,
                window_bounds,
            ...
            return window.spawn(cx, async move |_| {
                persistence::DB
                    .save_workspace(serialineopilot_workspace)
                    .await;
            });
        }
        Task::ready(())
    ...
    fn serialize_workspace_location(&self, cx: &App) -> Option<SerialineopilotWorkspaceLocation> {
        if let Some(ssh_project) = &self.serialineopilot_ssh_project {
            Some(SerialineopilotWorkspaceLocation::Ssh(ssh_project.clone()))
        } else if let Some(local_paths) = self.local_paths(cx) {
            if !local_paths.is_empty() {
                Some(SerialineopilotWorkspaceLocation::from_local_paths(
                    local_paths,
                ))
            } else {
                None
    ...
    pub(crate) fn load_workspace(
        serialineopilot_workspace: SerialineopilotWorkspace,
        ...
        cx.spawn_in(window, async move |workspace, cx| {
            ...
            if let Some((group, active_pane, items)) = serialineopilot_workspace
                .center_group
                .deserialize(
                    &project,
                    serialineopilot_workspace.id,
            ...
            let mut all_deserialineopilot_items = Vec::default();
            cx.update(|_, cx| {
                for item in center_items.unwrap_or_default().into_iter().flatten() {
                    if let Some(serializable_item_handle) = item.to_serializable_item_handle(cx) {
                        item_ids_by_kind
                            .entry(serializable_item_handle.serialineopilot_item_kind())
                            .or_insert(Vec::new())
                    ...
                    all_deserialineopilot_items.push(item);
                }
            })?;
            ...
            workspace.update_in(cx, |workspace, window, cx| {
                ...
                let docks = serialineopilot_workspace.docks;

                for (dock, serialineopilot_dock) in [
                ...
                {
                    dock.update(cx, |dock, cx| {
                        dock.serialineopilot_dock = Some(serialineopilot_dock.clone());
                ...
                .update(cx, |project, cx| {
                    project
                        ...
                        .update(cx, |breakpoint_store, cx| {
                            breakpoint_store.with_serialineopilot_breakpoints(
                                serialineopilot_workspace.breakpoints,
                                cx,
                            )
                        })
                })?
...
fn window_bounds_env_override() -> Option<Bounds<Pixels>> {
    NEOPILOT_WINDOW_POSITION
        .zip(*NEOPILOT_WINDOW_SIZE)
...
fn open_items(
    serialineopilot_workspace: Option<SerialineopilotWorkspace>,
    mut project_paths_to_open: Vec<(PathBuf, Option<ProjectPath>)>,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) -> impl 'static + Future<Output = Result<Vec<Option<Result<Box<dyn ItemHandle>>>>>> + use<> {
    let restored_items = serialineopilot_workspace.map(|serialineopilot_workspace| {
        Workspace::load_workspace(
            serialineopilot_workspace,
...
pub async fn last_opened_workspace_location() -> Option<SerialineopilotWorkspaceLocation> {
...
) -> Option<Vec<SerialineopilotWorkspaceLocation>> {
                    ...
                    .update(cx, |_, window, cx| {
                        let detail: SharedString = match err.error_code() {
                            ...
                            ErrorCode::UpgradeRequired => {
                                "Your are running an unsupported version of Neopilot. Please update to continue.".into()
                            }
...
pub async fn get_any_active_workspace(
    ...
    activate_any_workspace_window(&mut cx).context("could not open neopilot")
}
...
pub fn open_ssh_project_with_new_connection(
    ...
    cx.spawn(async move |cx| {
        let (serialineopilot_ssh_project, workspace_id, serialineopilot_workspace) =
...
async fn open_ssh_project_inner(
    ...
    cx.update_window(window.into(), |_, window, cx| {
        window.replace_root(cx, |window, cx| {
            ...
            workspace.set_serialineopilot_ssh_project(serialineopilot_ssh_project);
            workspace.update_history(cx);
            workspace
        });
    })?;
    ...
    window
        .update(cx, |_, window, cx| {
            window.activate_window();
            open_items(serialineopilot_workspace, project_paths_to_open, window, cx)
        })?
        .await?;
...
fn serialize_ssh_project(
    ...
    Result<(
        SerialineopilotSshProject,
        WorkspaceId,
        Option<SerialineopilotWorkspace>,
    )>,
> {
    cx.background_spawn(async move {
        let serialineopilot_ssh_project = persistence::DB
        ...
        let serialineopilot_workspace =
            persistence::DB.workspace_for_ssh_project(&serialineopilot_ssh_project);

        let workspace_id = if let Some(workspace_id) = serialineopilot_workspace
            .as_ref()
...
pub fn ssh_workspace_position_from_db(
    ...
    cx.background_spawn(async move {
        let serialineopilot_ssh_project = persistence::DB
            ...
            .await
            .context("fetching serialineopilot ssh project")?;
        let serialineopilot_workspace =
            persistence::DB.workspace_for_ssh_project(&serialineopilot_ssh_project);
        ...
        let (window_bounds, display) = if let Some(bounds) = window_bounds_env_override() {
            ...
            if let Some((serialineopilot_display, serialineopilot_status)) = restorable_bounds {
                (
                    Some(serialineopilot_status.0),
                    Some(serialineopilot_display),
                )
