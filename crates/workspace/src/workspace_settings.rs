...
pub enum RestoreOnStartupBehavior {
    ...
    LastWorkspace,
    /// Restore all workspaces that were open when quitting Neopilot.
    #[default]
    LastSession,
}
...
pub struct WorkspaceSettingsContent {
    ...
    /// Controls previous session restoration in freshly launched Neopilot instance.
    ...
    /// When set to false, Neopilot will use the built-in keyboard-first pickers.
    ///
    /// Default: true
    pub use_system_path_prompts: Option<bool>,
    /// Whether to use the system provided prompts.
    /// When set to false, Neopilot will use the built-in prompts.
    /// Note that this setting has no effect on Linux, where Neopilot will always
