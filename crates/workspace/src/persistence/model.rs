use super::{SerialineopilotAxis, SerialineopilotWindowBounds};
use crate::{
...
use util::{ResultExt, paths::SanitineopilotPath};
use uuid::Uuid;

#[derive(Debug, Clone, PartialEq, Deserialize, Serialize)]
pub struct SerialineopilotSshProject {
    pub id: SshProjectId,
    pub host: String,
    pub port: Option<u16>,
...
impl SerialineopilotSshProject {
...
impl StaticColumnCount for SerialineopilotSshProject {
    fn column_count() -> usize {
        5
    }
}

impl Bind for &SerialineopilotSshProject {
...
impl Column for SerialineopilotSshProject {
...
impl LocalPaths {
    pub fn new<P: AsRef<Path>>(paths: impl IntoIterator<Item = P>) -> Self {
        let mut paths: Vec<PathBuf> = paths
            .into_iter()
            .map(|p| SanitineopilotPath::from(p).into())
            .collect();
        // Ensure all future `neopilot workspace1 workspace2` and `neopilot workspace2 workspace1` calls are using the same workspace.
...
#[derive(Debug, PartialEq, Clone)]
pub enum SerialineopilotWorkspaceLocation {
    Local(LocalPaths, LocalPathsOrder),
    Ssh(SerialineopilotSshProject),
}

impl SerialineopilotWorkspaceLocation {
    /// Create a new `SerialineopilotWorkspaceLocation` from a list of local paths.
    ///
    ...
    /// ```
    /// use std::path::Path;
    /// use neopilot_workspace::SerialineopilotWorkspaceLocation;
    ///
    /// let location = SerialineopilotWorkspaceLocation::from_local_paths(vec![
    ///     Path::new("path/to/workspace1"),
    ///     Path::new("path/to/workspace2"),
    /// ]);
    /// assert_eq!(location, SerialineopilotWorkspaceLocation::Local(
    ///    LocalPaths::new(vec![
    ///         Path::new("path/to/workspace1"),
    ///         Path::new("path/to/workspace2"),
    ///    ]),
    ...
    pub fn sorted_paths(&self) -> Arc<Vec<PathBuf>> {
        match self {
            SerialineopilotWorkspaceLocation::Local(paths, order) => {
            ...
            SerialineopilotWorkspaceLocation::Ssh(ssh_project) => Arc::new(ssh_project.ssh_urls()),
        }
    }
}

#[derive(Debug, PartialEq, Clone)]
pub(crate) struct SerialineopilotWorkspace {
    pub(crate) id: WorkspaceId,
    pub(crate) location: SerialineopilotWorkspaceLocation,
    pub(crate) center_group: SerialineopilotPaneGroup,
    pub(crate) window_bounds: Option<SerialineopilotWindowBounds>,
...
#[derive(Debug, PartialEq, Clone)]
pub(crate) enum SerialineopilotPaneGroup {
    Group {
        axis: SerialineopilotAxis,
        flexes: Option<Vec<f32>>,
        children: Vec<SerialineopilotPaneGroup>,
    },
    Pane(SerialineopilotPane),
}

#[cfg(test)]
impl Default for SerialineopilotPaneGroup {
    fn default() -> Self {
        Self::Pane(SerialineopilotPane {
            children: vec![SerialineopilotItem::default()],
            active: false,
            pinned_count: 0,
        })
    }
...
impl SerialineopilotPaneGroup {
    ...
    pub(crate) async fn deserialize(
        ...
        match self {
            ...
            SerialineopilotPaneGroup::Pane(serialineopilot_pane) => {
                let pane = workspace
                    .update_in(cx, |workspace, window, cx| {
                        workspace.add_pane(window, cx).downgrade()
                    })
                    .log_err()?;
                let active = serialineopilot_pane.active;
                let new_items = serialineopilot_pane
...
pub struct SerialineopilotPane {
    pub(crate) active: bool,
    pub(crate) children: Vec<SerialineopilotItem>,
    pub(crate) pinned_count: usize,
}

impl SerialineopilotPane {
    pub fn new(children: Vec<SerialineopilotItem>, active: bool, pinned_count: usize) -> Self {
        SerialineopilotPane {
            children,
            active,
            pinned_count,
...
pub struct SerialineopilotItem {
    pub kind: Arc<str>,
    pub item_id: ItemId,
    pub active: bool,
    pub preview: bool,
...
#[cfg(test)]
impl Default for SerialineopilotItem {
    fn default() -> Self {
        SerialineopilotItem {
...
impl StaticColumnCount for SerialineopilotItem {
    fn column_count() -> usize {
        4
    }
}
impl Bind for &SerialineopilotItem {
    fn bind(&self, statement: &Statement, start_index: i32) -> Result<i32> {
...
impl Column for SerialineopilotItem {
    fn column(statement: &mut Statement, start_index: i32) -> Result<(Self, i32)> {
...
mod tests {
    ...
    fn test_serialize_local_paths() {
        let paths = vec!["b", "a", "c"];
        let serialineopilot = SerialineopilotWorkspaceLocation::from_local_paths(paths);
    ...
    fn test_sorted_paths() {
        ...
        let order = vec![2, 0, 1];
        let serialineopilot = SerialineopilotWorkspaceLocation::Local(
            LocalPaths(paths.clone()),
            LocalPathsOrder(order),
        );
        assert_eq!(
            serialineopilot.sorted_paths(),
            Arc::new(vec![
        ...
        assert_eq!(serialineopilot.sorted_paths(), paths);

        let urls = ["/a", "/b", "/c"];
        let serialineopilot = SerialineopilotWorkspaceLocation::Ssh(SerialineopilotSshProject {
            id: SshProjectId(0),
            host: "host".to_string(),
            port: Some(22),
