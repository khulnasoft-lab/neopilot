...
use model::{
    GroupId, ItemId, LocalPaths, PaneId, SerialineopilotItem, SerialineopilotPane,
    SerialineopilotPaneGroup, SerialineopilotSshProject, SerialineopilotWorkspace,
};

use self::model::{DockStructure, LocalPathsOrder, SerialineopilotWorkspaceLocation};

#[derive(Copy, Clone, Debug, PartialEq)]
pub(crate) struct SerialineopilotAxis(pub(crate) gpui::Axis);
impl sqlez::bindable::StaticColumnCount for SerialineopilotAxis {}
impl sqlez::bindable::Bind for SerialineopilotAxis {
    fn bind(
        &self,
...
impl sqlez::bindable::Column for SerialineopilotAxis {
    fn column(
        ...
        String::column(statement, start_index).and_then(|(axis_text, next_index)| {
            Ok((
                match axis_text.as_str() {
                    ...
                    _ => anyhow::bail!("Stored serialineopilot item kind is incorrect"),
                },
                next_index,
            ))
        })
...
pub(crate) struct SerialineopilotWindowBounds(pub(crate) WindowBounds);

impl StaticColumnCount for SerialineopilotWindowBounds {
    fn column_count() -> usize {
        5
    }
}

impl Bind for SerialineopilotWindowBounds {
    fn bind(&self, statement: &Statement, start_index: i32) -> Result<i32> {
        match self.0 {
            WindowBounds::Windowed(bounds) => {
                ...
                statement.bind(
                    &(
                        SerialineopilotPixels(bounds.origin.x),
                        SerialineopilotPixels(bounds.origin.y),
                        SerialineopilotPixels(bounds.size.width),
                        SerialineopilotPixels(bounds.size.height),
                    ),
                    next_index,
                )
            }
            WindowBounds::Maximineopilot(bounds) => {
                let next_index = statement.bind(&"Maximineopilot", start_index)?;
                statement.bind(
                    &(
                        SerialineopilotPixels(bounds.origin.x),
                        SerialineopilotPixels(bounds.origin.y),
...
impl Column for SerialineopilotWindowBounds {
    fn column(statement: &mut Statement, start_index: i32) -> Result<(Self, i32)> {
        ...
        let status = match window_state.as_str() {
            "Windowed" | "Fixed" => SerialineopilotWindowBounds(WindowBounds::Windowed(bounds)),
            "Maximineopilot" => SerialineopilotWindowBounds(WindowBounds::Maximineopilot(bounds)),
            "FullScreen" => SerialineopilotWindowBounds(WindowBounds::Fullscreen(bounds)),
...
struct SerialineopilotPixels(gpui::Pixels);
impl sqlez::bindable::StaticColumnCount for SerialineopilotPixels {}

impl sqlez::bindable::Bind for SerialineopilotPixels {
    fn bind(
        &self,
        statement: &sqlez::statement::Statement,
        start_index: i32,
...
define_connection! {
    ...
    &[
        ...
        CREATE TABLE workspaces(
            ...
            dock_visible INTEGER, // Deprecated. Preserving so users can downgrade Neopilot.
            dock_anchor TEXT, // Deprecated. Preserving so users can downgrade Neopilot.
            dock_pane INTEGER, // Deprecated.  Preserving so users can downgrade Neopilot.
    ...
    // Preserving so users can downgrade Neopilot.
    sql!(
...
impl WorkspaceDb {
    /// Returns a serialineopilot workspace for the given worktree_roots. If the passed array
    /// is empty, the most recent workspace is returned instead. If no workspace for the
    /// passed roots is stored, returns none.
    pub(crate) fn workspace_for_roots<P: AsRef<Path>>(
        &self,
        worktree_roots: &[P],
    ) -> Option<SerialineopilotWorkspace> {
        ...
        let (
            ...
            Option<SerialineopilotWindowBounds>,
            Option<Uuid>,
            Option<bool>,
            DockStructure,
            Option<u64>,
        ...
        let location = match local_paths_order {
            Some(order) => SerialineopilotWorkspaceLocation::Local(local_paths, order),
            None => {
                let order = LocalPathsOrder::default_for_paths(&local_paths);
                SerialineopilotWorkspaceLocation::Local(local_paths, order)
            }
        };

        Some(SerialineopilotWorkspace {
            id: workspace_id,
            location,
            center_group: self
    ...
    pub(crate) fn workspace_for_ssh_project(
        &self,
        ssh_project: &SerialineopilotSshProject,
    ) -> Option<SerialineopilotWorkspace> {
        ...
        Some(SerialineopilotWorkspace {
            id: workspace_id,
            location: SerialineopilotWorkspaceLocation::Ssh(ssh_project.clone()),
            center_group: self
                .get_center_pane_group(workspace_id)
                .context("Getting center group")
                .log_err()?,
    ...
    pub(crate) async fn save_workspace(&self, workspace: SerialineopilotWorkspace) {
        ...
        self.write(move |conn| {
            conn.with_savepoint("update_worktrees", || {
                ...
                match workspace.location {
                    SerialineopilotWorkspaceLocation::Local(local_paths, local_paths_order) => {
                        conn.exec_bound(sql!(
                    ...
                    SerialineopilotWorkspaceLocation::Ssh(ssh_project) => {
                        conn.exec_bound(sql!(
                            DELETE FROM toolchains WHERE workspace_id = ?1;
    ...
    pub(crate) async fn get_or_create_ssh_project(
        ...
        paths: Vec<String>,
        user: Option<String>,
    ) -> Result<SerialineopilotSshProject> {
    ...
    query! {
        async fn get_ssh_project(host: String, port: Option<u16>, paths: String, user: Option<String>) -> Result<Option<SerialineopilotSshProject>> {
    ...
    query! {
        async fn insert_ssh_project(host: String, port: Option<u16>, paths: String, user: Option<String>) -> Result<Option<SerialineopilotSshProject>> {
    ...
    query! {
        fn ssh_projects() -> Result<Vec<SerialineopilotSshProject>> {
            SELECT id, host, port, paths, user
            FROM ssh_projects
        }
    }
    ...
    query! {
        fn ssh_project(id: u64) -> Result<SerialineopilotSshProject> {
            SELECT id, host, port, paths, user
            FROM ssh_projects
            WHERE id = ?
        }
    ...
    pub(crate) fn last_window(
        &self,
    ) -> anyhow::Result<(Option<Uuid>, Option<SerialineopilotWindowBounds>)> {
        let mut prepared_query = self
            .select::<(Option<Uuid>, Option<SerialineopilotWindowBounds>)>(sql!(
                SELECT
                display,
    ...
    pub async fn recent_workspaces_on_disk(
        &self,
    ) -> Result<Vec<(WorkspaceId, SerialineopilotWorkspaceLocation)>> {
        ...
        for (id, location, order, ssh_project_id) in self.recent_workspaces()? {
            if let Some(ssh_project_id) = ssh_project_id.map(SshProjectId) {
                if let Some(ssh_project) = ssh_projects.iter().find(|rp| rp.id == ssh_project_id) {
                    result.push((
                        id,
                        SerialineopilotWorkspaceLocation::Ssh(ssh_project.clone()),
                    ));
                } else {
            ...
            {
                result.push((id, SerialineopilotWorkspaceLocation::Local(location, order)));
            } else {
                delete_tasks.push(self.delete_workspace_by_id(id));
            }
        }
    ...
    pub async fn last_workspace(&self) -> Result<Option<SerialineopilotWorkspaceLocation>> {
        Ok(self
    ...
    // session was closed (i.e. when Neopilot was quit).
    ...
    pub fn last_session_workspace_locations(
    ...
    ) -> Result<Vec<SerialineopilotWorkspaceLocation>> {
            ...
            self.session_workspaces(last_session_id.to_owned())?
        {
            if let Some(ssh_project_id) = ssh_project_id {
                let location =
                    SerialineopilotWorkspaceLocation::Ssh(self.ssh_project(ssh_project_id)?);
                workspaces.push((location, window_id.map(WindowId::from)));
            } else if location.paths().iter().all(|path| path.exists())
                && location.paths().iter().any(|path| path.is_dir())
            {
                let location = SerialineopilotWorkspaceLocation::Local(location, order);
                workspaces.push((location, window_id.map(WindowId::from)));
            }
        }
    ...
    fn get_center_pane_group(&self, workspace_id: WorkspaceId) -> Result<SerialineopilotPaneGroup> {
        Ok(self
            .get_pane_group(workspace_id, None)?
            .into_iter()
            .next()
            .unwrap_or_else(|| {
                SerialineopilotPaneGroup::Pane(SerialineopilotPane {
                    active: true,
                    children: vec![],
                    pinned_count: 0,
                })
    ...
    fn get_pane_group(
    ...
    ) -> Result<Vec<SerialineopilotPaneGroup>> {
        type GroupKey = (Option<GroupId>, WorkspaceId);
        type GroupOrPane = (
            Option<GroupId>,
            Option<SerialineopilotAxis>,
            Option<PaneId>,
            Option<bool>,
            Option<usize>,
            Option<String>,
        ...
        .map(|(group_id, axis, pane_id, active, pinned_count, flexes)| {
            ...
            if let Some((group_id, axis)) = group_id.zip(axis) {
                ...
                Ok(SerialineopilotPaneGroup::Group {
                    axis,
                    children: self.get_pane_group(workspace_id, Some(group_id))?,
                    flexes,
                })
            } else if let Some((pane_id, active, pinned_count)) = maybe_pane {
                Ok(SerialineopilotPaneGroup::Pane(SerialineopilotPane::new(
        ...
        .filter(|pane_group| match pane_group {
            Ok(SerialineopilotPaneGroup::Group { children, .. }) => !children.is_empty(),
            Ok(SerialineopilotPaneGroup::Pane(pane)) => !pane.children.is_empty(),
            _ => true,
        })
        .collect::<Result<_>>()
    }
    ...
    fn save_pane_group(
        conn: &Connection,
        workspace_id: WorkspaceId,
        pane_group: &SerialineopilotPaneGroup,
        ...
        match pane_group {
            SerialineopilotPaneGroup::Group {
                axis,
                children,
                flexes,
            } => {
            ...
            SerialineopilotPaneGroup::Pane(pane) => {
    ...
    fn get_items(&self, pane_id: PaneId) -> Result<Vec<SerialineopilotItem>> {
    ...
    query! {
        pub(crate) async fn set_window_open_status(workspace_id: WorkspaceId, bounds: SerialineopilotWindowBounds, display: Uuid) -> Result<()> {
            UPDATE workspaces
            SET window_state = ?2,
                window_x = ?3,
                window_y = ?4,
...
mod tests {
    ...
    use super::*;
    use crate::persistence::model::SerialineopilotWorkspace;
    use crate::persistence::model::{
        SerialineopilotItem, SerialineopilotPane, SerialineopilotPaneGroup,
    };
    use gpui;
    ...
    async fn test_breakpoints() {
        ...
        let workspace = SerialineopilotWorkspace {
            id,
            location: SerialineopilotWorkspaceLocation::from_local_paths(["/tmp"]),
    ...
    async fn test_remove_last_breakpoint() {
        ...
        let workspace_without_breakpoint = SerialineopilotWorkspace {
            id,
            location: SerialineopilotWorkspaceLocation::from_local_paths(["/tmp"]),
            center_group: Default::default(),
            window_bounds: Default::default(),
    ...
    async fn test_workspace_id_stability() {
        ...
        let mut workspace_1 = SerialineopilotWorkspace {
            id: WorkspaceId(1),
            location: SerialineopilotWorkspaceLocation::from_local_paths(["/tmp", "/tmp2"]),
            center_group: Default::default(),
            window_bounds: Default::default(),
            display: Default::default(),
            docks: Default::default(),
        ...
        let workspace_2 = SerialineopilotWorkspace {
            id: WorkspaceId(2),
            location: SerialineopilotWorkspaceLocation::from_local_paths(["/tmp"]),
            center_group: Default::default(),
            window_bounds: Default::default(),
        ...
        workspace_1.location =
            SerialineopilotWorkspaceLocation::from_local_paths(["/tmp", "/tmp3"]);
        db.save_workspace(workspace_1.clone()).await;
    ...
    fn group(axis: Axis, children: Vec<SerialineopilotPaneGroup>) -> SerialineopilotPaneGroup {
        SerialineopilotPaneGroup::Group {
            axis: SerialineopilotAxis(axis),
            flexes: None,
            children,
        }
    }
    ...
    async fn test_full_workspace_serialization() {
        ...
        let center_group = group(
            ...
            vec![
                group(
                    Axis::Vertical,
                    vec![
                        SerialineopilotPaneGroup::Pane(SerialineopilotPane::new(
                            vec![
                                SerialineopilotItem::new("Terminal", 5, false, false),
                                SerialineopilotItem::new("Terminal", 6, true, false),
                            ],
                            false,
                            0,
                        )),
                        SerialineopilotPaneGroup::Pane(SerialineopilotPane::new(
                            vec![
                                SerialineopilotItem::new("Terminal", 7, true, false),
                                SerialineopilotItem::new("Terminal", 8, false, false),
                            ],
                            false,
                            0,
                        )),
                ...
                SerialineopilotPaneGroup::Pane(SerialineopilotPane::new(
                    vec![
                        SerialineopilotItem::new("Terminal", 9, false, false),
                        SerialineopilotItem::new("Terminal", 10, true, false),
                    ],
                    false,
                    0,
                )),
        ...
        let workspace = SerialineopilotWorkspace {
            id: WorkspaceId(5),
            location: SerialineopilotWorkspaceLocation::Local(
    ...
    async fn test_workspace_assignment() {
        ...
        let workspace_1 = SerialineopilotWorkspace {
            id: WorkspaceId(1),
            location: SerialineopilotWorkspaceLocation::Local(
                LocalPaths::new(["/tmp", "/tmp2"]),
        ...
        let mut workspace_2 = SerialineopilotWorkspace {
            id: WorkspaceId(2),
            location: SerialineopilotWorkspaceLocation::from_local_paths(["/tmp"]),
            center_group: Default::default(),
            window_bounds: Default::default(),
        ...
        workspace_2.location =
            SerialineopilotWorkspaceLocation::from_local_paths(["/tmp", "/tmp2"]);
        ...
        let mut workspace_3 = SerialineopilotWorkspace {
            id: WorkspaceId(3),
            location: SerialineopilotWorkspaceLocation::Local(
                LocalPaths::new(["/tmp", "/tmp2"]),
                LocalPathsOrder::new([1, 0]),
        ...
        workspace_3.location =
            SerialineopilotWorkspaceLocation::from_local_paths(["/tmp3", "/tmp4", "/tmp2"]);
        db.save_workspace(workspace_3.clone()).await;
    ...
    async fn test_session_workspaces() {
        ...
        let workspace_1 = SerialineopilotWorkspace {
            id: WorkspaceId(1),
            location: SerialineopilotWorkspaceLocation::from_local_paths(["/tmp1"]),
            center_group: Default::default(),
            window_bounds: Default::default(),
            display: Default::default(),
            docks: Default::default(),
        ...
        let workspace_2 = SerialineopilotWorkspace {
            id: WorkspaceId(2),
            location: SerialineopilotWorkspaceLocation::from_local_paths(["/tmp2"]),
            center_group: Default::default(),
            window_bounds: Default::default(),
            display: Default::default(),
            docks: Default::default(),
        ...
        let workspace_3 = SerialineopilotWorkspace {
            id: WorkspaceId(3),
            location: SerialineopilotWorkspaceLocation::from_local_paths(["/tmp3"]),
            center_group: Default::default(),
            window_bounds: Default::default(),
            display: Default::default(),
            docks: Default::default(),
        ...
        let workspace_4 = SerialineopilotWorkspace {
            id: WorkspaceId(4),
            location: SerialineopilotWorkspaceLocation::from_local_paths(["/tmp4"]),
            center_group: Default::default(),
            window_bounds: Default::default(),
            display: Default::default(),
            docks: Default::default(),
        ...
        let workspace_5 = SerialineopilotWorkspace {
            id: WorkspaceId(5),
            location: SerialineopilotWorkspaceLocation::Ssh(ssh_project.clone()),
            center_group: Default::default(),
            window_bounds: Default::default(),
        ...
        let workspace_6 = SerialineopilotWorkspace {
            id: WorkspaceId(6),
            location: SerialineopilotWorkspaceLocation::Local(
    ...
    fn default_workspace<P: AsRef<Path>>(
        workspace_id: &[P],
        center_group: &SerialineopilotPaneGroup,
    ) -> SerialineopilotWorkspace {
        SerialineopilotWorkspace {
            id: WorkspaceId(4),
            location: SerialineopilotWorkspaceLocation::from_local_paths(workspace_id),
    ...
    async fn test_last_session_workspace_locations() {
        ...
        .into_iter()
        .map(
            |(id, locations, order, window_id)| SerialineopilotWorkspace {
                id: WorkspaceId(id),
                location: SerialineopilotWorkspaceLocation::Local(
        ...
        assert_eq!(
            have[0],
            SerialineopilotWorkspaceLocation::from_local_paths(&[dir4.path()])
        );
        assert_eq!(
            have[1],
            SerialineopilotWorkspaceLocation::from_local_paths([dir3.path()])
        );
        assert_eq!(
            have[2],
            SerialineopilotWorkspaceLocation::from_local_paths([dir2.path()])
        );
        assert_eq!(
            have[3],
            SerialineopilotWorkspaceLocation::from_local_paths([dir1.path()])
        );
        assert_eq!(
            have[4],
            SerialineopilotWorkspaceLocation::Local(
    ...
    async fn test_last_session_workspace_locations_ssh_projects() {
        ...
        .into_iter()
        .map(|(id, ssh_project, window_id)| SerialineopilotWorkspace {
            id: WorkspaceId(id),
            location: SerialineopilotWorkspaceLocation::Ssh(ssh_project),
            center_group: Default::default(),
            window_bounds: Default::default(),
            display: Default::default(),
            docks: Default::default(),
        ...
        assert_eq!(
            have[0],
            SerialineopilotWorkspaceLocation::Ssh(ssh_projects[3].clone())
        );
        assert_eq!(
            have[1],
            SerialineopilotWorkspaceLocation::Ssh(ssh_projects[2].clone())
        );
        assert_eq!(
            have[2],
            SerialineopilotWorkspaceLocation::Ssh(ssh_projects[1].clone())
        );
        assert_eq!(
            have[3],
            SerialineopilotWorkspaceLocation::Ssh(ssh_projects[0].clone())
        );
    }
            ...
            vec![
                group(
                    ...
                    vec![
                        SerialineopilotPaneGroup::Pane(SerialineopilotPane::new(
                            vec![
                                SerialineopilotItem::new("Terminal", 1, false, false),
                                SerialineopilotItem::new("Terminal", 2, true, false),
                            ],
                            false,
                            0,
                        )),
                        SerialineopilotPaneGroup::Pane(SerialineopilotPane::new(
                            vec![
                                SerialineopilotItem::new("Terminal", 4, false, false),
                                SerialineopilotItem::new("Terminal", 3, true, false),
                            ],
                            true,
                            0,
                        )),
                ...
                SerialineopilotPaneGroup::Pane(SerialineopilotPane::new(
                    vec![
                        SerialineopilotItem::new("Terminal", 5, true, false),
                        SerialineopilotItem::new("Terminal", 6, false, false),
                    ],
                    false,
                    0,
                )),
    ...
    async fn test_cleanup_panes() {
        ...
        workspace.center_group = group(
            ...
            vec![
                ...
                SerialineopilotPaneGroup::Pane(SerialineopilotPane::new(
                    vec![
                        SerialineopilotItem::new("Terminal", 4, true, false),
                        SerialineopilotItem::new("Terminal", 3, false, false),
                    ],
                    true,
                    0,
                )),
