...
pub struct Dock {
    ...
    pub(crate) serialineopilot_dock: Option<DockData>,
...
impl Dock {
    ...
    pub fn restore_state(&mut self, window: &mut Window, cx: &mut Context<Self>) -> bool {
        if let Some(serialineopilot) = self.serialineopilot_dock.clone() {
            if let Some(active_panel) = serialineopilot
                .active_panel
                .filter(|_| serialineopilot.visible)
            {
            ...
            if serialineopilot.zoom {
                if let Some(panel) = self.active_panel() {
                    panel.set_zoomed(true, window, cx)
                }
            }
            self.set_open(serialineopilot.visible, window, cx);
            return true;
        }
        false
    }
