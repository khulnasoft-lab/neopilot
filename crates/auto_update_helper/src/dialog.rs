...
pub(crate) fn create_dialog_window(receiver: Receiver<Result<()>>) -> Result<HWND> {
    unsafe {
        let class_name = windows::core::w!("Neopilot-Auto-Updater-Dialog-Class");
        ...
        let hwnd = CreateWindowExW(
            WS_EX_TOPMOST,
            class_name,
            windows::core::w!("Neopilot Editor"),
...
unsafe extern "system" fn wnd_proc(
    ...
    match msg {
        ...
        WM_PAINT => unsafe {
            ...
            let string = HSTRING::from("Neopilot Editor is updating...");
        ...
        WM_TERMINATE => {
            with_dialog_data(hwnd, |data| {
                if let Ok(result) = data.borrow_mut().rx.recv() {
                    if let Err(e) = result {
                        log::error!("Failed to update Neopilot: {:?}", e);
