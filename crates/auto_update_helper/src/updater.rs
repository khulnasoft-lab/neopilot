...
pub(crate) const JOBS: [Job; 6] = [
    // Delete old files
    |app_dir| {
        let neopilot_executable = app_dir.join("Neopilot.exe");
        log::info!("Removing old file: {}", neopilot_executable.display());
        std::fs::remove_file(&neopilot_executable).context(format!(
            "Failed to remove old file {}",
            neopilot_executable.display()
        ))
    },
    |app_dir| {
        let neopilot_cli = app_dir.join("bin\\neopilot.exe");
        log::info!("Removing old file: {}", neopilot_cli.display());
        std::fs::remove_file(&neopilot_cli).context(format!(
            "Failed to remove old file {}",
            neopilot_cli.display()
        ))
    },
    // Copy new files
    |app_dir| {
        let neopilot_executable_source = app_dir.join("install\\Neopilot.exe");
        let neopilot_executable_dest = app_dir.join("Neopilot.exe");
        log::info!(
            "Copying new file {} to {}",
            neopilot_executable_source.display(),
            neopilot_executable_dest.display()
        );
        std::fs::copy(&neopilot_executable_source, &neopilot_executable_dest)
            .map(|_| ())
            .context(format!(
    ...
    |app_dir| {
        let neopilot_cli_source = app_dir.join("install\\bin\\neopilot.exe");
        let neopilot_cli_dest = app_dir.join("bin\\neopilot.exe");
        log::info!(
            "Copying new file {} to {}",
            neopilot_cli_source.display(),
            neopilot_cli_dest.display()
        );
        std::fs::copy(&neopilot_cli_source, &neopilot_cli_dest)
            .map(|_| ())
            .context(format!(
                "Failed to copy new file {} to {}",
                neopilot_cli_source.display(),
...
pub(crate) const JOBS: [Job; 2] = [
    |_| {
        ...
        if let Ok(config) = std::env::var("NEOPILOT_AUTO_UPDATE") {
            match config.as_str() {
                ...
                .context("Anyhow!"),
                _ => panic!("Unknown NEOPILOT_AUTO_UPDATE value: {}", config),
            }
        } else {
            Ok(())
        }
...
pub(crate) fn perform_update(app_dir: &Path, hwnd: Option<isize>) -> Result<()> {
    ...
    let _ = std::process::Command::new(app_dir.join("Neopilot.exe"))
...
mod test {
    ...
    fn test_perform_update() {
        ...
        // Simulate a timeout
        unsafe { std::env::set_var("NEOPILOT_AUTO_UPDATE", "err") };
