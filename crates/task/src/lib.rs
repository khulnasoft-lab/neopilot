//! Baseline interface of Tasks in Neopilot: all tasks in Neopilot are intended to use those for implementing their own logic.
...
pub use debug_format::{
    ...
    NeopilotDebugConfig, Request, TcpArgumentsTemplate,
};
pub use neopilot_actions::RevealTarget;
...
/// Contains all information needed by Neopilot to spawn a new terminal tab for the given task.
...
/// Variables, available for use in [`TaskContext`] when a Neopilot's [`TaskTemplate`] gets resolved into a [`ResolvedTask`].
...
impl FromStr for VariableName {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let without_prefix = s.strip_prefix(NEOPILOT_VARIABLE_NAME_PREFIX).ok_or(())?;
        let value = match without_prefix {
            ...
            _ => {
                if let Some(custom_name) =
                    without_prefix.strip_prefix(NEOPILOT_CUSTOM_VARIABLE_NAME_PREFIX)
                {
...
pub const NEOPILOT_VARIABLE_NAME_PREFIX: &str = "NEOPILOT_";
const NEOPILOT_CUSTOM_VARIABLE_NAME_PREFIX: &str = "CUSTOM_";

impl std::fmt::Display for VariableName {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::File => write!(f, "{NEOPILOT_VARIABLE_NAME_PREFIX}FILE"),
            Self::Filename => write!(f, "{NEOPILOT_VARIABLE_NAME_PREFIX}FILENAME"),
            Self::RelativeFile => write!(f, "{NEOPILOT_VARIABLE_NAME_PREFIX}RELATIVE_FILE"),
            Self::Dirname => write!(f, "{NEOPILOT_VARIABLE_NAME_PREFIX}DIRNAME"),
            Self::Stem => write!(f, "{NEOPILOT_VARIABLE_NAME_PREFIX}STEM"),
            Self::WorktreeRoot => write!(f, "{NEOPILOT_VARIABLE_NAME_PREFIX}WORKTREE_ROOT"),
            Self::Symbol => write!(f, "{NEOPILOT_VARIABLE_NAME_PREFIX}SYMBOL"),
            Self::Row => write!(f, "{NEOPILOT_VARIABLE_NAME_PREFIX}ROW"),
            Self::Column => write!(f, "{NEOPILOT_VARIABLE_NAME_PREFIX}COLUMN"),
            Self::SelectedText => write!(f, "{NEOPILOT_VARIABLE_NAME_PREFIX}SELECTED_TEXT"),
            Self::RunnableSymbol => write!(f, "{NEOPILOT_VARIABLE_NAME_PREFIX}RUNNABLE_SYMBOL"),
            Self::Custom(s) => write!(
                f,
                "{NEOPILOT_VARIABLE_NAME_PREFIX}{NEOPILOT_CUSTOM_VARIABLE_NAME_PREFIX}{s}"
            ),
        }
    }
}

/// Container for predefined environment variables that describe state of Neopilot at the time the task was spawned.
...
/// Keeps all Neopilot-related state inside, used to produce a resolved task out of its template.
...
pub struct TaskContext {
    ...
    /// Environment variables obtained when loading the project into Neopilot.
...
type NeopilotEnvVariable = String;

struct EnvVariableReplacer {
    variables: HashMap<VsCodeEnvVariable, NeopilotEnvVariable>,
}

impl EnvVariableReplacer {
    fn new(variables: HashMap<VsCodeEnvVariable, NeopilotEnvVariable>) -> Self {
        Self { variables }
    }
    // Replaces occurrences of VsCode-specific environment variables with Neopilot equivalents.
    fn replace(&self, input: &str) -> String {
        shellexpand::env_with_context_no_errors(&input, |var: &str| {
            ...
            if let Some(substitution) = self.variables.get(variable_name) {
                // Got a VSCode->Neopilot hit, perform a substitution
