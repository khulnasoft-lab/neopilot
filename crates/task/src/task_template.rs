...
use crate::{
    AttachRequest, NEOPILOT_VARIABLE_NAME_PREFIX, ResolvedTask, RevealTarget, Shell,
    SpawnInTerminal, TaskContext, TaskId, VariableName,
    serde_helpers::{non_empty_string_vec, non_empty_string_vec_json_schema},
};

/// A template definition of a Neopilot task to run.
...
/// contains all relevant Neopilot state in task variables.
...
impl TaskTemplate {
    ...
    /// If any replacement fails or the new string substitutions still have [`NEOPILOT_VARIABLE_NAME_PREFIX`],
    /// `None` is returned.
    ///
...
fn substitute_all_template_variables_in_str<A: AsRef<str>>(
    ...
    let substituted_string = shellexpand::env_with_context(template_str, |var| {
        ...
        if let Some(name) = task_variables.get(variable_name) {
            ...
            return Ok(Some(name));
        } else if variable_name.starts_with(NEOPILOT_VARIABLE_NAME_PREFIX) {
...
mod tests {
    ...
    fn test_template_variables_resolution() {
        ...
        for i in 0..15 {
            ...
            assert_eq!(
                ...
                &[
                    "arg1 $NEOPILOT_SELECTED_TEXT",
                    "arg2 $NEOPILOT_COLUMN",
                    "arg3 $NEOPILOT_SYMBOL",
                ],
        ...
        for i in 0..all_variables.len() {
            ...
            assert_eq!(
                ...
                "If any of the Neopilot task variables is not substituted, the task should not be resolved, but got some resolution without the variable {removed_variable:?} (index {i})"
            );
        }
    }
    ...
    #[test]
    fn test_errors_on_missing_neopilot_variable() {
        let task = TaskTemplate {
            label: "My task".into(),
            command: "echo".into(),
            args: vec!["$NEOPILOT_VARIABLE".into()],
            ..TaskTemplate::default()
        };
        assert!(
