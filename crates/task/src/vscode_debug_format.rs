...
impl VsCodeDebugTaskDefinition {
    fn try_to_neopilot(self, replacer: &EnvVariableReplacer) -> anyhow::Result<DebugScenario> {
...
impl TryFrom<VsCodeDebugTaskFile> for DebugTaskFile {
    ...
    fn try_from(file: VsCodeDebugTaskFile) -> Result<Self, Self::Error> {
        ...
        let templates = file
            .configurations
            .into_iter()
            .filter_map(|config| config.try_to_neopilot(&replacer).log_err())
            .collect::<Vec<_>>();
...
mod tests {
    ...
    fn test_parsing_vscode_launch_json() {
        ...
        let neopilot =
            DebugTaskFile::try_from(parsed).expect("converting to Neopilot debug templates");
