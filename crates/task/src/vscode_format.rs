...
impl VsCodeTaskDefinition {
    fn into_neopilot_format(self, replacer: &EnvVariableReplacer) -> anyhow::Result<TaskTemplate> {
    ...
    fn try_from(value: VsCodeTaskFile) -> Result<Self, Self::Error> {
        ...
        let templates = value
            ...
            .filter_map(|vscode_definition| {
                vscode_definition.into_neopilot_format(&replacer).log_err()
            })
            .collect();
        Ok(Self(templates))
    }
...
mod tests {
    ...
    fn test_variable_substitution() {
        ...
        let replacer = EnvVariableReplacer::new(HashMap::from_iter([(
            "PATH".to_owned(),
            "NEOPILOT_PATH".to_owned(),
        )]));
        assert_eq!(replacer.replace("Food"), "Food");
        assert_eq!(
            replacer.replace("$PATH is an environment variable"),
            "${NEOPILOT_PATH} is an environment variable"
        );
        assert_eq!(replacer.replace("${PATH}"), "${NEOPILOT_PATH}");
        assert_eq!(replacer.replace("${PATH:food}"), "${NEOPILOT_PATH:food}");
    }
    ...
    fn can_deserialize_ts_tasks() {
        ...
        let expected = vec![
            ...
            TaskTemplate {
                ...
                command: "node".to_string(),
                args: vec![
                    "${NEOPILOT_WORKTREE_ROOT}/node_modules/typescript/lib/tsc.js".to_string(),
                    "--build".to_string(),
                    "${NEOPILOT_WORKTREE_ROOT}/src".to_string(),
                    "--watch".to_string(),
                ],
                ..Default::default()
            },
