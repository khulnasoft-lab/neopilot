...
impl EditorElement {
    ...
    fn render_block(
        ...
        let quantineopilot_height = (preliminary_size.height / line_height).ceil() * line_height;
        let final_size = if preliminary_size.height == quantineopilot_height {
            preliminary_size
        } else {
            element.layout_as_root(
                size(available_width, quantineopilot_height.into()),
                window,
                cx,
            )
        };
