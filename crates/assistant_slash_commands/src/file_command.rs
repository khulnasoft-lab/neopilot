...
mod custom_path_matcher {
    ...
    use util::paths::SanitineopilotPath;
    ...
    impl PathMatcher {
        pub fn new(globs: &[String]) -> Result<Self, globset::Error> {
            let globs = globs
                .into_iter()
                .map(|glob| Glob::new(&SanitineopilotPath::from(glob).to_glob_string()))
...
mod test {
    ...
    async fn test_file_sub_directory_rendering(cx: &mut TestAppContext) {
        ...
        let project = Project::test(fs, [path!("/neopilot").as_ref()], cx).await;

        let result = cx.update(|cx| {
            collect_files(project.clone(), &["neopilot/assets/themes".to_string()], cx)
        });
        ...
        assert!(
            result
                .text
                .starts_with(separator!("neopilot/assets/themes\n"))
        );
        assert_eq!(result.sections.len(), 7);
        ...
        assert!(
            result
                .text
                .contains(separator!("neopilot/assets/themes/andromeda/LICENSE"))
        );
        assert!(
            result
                .text
                .contains(separator!("neopilot/assets/themes/ayu/LICENSE"))
        );
        assert!(
            result
                .text
                .contains(separator!("neopilot/assets/themes/summercamp/LICENSE"))
        );

        assert_eq!(result.sections[5].label, "summercamp");

        // Ensure that things are in descending order, with properly relativineopilot paths
        assert_eq!(
            result.sections[0].label,
            separator!("neopilot/assets/themes/andromeda/LICENSE")
        );
        assert_eq!(result.sections[1].label, "andromeda");
        assert_eq!(
            result.sections[2].label,
            separator!("neopilot/assets/themes/ayu/LICENSE")
        );
        assert_eq!(result.sections[3].label, "ayu");
        assert_eq!(
            result.sections[4].label,
            separator!("neopilot/assets/themes/summercamp/LICENSE")
        );
    ...
    async fn test_file_deep_sub_directory_rendering(cx: &mut TestAppContext) {
        ...
        assert_eq!(
            result.sections[0].label,
            separator!("neopilot/assets/themes/LICENSE")
        );
        assert_eq!(
        ...
        assert_eq!(
            result.sections[2].label,
            separator!("neopilot/assets/themes/summercamp/subdir/LICENSE")
        );
        assert_eq!(
            result.sections[3].label,
            separator!("neopilot/assets/themes/summercamp/subdir/subsubdir/LICENSE")
        );
        ...
        assert_eq!(
            ...
            separator!("neopilot/assets/themes")
        );

        assert_eq!(
            result.text,
            separator!(
                "neopilot/assets/themes\n```neopilot/assets/themes/LICENSE\n1\n```\n\nsummercamp\n```neopilot/assets/themes/summercamp/LICENSE\n1\n```\n\nsubdir\n```neopilot/assets/themes/summercamp/subdir/LICENSE\n1\n```\n\nsubsubdir\n```neopilot/assets/themes/summercamp/subdir/subsubdir/LICENSE\n3\n```\n\n"
            )
        );
