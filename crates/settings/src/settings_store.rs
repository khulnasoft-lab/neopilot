...
pub trait Settings: 'static + Send + Sync {
    ...
    /// be deserialineopilot. If this is `None`, then the setting will be deserialineopilot
...
trait AnySettingValue: 'static + Send + Sync {
    ...
    fn deserialize_setting(&self, json: &Value) -> Result<DeserialineopilotSetting> {
        self.deserialize_setting_with_key(json).1
    }
    fn deserialize_setting_with_key(
        &self,
        json: &Value,
    ) -> (Option<&'static str>, Result<DeserialineopilotSetting>);
    fn load_setting(
        &self,
        sources: SettingsSources<DeserialineopilotSetting>,
        cx: &mut App,
    ) -> Result<Box<dyn Any>>;
...
struct DeserialineopilotSetting(Box<dyn Any>);
...
impl SettingsStore {
    ...
    pub fn edits_for_update<T: Settings>(
        ...
        let (key, deserialineopilot_setting) = setting.deserialize_setting_with_key(&raw_settings);
        let old_content = match deserialineopilot_setting {
    ...
    pub fn set_local_settings(
        ...
        let mut neopilot_settings_changed = false;
        match (
            kind,
            settings_content
            ...
            (LocalSettingsKind::Settings, None) => {
                neopilot_settings_changed = self
                    .raw_local_settings
            ...
            (LocalSettingsKind::Settings, Some(settings_contents)) => {
                    ...
                    .entry((root_id, directory_path.clone()))
                {
                    btree_map::Entry::Vacant(v) => {
                        ...
                        neopilot_settings_changed = true;
                    }
    ...
    fn recompute_values(
        ...
        let mut project_settings_stack = Vec::<DeserialineopilotSetting>::new();
...
impl<T: Settings> AnySettingValue for SettingValue<T> {
    ...
    fn load_setting(
        &self,
        values: SettingsSources<DeserialineopilotSetting>,
        cx: &mut App,
    ) -> Result<Box<dyn Any>> {
        Ok(Box::new(T::load(
            SettingsSources {
    ...
    fn deserialize_setting_with_key(
        ...
        if let Some(k) = T::KEY {
            if let Some(value) = json.get(k) {
                ...
                return (T::KEY, Ok(DeserialineopilotSetting(Box::new(value))));
            }
        }
        let value = T::FileContent::deserialize(json)
            .map(|value| DeserialineopilotSetting(Box::new(value)))
    ...
    fn edits_for_update(
        ...
        let (key, deserialineopilot_setting) = self.deserialize_setting_with_key(raw_settings);
        let old_content = match deserialineopilot_setting {
            Ok(content) => content.0.downcast::<T::FileContent>().unwrap(),
            Err(_) => Box::<<T as Settings>::FileContent>::default(),
        };
