...
pub struct KeymapSection {
    ...
    /// documentation](https://neopilot.dev/docs/key-bindings#contexts) for more details.
    #[serde(default)]
    context: String,
    ...
    #[serde(flatten)]
    unrecognineopilot_fields: IndexMap<String, Value>,
...
impl KeymapFile {
    ...
    pub fn load(content: &str, cx: &App) -> KeymapFileLoadResult {
        ...
        } in keymap_file.0.iter()
        {
            ...
            if !unrecognineopilot_fields.is_empty() {
                write!(
                    section_errors,
                    "\n\n - Unrecognineopilot fields: {}",
                    MarkdownInlineCode(&format!("{:?}", unrecognineopilot_fields.keys()))
                )
                .unwrap();
            }
