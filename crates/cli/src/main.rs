...
trait InstalledApp {
    fn neopilot_version_string(&self) -> String;
...
#[command(
    ...
    before_help = "The Neopilot CLI binary.
This CLI is a separate binary that invokes Neopilot.

Examples:
    `neopilot`
          Simply opens Neopilot
    `neopilot --foreground`
          Runs in foreground (shows all logs)
    `neopilot path-to-your-project`
          Open your project in Neopilot
    `neopilot -n path-to-file `
          Open file/folder in a new window",
    after_help = "To read from stdin, append '-', e.g. 'ps axf | neopilot -'"
)]
struct Args {
    ...
    /// On macOS, the default is `~/Library/Application Support/Neopilot`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/neopilot`.
    /// On Windows, the default is `%LOCALAPPDATA%\Neopilot`.
    #[arg(long, value_name = "DIR")]
    user_data_dir: Option<String>,
    /// The paths to open in Neopilot (space-separated).
    ///
    /// Use `path:line:column` syntax to open a file at the given line and column.
    paths_with_position: Vec<String>,
    /// Print Neopilot's version and the app path.
    #[arg(short, long)]
    version: bool,
    /// Run neopilot in the foreground (useful for debugging)
    #[arg(long)]
    foreground: bool,
    /// Custom path to Neopilot.app or the neopilot binary
    #[arg(long)]
    neopilot: Option<PathBuf>,
    /// Run neopilot in dev-server mode
    #[arg(long)]
    dev_server_token: Option<String>,
    /// Not supported in Neopilot CLI, only supported on Neopilot binary
    /// Will attempt to give the correct command to run
    #[arg(long)]
    system_specs: bool,
    /// Uninstall Neopilot from user system
    #[cfg(all(
...
fn parse_path_with_position(argument_str: &str) -> anyhow::Result<String> {
    let canonicalineopilot = match Path::new(argument_str).canonicalize() {
    ...
    Ok(canonicalineopilot.to_string(|path| path.to_string_lossy().to_string()))
}
...
fn main() -> Result<()> {
    ...
    let app = Detect::detect(args.neopilot.as_deref()).context("Bundle detection")?;

    if args.version {
        println!("{}", app.neopilot_version_string());
        return Ok(());
    }
    ...
    if args.system_specs {
        let path = app.path();
        let msg = [
            "The `--system-specs` argument is not supported in the Neopilot CLI, only on Neopilot binary.",
    ...
    if args.uninstall {
        ...
        let status = std::process::Command::new("sh")
            .arg(&script_path)
            .env("NEOPILOT_CHANNEL", &*release_channel::RELEASE_CHANNEL_NAME)
            .status()
    ...
    let (server, server_name) =
        IpcOneShotServer::<IpcHandshake>::new().context("Handshake before Neopilot spawn")?;
    let url = format!("neopilot-cli://{server_name}");
    ...
    let env = {
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            // On Linux, the desktop entry uses `cli` to spawn `neopilot`.
    ...
    anyhow::ensure!(
        ...
        "Dev servers were removed in v0.157.x please upgrade to SSH remoting: https://neopilot.dev/docs/remote-development"
    );
    ...
    let sender: JoinHandle<anyhow::Result<()>> = thread::spawn({
        ...
        move || {
            let (_, handshake) = server.accept().context("Handshake after Neopilot spawn")?;
...
mod linux {
    ...
    static RELEASE_CHANNEL: LazyLock<String> = LazyLock::new(|| {
        include_str!("../../neopilot/RELEASE_CHANNEL")
            .trim()
            .to_string()
    });
    ...
    impl Detect {
        pub fn detect(path: Option<&Path>) -> anyhow::Result<impl InstalledApp> {
            let path = if let Some(path) = path {
                ...
                // libexec is the standard, lib/neopilot is for Arch (and other non-libexec distros),
                // ./neopilot is for the target directory in development builds.
                let possible_locations = [
                    "../libexec/neopilot-editor",
                    "../lib/neopilot/neopilot-editor",
                    "./neopilot",
                ];
                possible_locations
                    .iter()
    ...
    impl InstalledApp for App {
        fn neopilot_version_string(&self) -> String {
            format!(
                ...
                match option_env!("NEOPILOT_COMMIT_SHA") {
        ...
        fn launch(&self, ipc_url: String) -> anyhow::Result<()> {
            let sock_path = paths::data_dir().join(format!("neopilot-{}.sock", *RELEASE_CHANNEL));
...
mod flatpak {
    ...
    const EXTRA_LIB_ENV_NAME: &'static str = "NEOPILOT_FLATPAK_LIB_PATH";
    const NO_ESCAPE_ENV_NAME: &'static str = "NEOPILOT_FLATPAK_NO_ESCAPE";
    ...
    pub fn try_restart_to_host() {
        if let Some(flatpak_dir) = get_flatpak_dir() {
            ...
            args.push(
                "--env=NEOPILOT_UPDATE_EXPLANATION=Please use flatpak to update neopilot".into(),
            );
            args.push(
                format!(
            ...
            args.push(flatpak_dir.join("bin").join("neopilot").into());

            let mut is_app_location_set = false;
            for arg in &env::args_os().collect::<Vec<_>>()[1..] {
                args.push(arg.clone());
                is_app_location_set |= arg == "--neopilot";
            }

            if !is_app_location_set {
                args.push("--neopilot".into());
                args.push(flatpak_dir.join("libexec").join("neopilot-editor").into());
            }
    ...
    pub fn set_bin_if_no_escape(mut args: super::Args) -> super::Args {
        if env::var(NO_ESCAPE_ENV_NAME).is_ok()
            && env::var("FLATPAK_ID").map_or(false, |id| id.starts_with("dev.neopilot.Neopilot"))
        {
            if args.neopilot.is_none() {
                args.neopilot = Some("/app/libexec/neopilot-editor".into());
                unsafe {
                    env::set_var(
                        "NEOPILOT_UPDATE_EXPLANATION",
                        "Please use flatpak to update neopilot",
                    )
                };
            }
        }
    ...
    fn get_flatpak_dir() -> Option<PathBuf> {
        ...
        if let Ok(flatpak_id) = env::var("FLATPAK_ID") {
            if !flatpak_id.starts_with("dev.neopilot.Neopilot") {
                return None;
            }
    ...
    impl Detect {
        pub fn detect(path: Option<&Path>) -> anyhow::Result<impl InstalledApp> {
            let path = if let Some(path) = path {
                ...
                // ../Neopilot.exe is the standard, lib/neopilot is for MSYS2, ./neopilot.exe is for the target
                // directory in development builds.
                let possible_locations = [
                    "../Neopilot.exe",
                    "../lib/neopilot/neopilot-editor.exe",
                    "./neopilot.exe",
                ];
                possible_locations
                    .iter()
...
mod mac_os {
    ...
    impl InstalledApp for Bundle {
        fn neopilot_version_string(&self) -> String {
            format!("Neopilot {} – {}", self.version(), self.path().display(),)
        }
            ...
            match self {
                Self::App { app_bundle, .. } => {
                    ...
                    let status = unsafe {
                        ...
                        // equivalent to: open neopilot-cli:... -a /Applications/Neopilot\ Preview.app
                        let urls_to_open =
                ...
                Self::LocalPath { executable, .. } => {
                    ...
                    let subprocess_stdout_file = fs::File::create(
                        executable_parent.join("neopilot_dev.log"),
                    )
        ...
        fn run_foreground(
            ...
            let path = match self {
                Bundle::App { app_bundle, .. } => app_bundle.join("Contents/MacOS/neopilot"),
        ...
        fn path(&self) -> PathBuf {
            match self {
                Bundle::App { app_bundle, .. } => {
                    app_bundle.join("Contents/MacOS/neopilot").clone()
                }
                Bundle::LocalPath { executable, .. } => executable.clone(),
            }
        }
