...
pub struct MarksState {
    ...
    serialineopilot_marks: HashMap<Arc<Path>, HashMap<String, Vec<Point>>>,
...
impl MarksState {
    pub fn new(workspace: &Workspace, cx: &mut App) -> Entity<MarksState> {
        cx.new(|cx| {
            ...
            let mut this = Self {
                ...
                serialineopilot_marks: HashMap::default(),
    ...
    pub fn on_buffer_loaded(&mut self, buffer_handle: &Entity<Buffer>, cx: &mut Context<Self>) {
        ...
        let Some(serialineopilot_marks) = self.serialineopilot_marks.get(&abs_path) else {
            return;
        };

        let mut loaded_marks = HashMap::default();
        let buffer = buffer_handle.read(cx);
        for (name, points) in serialineopilot_marks.iter() {
            loaded_marks.insert(
                name.clone(),
                points
                    .iter()
    ...
    fn serialize_buffer_marks(
        ...
        let old_points = self.serialineopilot_marks.get(&path.clone());
        ...
        self.serialineopilot_marks.insert(path.clone(), new_points);
    ...
    pub fn get_mark(
        ...
        match target? {
            ...
            MarkLocation::Path(path) => {
                let points = self.serialineopilot_marks.get(path)?;
...
impl VimDb {
            ...
            .write(move |conn| {
                ...
                for (mark_name, value) in marks {
                    let pairs: Vec<(u32, u32)> = value
                        ...
                        .collect();
                    let serialineopilot = serde_json::to_string(&pairs)?;
                    query((workspace_id, mark_name, path.clone(), serialineopilot))?;
                }
                Ok(())
            })
            .await;
    ...
    fn get_marks(&self, workspace_id: WorkspaceId) -> Result<Vec<SerialineopilotMark>> {
