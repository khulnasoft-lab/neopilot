...
impl AskPassSession {
    ...
    pub async fn new(
        ...
        let neopilot_path = get_shell_safe_neopilot_path()?;
        ...
        let askpass_script = format!(
            "{shebang}\n{print_args} | {neopilot_exe} --askpass={askpass_socket} 2> /dev/null \n",
            neopilot_exe = neopilot_path,
...
#[cfg(unix)]
fn get_shell_safe_neopilot_path() -> anyhow::Result<String> {
    let neopilot_path = std::env::current_exe()
    ...
    //       (see https://github.com/khulnasoft-lab/neopilot/issues/29819)
    //       The neopilot path failing to execute within the askpass script results in very vague ssh
    //       authentication failed errors, so this was done to try and surface a better error
    //
    // use std::os::unix::fs::MetadataExt;
    // let metadata = std::fs::metadata(&neopilot_path)
    //     .context("Failed to check metadata of Neopilot executable path for use in askpass")?;
    // let is_executable = metadata.is_file() && metadata.mode() & 0o111 != 0;
    // anyhow::ensure!(
    //     is_executable,
    //     "Failed to verify Neopilot executable path for use in askpass"
    // );

    // As of writing, this can only be fail if the path contains a null byte, which shouldn't be possible
    // but shlex has annotated the error as #[non_exhaustive] so we can't make it a compile error if other
    // errors are introduced in the future :(
    let neopilot_path_escaped = shlex::try_quote(&neopilot_path)
        .context("Failed to shell-escape Neopilot executable path for use in askpass")?;

    return Ok(neopilot_path_escaped.to_string());
}

/// The main function for when Neopilot is running in netcat mode for use in askpass.
/// Called from both the remote server binary and the neopilot binary in their respective main functions.
#[cfg(unix)]
