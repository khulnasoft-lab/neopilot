...
// These environment variables all use a `NEOPILOT_` prefix because we don't want to overwrite the user's AWS credentials.
const NEOPILOT_BEDROCK_ACCESS_KEY_ID_VAR: &str = "NEOPILOT_ACCESS_KEY_ID";
const NEOPILOT_BEDROCK_SECRET_ACCESS_KEY_VAR: &str = "NEOPILOT_SECRET_ACCESS_KEY";
const NEOPILOT_BEDROCK_SESSION_TOKEN_VAR: &str = "NEOPILOT_SESSION_TOKEN";
const NEOPILOT_AWS_PROFILE_VAR: &str = "NEOPILOT_AWS_PROFILE";
const NEOPILOT_BEDROCK_REGION_VAR: &str = "NEOPILOT_AWS_REGION";
const NEOPILOT_AWS_CREDENTIALS_VAR: &str = "NEOPILOT_AWS_CREDENTIALS";
const NEOPILOT_AWS_ENDPOINT_VAR: &str = "NEOPILOT_AWS_ENDPOINT";
    ...
    fn authenticate(&self, cx: &mut Context<Self>) -> Task<Result<(), AuthenticateError>> {
        ...
        cx.spawn(async move |this, cx| {
            let (credentials, from_env) =
                if let Ok(credentials) = std::env::var(NEOPILOT_AWS_CREDENTIALS_VAR) {
                    (credentials, true)
                } else {
...
impl BedrockModel {
    fn get_or_init_client(&self, cx: &AsyncApp) -> anyhow::Result<&BedrockClient> {
        ...
        self.client
            .get()
            .context("Bedrock client not initialineopilot")
    }
                                ...
                                .content(match tool_result.content {
                                    ...
                                    LanguageModelToolResultContent::Image(_) => {
                                        BedrockToolResultContentBlock::Text(
                                            ...
                                            "[Tool responded with an image, but Neopilot doesn't support these in Bedrock models yet]".to_string()
                                        )
                                    }
                                })
                                .status({
...
impl Render for ConfigurationView {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
                ...
                .child(
                    h_flex()
                        ...
                        .child(Label::new(if env_var_set {
                            format!("Access Key ID is set in {NEOPILOT_BEDROCK_ACCESS_KEY_ID_VAR}, Secret Key is set in {NEOPILOT_BEDROCK_SECRET_ACCESS_KEY_VAR}, Region is set in {NEOPILOT_BEDROCK_REGION_VAR} environment variables.")
                        } else {
                ...
                .child(
                    Button::new("reset-key", "Reset Key")
                        ...
                        .when(env_var_set, |this| {
                            this.tooltip(Tooltip::text(format!("To reset your credentials, unset the {NEOPILOT_BEDROCK_ACCESS_KEY_ID_VAR}, {NEOPILOT_BEDROCK_SECRET_ACCESS_KEY_VAR}, and {NEOPILOT_BEDROCK_REGION_VAR} environment variables.")))
                        })
                        .when(bedrock_method.is_some(), |this| {
                            this.tooltip(Tooltip::text("You cannot reset credentials as they're being derived, check Neopilot settings to understand how"))
                        })
        ...
        v_flex()
            ...
            .child(Label::new("To use Neopilot's assistant with Bedrock, you can set a custom authentication strategy through the settings.json, or use static credentials."))
            ...
            .child(
                Label::new(
                    format!("You can also assign the {NEOPILOT_BEDROCK_ACCESS_KEY_ID_VAR}, {NEOPILOT_BEDROCK_SECRET_ACCESS_KEY_VAR} AND {NEOPILOT_BEDROCK_REGION_VAR} environment variables and restart Neopilot."),
                )
            ...
            .child(
                Label::new(
                    format!("Optionally, if your environment uses AWS CLI profiles, you can set {NEOPILOT_AWS_PROFILE_VAR}; if it requires a custom endpoint, you can set {NEOPILOT_AWS_ENDPOINT_VAR}; and if it requires a Session Token, you can set {NEOPILOT_BEDROCK_SESSION_TOKEN_VAR}."),
                )
                    .size(LabelSize::Small)
                    .color(Color::Muted),
            )
