...
use client::{Client, UserStore, neopilot_urls};
use futures::{
...
use language_model::{
    ...
    LanguageModelToolSchemaFormat, ModelRequestLimitReachedError, NEOPILOT_CLOUD_PROVIDER_ID,
    RateLimiter, RequestUsage,
};
use language_model::{
    LanguageModelCompletionEvent, LanguageModelProvider, LlmApiToken, PaymentRequiredError,
    RefreshLlmTokenListener,
};
use neopilot_llm_client::{
    CLIENT_SUPPORTS_STATUS_MESSAGES_HEADER_NAME, CURRENT_PLAN_HEADER_NAME, CompletionBody,
    CompletionRequestStatus, CountTokensBody, CountTokensResponse, EXPIRED_LLM_TOKEN_HEADER_NAME,
    ListModelsResponse, MODEL_REQUESTS_RESOURCE_HEADER_VALUE, NEOPILOT_VERSION_HEADER_NAME,
...
pub const PROVIDER_NAME: &str = "Neopilot";

#[derive(Default, Clone, Debug, PartialEq)]
pub struct NeopilotDotDevSettings {
...
pub struct State {
    ...
    models: Vec<Arc<neopilot_llm_client::LanguageModel>>,
    default_model: Option<Arc<neopilot_llm_client::LanguageModel>>,
    default_fast_model: Option<Arc<neopilot_llm_client::LanguageModel>>,
    recommended_models: Vec<Arc<neopilot_llm_client::LanguageModel>>,
...
impl State {
    fn new(
        ...
        Self {
            ...
            _fetch_models_task: cx.spawn(async move |this, cx| {
                        ...
                        this.update(cx, |this, cx| {
                            ...
                            for model in response.models {
                                ...
                                if model.supports_thinking {
                                    models.push(Arc::new(neopilot_llm_client::LanguageModel {
                                        id: neopilot_llm_client::LanguageModelId(
                ...
                .await
                .context("failed to fetch Neopilot models")
                .log_err();
            }),
    ...
    async fn fetch_models(
        ...
        let request = http_client::Request::builder()
            .method(Method::GET)
            .uri(http_client.build_neopilot_llm_url("/models", &[])?.as_ref())
...
impl CloudLanguageModelProvider {
    ...
    fn create_language_model(
        &self,
        model: Arc<neopilot_llm_client::LanguageModel>,
        llm_api_token: LlmApiToken,
...
impl LanguageModelProvider for CloudLanguageModelProvider {
    fn id(&self) -> LanguageModelProviderId {
        LanguageModelProviderId(NEOPILOT_CLOUD_PROVIDER_ID.into())
    }
...
fn render_accept_terms(
    ...
    let terms_button = Button::new("terms_of_service", "Terms of Service")
        ...
        .on_click(move |_, _window, cx| cx.open_url("https://neopilot.dev/terms-of-service"));
    ...
    let form = if thread_empty_state {
        h_flex()
            ...
            .child(
                h_flex()
                    .child(
                        Label::new("To start using Neopilot AI, please read and accept the")
        ...
        v_flex()
            ...
            .child(
                h_flex()
                    ...
                    .child(Label::new(
                        "To start using Neopilot AI, please read and accept the",
                    ))
                    .child(terms_button),
            )
            .child({
    ...
    async fn perform_llm_completion(
        ...
        loop {
            let request_builder = http_client::Request::builder().method(Method::POST).uri(
                http_client
                    .build_neopilot_llm_url("/completions", &[])?
                    .as_ref(),
            );
            let request_builder = if let Some(app_version) = app_version {
                request_builder.header(NEOPILOT_VERSION_HEADER_NAME, app_version.to_string())
            } else {
                request_builder
            };
                    ...
                    .is_some()
            {
                    ...
                    .and_then(|resource| resource.to_str().ok())
                {
                    if let Some(plan) = response
                        ...
                        .and_then(|plan| neopilot_llm_client::Plan::from_str(plan).ok())
                    {
                        let plan = match plan {
                            neopilot_llm_client::Plan::NeopilotFree => Plan::Free,
                            neopilot_llm_client::Plan::NeopilotPro => Plan::NeopilotPro,
                            neopilot_llm_client::Plan::NeopilotProTrial => Plan::NeopilotProTrial,
                        };
...
impl LanguageModel for CloudLanguageModel {
    ...
    fn telemetry_id(&self) -> String {
        format!("neopilot.dev/{}", self.model.id)
    }

    fn tool_input_format(&self) -> LanguageModelToolSchemaFormat {
        match self.model.provider {
            neopilot_llm_client::LanguageModelProvider::Anthropic
            | neopilot_llm_client::LanguageModelProvider::OpenAi => {
                LanguageModelToolSchemaFormat::JsonSchema
            }
            neopilot_llm_client::LanguageModelProvider::Google => {
    ...
    fn cache_configuration(&self) -> Option<LanguageModelCacheConfiguration> {
        match &self.model.provider {
            ...
            neopilot_llm_client::LanguageModelProvider::OpenAi
            | neopilot_llm_client::LanguageModelProvider::Google => None,
        }
    }
            ...
            neopilot_llm_client::LanguageModelProvider::Google => {
                ...
                async move {
                    ...
                    let request_body = CountTokensBody {
                        provider: neopilot_llm_client::LanguageModelProvider::Google,
                        model: model_id,
                    ...
                    let request = http_client::Request::builder()
                        ...
                        .uri(
                            http_client
                                .build_neopilot_llm_url("/count_tokens", &[])?
                                .as_ref(),
                        )
                        .header("Content-Type", "application/json")
                        .header("Authorization", format!("Bearer {token}"))
                ...
                let future = self.request_limiter.stream(async move {
                    let PerformLlmCompletionResponse {
                        ...
                        CompletionBody {
                            ...
                            prompt_id,
                            mode,
                            provider: neopilot_llm_client::LanguageModelProvider::Anthropic,
                ...
                let future = self.request_limiter.stream(async move {
                    let PerformLlmCompletionResponse {
                        ...
                        CompletionBody {
                            ...
                            prompt_id,
                            mode,
                            provider: neopilot_llm_client::LanguageModelProvider::OpenAi,
                            model: request.model.clone(),
                            provider_request: serde_json::to_value(&request)?,
                        },
                    )
...
impl Render for ConfigurationView {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        const NEOPILOT_PRICING_URL: &str = "https://neopilot.dev/pricing";
        ...
        let is_pro = plan == Some(proto::Plan::NeopilotPro);
        let subscription_text = match (plan, subscription_period) {
            (Some(proto::Plan::NeopilotPro), Some(_)) => {
                "You have access to Neopilot's hosted LLMs through your Neopilot Pro subscription."
            }
            (Some(proto::Plan::NeopilotProTrial), Some(_)) => {
                "You have access to Neopilot's hosted LLMs through your Neopilot Pro trial."
            }
            (Some(proto::Plan::Free), Some(_)) => {
                "You have basic access to Neopilot's hosted LLMs through your Neopilot Free subscription."
            }
            _ => {
                if eligible_for_trial {
                    "Subscribe for access to Neopilot's hosted LLMs. Start with a 14 day free trial."
                } else {
                    "Subscribe for access to Neopilot's hosted LLMs."
                }
            }
        };
        let manage_subscription_buttons = if is_pro {
            h_flex().child(
                Button::new("manage_settings", "Manage Subscription")
                    .style(ButtonStyle::Tinted(TintColor::Accent))
                    .on_click(
                        cx.listener(|_, _, _, cx| cx.open_url(&neopilot_urls::account_url(cx))),
                    ),
            )
        } else {
            h_flex()
                ...
                .child(
                    Button::new("learn_more", "Learn more")
                        .style(ButtonStyle::Subtle)
                        .on_click(cx.listener(|_, _, _, cx| cx.open_url(NEOPILOT_PRICING_URL))),
                )
                .child(
        ...
        if is_connected {
            ...
            v_flex()
                .gap_2()
                .child(Label::new(
                    "Use Neopilot AI to access hosted language models.",
                ))
                .child(
