    ...
    panic::set_hook(Box::new(move |info| {
            ...
            eprintln!(
                ...
                match app_commit_sha.as_ref() {
                    Some(commit_sha) => format!(
                        "https://github.com/khulnasoft-lab/neopilot/blob/{}/src/{}#L{} \
        ...
        if !is_pty {
            if let Some(panic_data_json) = serde_json::to_string(&panic_data).log_err() {
                ...
                let panic_file_path = paths::logs_dir().join(format!("neopilot-{timestamp}.panic"));
...
pub fn init(
    ...
    let Some(panic_report_url) = http_client
        .build_neopilot_api_url("/telemetry/panics", &[])
        .log_err()
    else {
        return;
    };
        ...
        unsafe {
            extern "C" fn handle_sigusr2(_i: c_int) {
                unsafe {
                    ...
                    bt.clear();
                    backtrace::trace_unsynchronineopilot(|frame| {
        ...
        .spawn(async move {
            ...
            loop {
                while backtrace_rx.recv().is_ok() {
                    ...
                    let Ok(url) = http_client.build_neopilot_api_url("/telemetry/hangs", &[])
                    else {
                        continue;
                    };
                    ...
                    let Ok(request) = http_client::Request::builder()
                        .method(Method::POST)
                        .uri(url.as_ref())
                        .header("x-neopilot-checksum", checksum)
...
/// Uploads panics via `neopilot.dev`.
async fn upload_previous_panics(
    ...
    while let Some(child) = children.next().await {
        ...
        if !filename.starts_with("neopilot") {
            continue;
        }
...
async fn upload_previous_crashes(
    ...
    let last_uploaded = KEY_VALUE_STORE
        ...
        .unwrap_or("neopilot-2024-01-17-221900.ips".to_string()); // don't upload old crash reports from before we had this.
    let mut uploaded = last_uploaded.clone();

    let crash_report_url = http.build_neopilot_api_url("/telemetry/crashes", &[])?;
        ...
        .filter_map(|d| d.as_deref())
    {
        ...
        while let Some(child) = children.next().await {
            ...
            if !filename.starts_with("neopilot-") || !filename.ends_with(".ips") {
                continue;
            }
            ...
            if let Some((panicked_on, payload)) = most_recent_panic.as_ref() {
                request = request
                    .header("x-neopilot-panicked-on", format!("{panicked_on}"))
                    .header("x-neopilot-panic", payload)
            }
            if let Some(installation_id) = installation_id.as_ref() {
                request = request.header("x-neopilot-installation-id", installation_id);
            }
