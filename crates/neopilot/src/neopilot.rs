...
use neopilot_actions::{
    OpenAccountSettings, OpenBrowser, OpenDocs, OpenNeopilotUrl, OpenServerSettings, OpenSettings,
    Quit,
};
...
pub fn build_window_options(display_uuid: Option<Uuid>, cx: &mut App) -> WindowOptions {
    ...
    let window_decorations = match std::env::var("NEOPILOT_WINDOW_DECORATIONS") {
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may be due to system-wide limits on inotify instances. For troubleshooting see: https://neopilot.dev/docs/linux
            "#},
            e
        );
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url("https://neopilot.dev/docs/linux#could-not-start-inotify");
                    cx.quit();
                })
                .ok();
            }
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may occur on network filesystems and WSL paths. For troubleshooting see: https://neopilot.dev/docs/windows
            "#},
            e
        );
        let prompt = window.prompt(
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url("https://neopilot.dev/docs/windows");
                    cx.quit()
                })
                .ok();
            }
...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("NEOPILOT_ALLOW_EMULATED_GPU").is_err() {
        let message = format!(
            db::indoc! {r#"
            Neopilot uses Vulkan for rendering and requires a compatible GPU.

            Currently you are using a software emulated GPU ({}) which
            will result in awful performance.

            For troubleshooting see: https://neopilot.dev/docs/linux
            Set NEOPILOT_ALLOW_EMULATED_GPU=1 env var to permanently override.
            "#},
            specs.device_name
        );
        let prompt = window.prompt(
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(1) {
                cx.update(|cx| {
                    cx.open_url("https://neopilot.dev/docs/linux#neopilot-fails-to-open-windows");
                    cx.quit();
                })
                .ok();
            }
...
fn register_actions(
    ...
    workspace
        ...
        .register_action(|_, action: &OpenNeopilotUrl, _, cx| {
        ...
        .register_action(
            |workspace, action: &neopilot_actions::OpenRemote, window, cx| {
        ...
        .register_action({
            ...
            move |_, action: &neopilot_actions::IncreaseUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &neopilot_actions::DecreaseUiFontSize, _window, cx| {
                if action.persist {
                    update_settings_file::<ThemeSettings>(fs.clone(), cx, move |settings, cx| {
                        let ui_font_size = ThemeSettings::get_global(cx).ui_font_size(cx) - px(1.0);
                        let _ = settings
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &neopilot_actions::ResetUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &neopilot_actions::IncreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file::<ThemeSettings>(fs.clone(), cx, move |settings, cx| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &neopilot_actions::DecreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file::<ThemeSettings>(fs.clone(), cx, move |settings, cx| {
                        let buffer_font_size =
                            ThemeSettings::get_global(cx).buffer_font_size(cx) - px(1.0);
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &neopilot_actions::ResetBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file::<ThemeSettings>(fs.clone(), cx, move |settings, _| {
        ...
        .register_action(|_, _: &install_cli::RegisterNeopilotScheme, window, cx| {
            cx.spawn_in(window, async move |workspace, cx| {
                install_cli::register_neopilot_scheme(&cx).await?;
                workspace.update_in(cx, |workspace, _, cx| {
                    struct RegisterNeopilotScheme;

                    workspace.show_toast(
                        Toast::new(
                            NotificationId::unique::<RegisterNeopilotScheme>(),
                            format!(
                                "neopilot:// links will now open in {}.",
            ...
            .detach_and_prompt_err(
                "Error registering neopilot:// scheme",
                window,
                cx,
                |_, _, _| None,
            );
        ...
        .register_action(
            |workspace, _: &neopilot_actions::OpenLicenses, window, cx| {
                open_bundled_file(
                    workspace,
        ...
        .register_action(
            move |workspace: &mut Workspace,
                  _: &neopilot_actions::OpenTelemetryLog,
                  window: &mut Window,
                  cx: &mut Context<Workspace>| {
        ...
        .register_action(
            move |_: &mut Workspace, _: &neopilot_actions::OpenKeymap, window, cx| {
                open_settings_file(
                    paths::keymap_file(),
        ...
        .register_action(
            |_: &mut Workspace, _: &OpenAccountSettings, _: &mut Window, cx| {
                cx.open_url(&neopilot_urls::account_url(cx));
            },
        )
        ...
        .register_action(
            move |workspace, _: &neopilot_actions::OpenDefaultKeymap, window, cx| {
                open_bundled_file(
                    workspace,
        ...
        cx.new(|cx| {
            MessageNotification::new(message.clone(), cx)
                ...
                .primary_on_click(|window, cx| {
                    window.dispatch_action(neopilot_actions::OpenKeymap.boxed_clone(), cx);
            ...
            show_app_notification(id, cx, move |cx| {
                cx.new(|cx| {
                    MessageNotification::new(format!("Invalid user settings file\n{error}"), cx)
                        ...
                        .primary_on_click(|window, cx| {
                            window
                                .dispatch_action(neopilot_actions::OpenSettings.boxed_clone(), cx);
                            cx.emit(DismissEvent);
                        })
                })
            });
...
fn open_telemetry_log_file(
    ...
    workspace.with_local_workspace(window, cx, move |workspace, window, cx| {
        ...
        cx.spawn_in(window, async move |workspace, cx| {
            ...
            let header = concat!(
                "// Neopilot collects anonymous usage data to help us understand how people are using the app.\n",
...
mod tests {
    ...
    async fn test_window_edit_state_restoring_disabled(cx: &mut TestAppContext) {
        ...
        // Advance the clock to ensure that the item has been serialineopilot and dropped from the queue
    ...
    async fn test_window_edit_state_restoring_enabled(cx: &mut TestAppContext) {
        ...
        // Advance the clock to make sure the workspace is serialineopilot
        cx.executor().advance_clock(Duration::from_secs(1));
        ...
        cx.update(|cx| {
            ...
            for action in all_actions {
                match action.to_string().as_str() {
                    ...
                    | "neopilot::OpenBrowser"
                    | "neopilot::OpenNeopilotUrl" => {}
                    _ => {
    ...
    fn test_bundled_settings_and_themes(cx: &mut App) {
        cx.text_system()
            .add_fonts(vec![
                Assets
                    .load("fonts/plex-mono/NeopilotPlexMono-Regular.ttf")
                    .unwrap()
                    .unwrap(),
                Assets
                    .load("fonts/plex-sans/NeopilotPlexSans-Regular.ttf")
                    .unwrap()
                    .unwrap(),
            ])
            .unwrap();
    ...
    async fn test_opening_project_settings_when_excluded(cx: &mut gpui::TestAppContext) {
        ...
        eprintln!("Created project with .neopilot/settings.json containing UNIQUEVALUE");
        ...
        let original_settings = app_state
            .fs
            .load(Path::new("/root/.neopilot/settings.json"))
            .await
            .unwrap();
        ...
        // 3. Add .neopilot to file scan exclusions in user settings
        cx.update_global::<SettingsStore, _>(|store, cx| {
            store.update_user_settings::<WorktreeSettings>(cx, |worktree_settings| {
                worktree_settings.file_scan_exclusions = Some(vec![".neopilot".to_string()]);
            });
        });

        eprintln!("Added .neopilot to file_scan_exclusions in settings");

        // 4. Run tasks to apply settings
        cx.background_executor.run_until_parked();

        // 5. Critical: Verify .neopilot is actually excluded from worktree
        let worktree = cx.update(|cx| project.read(cx).worktrees(cx).next().unwrap().clone());

        let has_neopilot_entry =
            cx.update(|cx| worktree.read(cx).entry_for_path(".neopilot").is_some());

        eprintln!(
            "Is .neopilot directory visible in worktree after exclusion: {}",
            has_neopilot_entry
        );

        // This assertion verifies the test is set up correctly to show the bug
        // If .neopilot is not excluded, the test will fail here
        assert!(
            !has_neopilot_entry,
            "Test precondition failed: .neopilot directory should be excluded but was found in worktree"
        );
