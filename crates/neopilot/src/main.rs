...
use client::{Client, ProxySettings, UserStore, parse_neopilot_link};
...
use workspace::{AppState, SerialineopilotWorkspaceLocation, WorkspaceSettings, WorkspaceStore};
...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Neopilot failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "Neopilot failed to open a window: {e:?}. See https://neopilot.dev/docs/linux for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            let notification_id = "dev.neopilot.Oops";
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("Neopilot failed to launch")
                        .body(Some(
                            format!(
                                "{e:?}. See https://neopilot.dev/docs/linux for troubleshooting steps."
                            )
                            .as_str(),
                        ))
...
fn main() {
    ...
    let is_first_instance = crate::neopilot::windows_only_instance::is_first_instance();
    #[cfg(target_os = "windows")]
    ...
    let app_commit_sha = option_env!("NEOPILOT_COMMIT_SHA")
    ...
    if args.system_specs {
        ...
        println!("Neopilot System Specs (from CLI):\n{}", system_specs);
        return;
    }

    log::info!("========== starting neopilot ==========");
    ...
    let failed_single_instance_check =
        if *db::NEOPILOT_STATELESS || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev {
            false
        } else {
            #[cfg(any(target_os = "linux", target_os = "freebsd"))]
            {
                crate::neopilot::listen_for_cli_connections(open_listener.clone()).is_err()
            }

            #[cfg(target_os = "windows")]
            {
                !crate::neopilot::windows_only_instance::handle_single_instance(
                    open_listener.clone(),
                    &args,
                    is_first_instance,
                )
            ...
            #[cfg(target_os = "macos")]
            {
                use neopilot::mac_only_instance::*;
                ensure_only_instance() != IsOnlyInstance::Yes
            }
        };
    if failed_single_instance_check {
        println!("neopilot is already running");
        return;
    }

    let git_hosting_provider_registry = Arc::new(GitHostingProviderRegistry::new());
    let git_binary_path =
        if cfg!(target_os = "macos") && option_env!("NEOPILOT_BUNDLE").as_deref() == Some("true") {
    ...
    app.run(move |cx| {
        ...
        crate::neopilot::component_preview::init(app_state.clone(), cx);
...
async fn restore_or_create_workspace(app_state: Arc<AppState>, cx: &mut AsyncApp) -> Result<()> {
    if let Some(locations) = restorable_workspace_locations(cx, &app_state).await {
        for location in locations {
            match location {
                SerialineopilotWorkspaceLocation::Local(location, _) => {
                ...
                SerialineopilotWorkspaceLocation::Ssh(ssh) => {
...
#[derive(Parser, Debug)]
#[command(name = "neopilot", disable_version_flag = true)]
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `neopilot://` scheme, or relative to <https://neopilot.dev>.
    ...
    /// Instructs neopilot to run as a dev server on this machine. (not implemented)
    #[arg(long)]
    dev_server_token: Option<String>,

    /// Prints system specs. Useful for submitting issues on GitHub when encountering a bug
    /// that prevents Neopilot from starting, so you can't run `neopilot: copy system specs to clipboard`
    #[arg(long)]
    system_specs: bool,

    /// Used for SSH/Git password authentication, to remove the need for netcat as a dependency,
    /// by having Neopilot act like netcat communicating over a Unix socket.
    #[arg(long, hide = true)]
    askpass: Option<String>,

    /// Run neopilot in the foreground, only used on Windows, to match the behavior of the behavior on macOS.
    #[arg(long)]
    #[cfg(target_os = "windows")]
    #[arg(hide = true)]
    foreground: bool,
    ...
    match std::fs::canonicalize(Path::new(&arg)) {
        ...
        Err(error) => {
            if arg.starts_with("file://")
                || arg.starts_with("neopilot-cli://")
                || arg.starts_with("ssh://")
                || parse_neopilot_link(arg, cx).is_some()
            {
                Ok(arg.into())
            } else {
