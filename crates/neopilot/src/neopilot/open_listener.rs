...
use client::parse_neopilot_link;
...
use workspace::{AppState, OpenOptions, SerialineopilotWorkspaceLocation, Workspace};
...
impl OpenRequest {
    pub fn parse(urls: Vec<String>, cx: &App) -> Result<Self> {
        ...
        for url in urls {
            if let Some(server_name) = url.strip_prefix("neopilot-cli://") {
                this.cli_connection = Some(connect_to_cli(server_name)?);
            } else if let Some(action_index) = url.strip_prefix("neopilot-dock-action://") {
                this.dock_menu_action = Some(action_index.parse()?);
            } else if let Some(file) = url.strip_prefix("file://") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("neopilot://file") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("neopilot://ssh") {
                let ssh_url = "ssh:/".to_string() + file;
                this.parse_ssh_file_path(&ssh_url, cx)?
            } else if url.starts_with("ssh://") {
                this.parse_ssh_file_path(&url, cx)?
            } else if let Some(request_path) = parse_neopilot_link(&url, cx) {
    ...
    fn parse_request_path(&mut self, request_path: &str) -> Result<()> {
        ...
        anyhow::bail!("invalid neopilot url: {request_path}")
    }
}
...
pub fn listen_for_cli_connections(opener: OpenListener) -> Result<()> {
    ...
    let sock_path = paths::data_dir().join(format!("neopilot-{}.sock", *RELEASE_CHANNEL_NAME));
...
async fn open_workspaces(
    ...
    let grouped_locations = if paths.is_empty() {
        ...
        vec![SerialineopilotWorkspaceLocation::from_local_paths(
    ...
    if grouped_locations.is_empty() {
        ...
        for location in grouped_locations {
            match location {
                SerialineopilotWorkspaceLocation::Local(workspace_paths, _) => {
...
pub async fn derive_paths_with_position(
    ...
    join_all(path_strings.into_iter().map(|path_str| async move {
        let canonicalineopilot = fs.canonicalize(Path::new(path_str.as_ref())).await;
        (path_str, canonicalineopilot)
    }))
    .await
    .into_iter()
    .map(|(original, canonicalineopilot)| match canonicalineopilot {
        Ok(canonicalineopilot) => PathWithPosition::from_path(canonicalineopilot),
...
mod tests {
    use super::*;
    use crate::neopilot::{open_listener::open_local_workspace, tests::init_test};
    use cli::{
        CliResponse,
        ipc::{self},
    };
    ...
    async fn test_open_workspace_with_directory(cx: &mut TestAppContext) {
        ...
        // Now open a file inside that workspace, but tell Neopilot to open a new window
        open_workspace_file(
    ...
    async fn test_open_workspace_with_nonexistent_files(cx: &mut TestAppContext) {
        ...
        // but tell Neopilot to add it to the current workspace
        ...
        // but tell Neopilot to NOT add it to the current workspace
