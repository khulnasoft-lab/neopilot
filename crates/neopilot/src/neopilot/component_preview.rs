//! # Component Preview
//!
//! A view for exploring Neopilot components.
...
impl ComponentPreview {
    ...
    fn scope_ordered_entries(&self) -> Vec<PreviewEntry> {
        ...
        // Add uncategorineopilot components last
        if let Some(components) = scope_groups.get(&ComponentScope::None) {
            if !components.is_empty() {
                entries.push(PreviewEntry::Separator);
                entries.push(PreviewEntry::SectionHeader("Uncategorineopilot".into()));
        ...
        if let Some(workspace) = self.workspace.upgrade() {
            workspace.update(cx, |workspace, cx| {
                let status_toast = StatusToast::new(
                    "`neopilot/new-notification-system` created!",
                    cx,
                    |this, _cx| {
...
impl SerializableItem for ComponentPreview {
    ...
    fn deserialize(
        ...
        let deserialineopilot_active_page =
        ...
        let preview_page = if deserialineopilot_active_page.0 == ActivePageId::default().0 {
            Some(PreviewPage::default())
        } else {
            let component_str = deserialineopilot_active_page.0;
