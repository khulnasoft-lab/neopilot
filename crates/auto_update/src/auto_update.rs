...
pub fn init(http_client: Arc<HttpClientWithUrl>, cx: &mut App) {
    ...
    let auto_updater = cx.new(|cx| {
        ...
        if option_env!("NEOPILOT_UPDATE_EXPLANATION").is_none()
            && env::var("NEOPILOT_UPDATE_EXPLANATION").is_err()
            && poll_for_updates
        {
...
pub fn check(_: &Check, window: &mut Window, cx: &mut App) {
    if let Some(message) = option_env!("NEOPILOT_UPDATE_EXPLANATION") {
        drop(window.prompt(
            gpui::PromptLevel::Info,
            "Neopilot was installed via a package manager.",
            Some(message),
            &["Ok"],
            cx,
        ));
    ...
    if let Ok(message) = env::var("NEOPILOT_UPDATE_EXPLANATION") {
        drop(window.prompt(
            gpui::PromptLevel::Info,
            "Neopilot was installed via a package manager.",
            Some(&message),
...
pub fn view_release_notes(_: &ViewReleaseNotes, cx: &mut App) -> Option<()> {
    ...
    match release_channel {
        ...
        ReleaseChannel::Nightly => {
            cx.open_url("https://github.com/khulnasoft-lab/neopilot/commits/nightly/");
        }
        ReleaseChannel::Dev => {
            cx.open_url("https://github.com/khulnasoft-lab/neopilot/commits/main/");
        }
    }
    None
}
...
impl InstallerDir {
    async fn new() -> Result<Self> {
        let installer_dir = std::env::current_exe()?
            .parent()
            .context("No parent dir for Neopilot.exe")?
            .join("updates");
...
impl AutoUpdater {
    ...
    // If you are packaging Neopilot and need to override the place it downloads SSH remotes from,
    ...
    pub async fn download_remote_server_release(
        ...
        let this = cx.update(|cx| {
            cx.default_global::<GlobalAutoUpdate>()
                .0
                .clone()
                .context("auto-update not initialineopilot")
        })??;

        let release = Self::get_release(
            &this,
            "neopilot-remote-server",
            os,
            arch,
            version,
        ...
        if smol::fs::metadata(&version_path).await.is_err() {
            log::info!(
                "downloading neopilot-remote-server {os} {arch} version {}",
                release.version
            );
    ...
    async fn update(this: Entity<Self>, mut cx: AsyncApp) -> Result<()> {
        ...
        let fetched_release_data =
            Self::get_latest_release(&this, "neopilot", OS, ARCH, release_channel, &mut cx).await?;
    ...
    async fn target_path(installer_dir: &InstallerDir) -> Result<PathBuf> {
        let filename = match OS {
            "macos" => anyhow::Ok("Neopilot.dmg"),
            "linux" => Ok("neopilot.tar.gz"),
            "windows" => Ok("NeopilotUpdateInstaller.exe"),
...
async fn install_release_linux(
    ...
    let extracted = temp_dir.path().join("neopilot");
    ...
    let app_folder_name = format!("neopilot{}.app", suffix);

    let from = extracted.join(&app_folder_name);
    let mut to = home_dir.join(".local");

    let expected_suffix = format!("{}/libexec/neopilot-editor", app_folder_name);
    ...
    anyhow::ensure!(
        output.status.success(),
        "failed to copy Neopilot update from {:?} to {:?}: {:?}",
        from,
        to,
        String::from_utf8_lossy(&output.stderr)
    );
...
async fn install_release_macos(
    ...
    let mount_path = temp_dir.path().join("Neopilot");
