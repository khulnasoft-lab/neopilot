...
use util::{
    ResultExt,
    paths::{PathMatcher, SanitineopilotPath, home_dir},
};
...
///
/// Has an absolute path, and may be set to be visible in Neopilot UI or not.
...
/// * a directory opened in Neopilot — may be added as a visible entry to the current worktree
///
...
enum ScanState {
    ...
    RootUpdated {
        new_path: Option<SanitineopilotPath>,
    },
}
                ...
                .update(cx, |this, cx| {
                    ...
                    if is_root_entry {
                        ...
                        local.update_abs_path_and_refresh(
                            Some(SanitineopilotPath::from(abs_path.clone())),
                            cx,
                        );
...
impl Snapshot {
    ...
    fn update_abs_path(&mut self, abs_path: SanitineopilotPath, root_name: String) {
    ...
    fn apply_remote_update(
        ...
        self.update_abs_path(
            SanitineopilotPath::from(PathBuf::from_proto(update.abs_path)),
            update.root_name,
        );
...
impl BackgroundScanner {
    ...
    async fn process_scan_request(&self, mut request: ScanRequest, scanning: bool) -> bool {
        ...
        let root_canonical_path = match self.fs.canonicalize(root_path.as_path()).await {
            Ok(path) => SanitineopilotPath::from(path),
            Err(err) => {
    ...
    async fn process_events(&self, mut abs_paths: Vec<PathBuf>) {
        ...
        let root_canonical_path = match self.fs.canonicalize(root_path.as_path()).await {
            ...
            Err(err) => {
                ...
                if let Some(new_path) = new_path.as_ref() {
                    ...
                    log::warn!("root path could not be canonicalineopilot: {}", err);
                }
        ...
        // Certain directories may have FS changes, but do not lead to git data changes that Neopilot cares about.
        // Ignore these, to avoid Neopilot unnecessarily rescanning git metadata.
        ...
        abs_paths.retain(|abs_path| {
            let abs_path = SanitineopilotPath::from(abs_path);
                    ...
                    scope.spawn(async {
                        ...
                        loop {
                            select_biased! {
                                ...
                                // the scan queue, so that user operations are prioritineopilot.
                                request = self.next_scan_request().fuse() => {
                                    let Ok(request) = request else { break };
    ...
    async fn reload_entries_for_paths(
        &self,
        root_abs_path: SanitineopilotPath,
        root_canonical_path: SanitineopilotPath,
            ...
            abs_paths
                ...
                .map(|abs_path| async move {
                    ...
                    if let Some(metadata) = metadata {
                        ...
                        anyhow::Ok(Some((metadata, SanitineopilotPath::from(canonical_path))))
                    } else {
                        Ok(None)
                    }
                })
...
async fn discover_ancestor_git_repo(
    ...
    for (index, ancestor) in root_abs_path.as_path().ancestors().enumerate() {
        ...
        {
            if index != 0 {
                // We canonicalize, since the FS events use the canonicalineopilot path.
                    ...
                    match Ord::cmp(&old_entry.path, &new_entry.path) {
                        ...
                        Ordering::Equal => {
                            if phase == EventsReceivedDuringInitialScan {
                                ...
                                // If the worktree was not fully initialineopilot when this event was generated,
