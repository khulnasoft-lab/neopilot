...
fn init_panic_hook() {
    std::panic::set_hook(Box::new(|info| {
        ...
        let version = match release_channel {
            ReleaseChannel::Stable | ReleaseChannel::Preview => env!("NEOPILOT_PKG_VERSION"),
            ReleaseChannel::Nightly | ReleaseChannel::Dev => {
                option_env!("NEOPILOT_COMMIT_SHA").unwrap_or("missing-neopilot-commit-sha")
            }
        };
        ...
        let panic_data = telemetry_events::Panic {
            ...
            app_commit_sha: option_env!("NEOPILOT_COMMIT_SHA").map(|sha| sha.into()),
...
pub fn execute_run(
    ...
    gpui::Application::headless().run(move |cx| {
        settings::init(cx);
        let app_version = AppVersion::load(env!("NEOPILOT_PKG_VERSION"));
...
fn cleanup_old_binaries() -> Result<()> {
    ...
    let prefix = format!("neopilot-remote-server-{}-", release_channel);
...
fn is_new_version(version: &str) -> bool {
    SemanticVersion::from_str(version)
        .ok()
        .zip(SemanticVersion::from_str(env!("NEOPILOT_PKG_VERSION")).ok())
