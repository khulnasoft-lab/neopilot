...
async fn test_extension_store(cx: &mut TestAppContext) {
    ...
    let mut expected_index = ExtensionIndex {
        extensions: [
            (
                ...
                ExtensionIndexEntry {
                    manifest: Arc::new(ExtensionManifest {
                        id: "neopilot-ruby".into(),
                        name: "Neopilot Ruby".into(),
                        version: "1.0.0".into(),
            ...
            (
                "neopilot-monokai".into(),
                ExtensionIndexEntry {
                    manifest: Arc::new(ExtensionManifest {
                        id: "neopilot-monokai".into(),
                        name: "Neopilot Monokai".into(),
                        version: "2.0.0".into(),
                        schema_version: SchemaVersion::ZERO,
                        description: None,
                        authors: vec![],
        ...
        themes: [
            (
                ...
                ExtensionIndexThemeEntry {
                    extension: "neopilot-monokai".into(),
    ...
    fs.insert_tree(
        "/the-extension-dir/installed/neopilot-gruvbox",
        json!({
            "extension.json": r#"{
                "id": "neopilot-gruvbox",
                "name": "Neopilot Gruvbox",
                "version": "1.0.0",
                "themes": {
    ...
    expected_index.extensions.insert(
        "neopilot-gruvbox".into(),
        ExtensionIndexEntry {
            manifest: Arc::new(ExtensionManifest {
                id: "neopilot-gruvbox".into(),
                name: "Neopilot Gruvbox".into(),
                version: "1.0.0".into(),
                schema_version: SchemaVersion::ZERO,
                description: None,
                authors: vec![],
    ...
    expected_index.themes.insert(
        "Gruvbox".into(),
        ExtensionIndexThemeEntry {
            extension: "neopilot-gruvbox".into(),
    ...
    // Create new extension store, as if Neopilot were restarting.
    drop(store);
    let store = cx.new(|cx| {
        ExtensionStore::new(
            PathBuf::from("/the-extension-dir"),
    ...
    store.update(cx, |store, cx| {
        store.uninstall_extension("neopilot-ruby".into(), cx)
    });

    cx.executor().advance_clock(RELOAD_DEBOUNCE_DURATION);
    expected_index.extensions.remove("neopilot-ruby");
