...
pub use neopilot_actions::{Rerun, Spawn};
        ...
        fs.insert_tree(
            ...
            json!({
                ".neopilot": {
                    "tasks.json": r#"[
                        {
                            "label": "hello from $NEOPILOT_FILE:$NEOPILOT_ROW:$NEOPILOT_COLUMN",
                            "command": "echo",
                            "args": ["hello", "from", "$NEOPILOT_FILE", ":", "$NEOPILOT_ROW", ":", "$NEOPILOT_COLUMN"]
                        },
                        {
                            "label": "opened now: $NEOPILOT_WORKTREE_ROOT",
                            "command": "echo",
                            "args": ["opened", "now:", "$NEOPILOT_WORKTREE_ROOT"]
                        }
                    ]"#,
                },
            ...
            language_registry.add(Arc::new(
                ...
                .with_context_provider(Some(Arc::new(
                    ContextProviderWithTasks::new(TaskTemplates(vec![
                        ...
                        TaskTemplate {
                            label: "TypeScript task from file $NEOPILOT_FILE".to_string(),
                            command: "npm run build".to_string(),
                            ..TaskTemplate::default()
                        },
                        TaskTemplate {
                            label: "Another task from file $NEOPILOT_FILE".to_string(),
