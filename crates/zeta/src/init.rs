...
use crate::{RateCompletionModal, onboarding_modal::NeopilotPredictModal};
...
pub fn init(cx: &mut App) {
    cx.observe_new(move |workspace: &mut Workspace, _, _cx| {
        ...
        workspace.register_action(
            move |workspace, _: &neopilot_actions::OpenNeopilotPredictOnboarding, window, cx| {
                NeopilotPredictModal::toggle(
                    workspace,
...
fn feature_gate_predict_edits_rating_actions(cx: &mut App) {
    ...
    CommandPaletteFilter::update_global(cx, |filter, _cx| {
        ...
        filter.hide_action_types(&[neopilot_actions::OpenNeopilotPredictOnboarding.type_id()]);
    });
