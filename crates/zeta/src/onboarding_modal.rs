...
use crate::{NEOPILOT_PREDICT_DATA_COLLECTION_CHOICE, onboarding_event};
...
/// Introduces user to Neopilot's Edit Prediction feature and terms of service
pub struct NeopilotPredictModal {
...
impl NeopilotPredictModal {
    ...
    fn view_terms(&mut self, _: &ClickEvent, _: &mut Window, cx: &mut Context<Self>) {
        cx.open_url("https://neopilot.dev/terms-of-service");
        cx.notify();
    ...
    fn view_blog(&mut self, _: &ClickEvent, _: &mut Window, cx: &mut Context<Self>) {
        cx.open_url("https://neopilot.dev/blog/edit-prediction");
        cx.notify();
    ...
    fn inline_completions_doc(&mut self, _: &ClickEvent, _: &mut Window, cx: &mut Context<Self>) {
        cx.open_url("https://neopilot.dev/docs/configuring-neopilot#disabled-globs");
        cx.notify();
    ...
    fn accept_and_enable(&mut self, _: &ClickEvent, window: &mut Window, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |this, cx| {
            ...
            KEY_VALUE_STORE
                .write_kvp(
                    NEOPILOT_PREDICT_DATA_COLLECTION_CHOICE.into(),
            ...
            this.update(cx, |this, cx| {
                update_settings_file::<AllLanguageSettings>(this.fs.clone(), cx, move |file, _| {
                    file.features
                        ...
                        .edit_prediction_provider = Some(EditPredictionProvider::Neopilot);
                });
...
impl EventEmitter<DismissEvent> for NeopilotPredictModal {}

impl Focusable for NeopilotPredictModal {
    fn focus_handle(&self, _cx: &App) -> FocusHandle {
        self.focus_handle.clone()
    }
}

impl ModalView for NeopilotPredictModal {}
...
impl NeopilotPredictModal {
    fn render_data_collection_explanation(&self, cx: &Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(info_item(
                "Neopilot automatically detects if your project is open source.",
            ))
            ...
            .child(multiline_info_item(
                ...
                label_item("you open in Neopilot.")
            ))
            .child(multiline_info_item(
...
impl Render for NeopilotPredictModal {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let base = v_flex()
            ...
            .key_context("NeopilotPredictModal")
            .relative()
            .w(px(550.))
            .h_full()
            .max_h(max_height)
            ...
            .child(
                div()
                    ...
                    .child(
                        svg()
                            .path("icons/neopilot_predict_bg.svg")
                ...
                h_flex()
                    ...
                    .child(
                        v_flex()
                            .gap_1()
                            .child(
                                Label::new("Introducing Neopilot AI's")
        ...
        if self.user_store.read(cx).current_user().is_some() {
            let copy = match self.sign_in_status {
                SignInStatus::Idle => {
                    "Neopilot can now predict your next edit on every keystroke. Powered by Zeta, our open-source, open-dataset language model."
                }
                    ...
                    h_flex().child(
                        Checkbox::new("plan", ToggleState::Selected)
                            ...
                            .label(format!(
                                ...
                                match plan {
                                    proto::Plan::Free => "Neopilot Free plan",
                                    proto::Plan::NeopilotPro => "Neopilot Pro plan",
                                    proto::Plan::NeopilotProTrial => "Neopilot Pro trial",
                                }
                            )),
                    ),
                )
            ...
            base.child(
                Label::new("To set Neopilot as your edit prediction provider, please sign in.")
                    .color(Color::Muted),
            )
            .child(
                v_flex()
