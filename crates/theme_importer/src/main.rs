...
const NEOPILOT_THEME_SCHEMA_URL: &str = "https://neopilot.dev/schema/themes/v0.2.0.json";
...
struct FamilyMetadata {
    ...
    ///
    /// Use this to ensure certain Neopilot syntax tokens are matched
...
fn main() -> Result<()> {
    ...
    theme.as_object_mut().unwrap().insert(
        "$schema".to_string(),
        serde_json::Value::String(NEOPILOT_THEME_SCHEMA_URL.to_string()),
    );
