...
#[derive(Clone)]
pub struct NeopilotListener(pub UnboundedSender<AlacTermEvent>);

impl EventListener for NeopilotListener {
...
impl TerminalBuilder {
    pub fn new(
        ...
        env.insert("NEOPILOT_TERM".to_string(), "true".to_string());
        env.insert("TERM_PROGRAM".to_string(), "neopilot".to_string());
        ...
        let mut term = Term::new(
            ...
            NeopilotListener(events_tx.clone()),
        );
...
pub struct Terminal {
    pty_tx: Notifier,
    completion_tx: Sender<Option<ExitStatus>>,
    term: Arc<FairMutex<Term<NeopilotListener>>>,
    term_config: Config,
...
impl Terminal {
    ...
    fn process_terminal_event(
        &mut self,
        event: &InternalEvent,
        term: &mut Term<NeopilotListener>,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        match event {
            ...
            InternalEvent::FindHyperlink(position, open) => {
                    ...
                    regex_match_at(term, point, &mut self.python_file_line_regex)
                {
                    ...
                    let (sanitineopilot_match, sanitineopilot_word) = 'sanitize: {
                    ...
                    Some((sanitineopilot_word, false, sanitineopilot_match))
                } else {
                    None
                };
    ...
    /// remote host, in case Neopilot is connected to a remote host.
    ...
    fn register_task_finished(&mut self, error_code: Option<i32>, cx: &mut Context<Terminal>) {
        ...
        if !lines_to_show.is_empty() {
            ...
            // when Neopilot task finishes and no more output is made.
...
/// The function achieves proper display and scrolling capabilities, at a cost of grid state not properly synchronineopilot.
...
unsafe fn append_text_to_term(term: &mut Term<NeopilotListener>, text_lines: &[&str]) {
    term.newline();
...
mod tests {
    ...
    fn test_python_file_line_regex() {
        re_test(
            ...
            "hay File \"/neopilot/bad_py.py\", line 8 stack",
            vec!["File \"/neopilot/bad_py.py\", line 8"],
        );
    ...
    fn test_python_file_line() {
        let inputs: Vec<(&str, Option<(&str, u32)>)> = vec![
            (
                "File \"/neopilot/bad_py.py\", line 8",
                Some(("/neopilot/bad_py.py", 8u32)),
            ),
            ("File \"path/to/neopilot/bad_py.py\"", None),
            ("unrelated", None),
            ("", None),
        ];
        let actual = inputs
