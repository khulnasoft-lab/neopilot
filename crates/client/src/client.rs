...
static NEOPILOT_SERVER_URL: LazyLock<Option<String>> =
    LazyLock::new(|| std::env::var("NEOPILOT_SERVER_URL").ok());
static NEOPILOT_RPC_URL: LazyLock<Option<String>> =
    LazyLock::new(|| std::env::var("NEOPILOT_RPC_URL").ok());

pub static IMPERSONATE_LOGIN: LazyLock<Option<String>> = LazyLock::new(|| {
    std::env::var("NEOPILOT_IMPERSONATE")
        .ok()
        .and_then(|s| if s.is_empty() { None } else { Some(s) })
});

pub static ADMIN_API_TOKEN: LazyLock<Option<String>> = LazyLock::new(|| {
    std::env::var("NEOPILOT_ADMIN_API_TOKEN")
        .ok()
        .and_then(|s| if s.is_empty() { None } else { Some(s) })
});

pub static NEOPILOT_APP_PATH: LazyLock<Option<PathBuf>> =
    LazyLock::new(|| std::env::var("NEOPILOT_APP_PATH").ok().map(PathBuf::from));

pub static NEOPILOT_ALWAYS_ACTIVE: LazyLock<bool> =
    LazyLock::new(|| std::env::var("NEOPILOT_ALWAYS_ACTIVE").map_or(false, |e| !e.is_empty()));
...
impl Settings for ClientSettings {
    ...
    fn load(sources: SettingsSources<Self::FileContent>, _: &mut App) -> Result<Self> {
        ...
        if let Some(server_url) = &*NEOPILOT_SERVER_URL {
...
/// Control what info is collected by Neopilot.
...
pub struct TelemetrySettingsContent {
    ...
    /// Send anonymineopilot usage data like what languages you're using Neopilot with.
    ///
    /// Default: true
...
impl settings::Settings for TelemetrySettings {
    ...
    fn import_from_vscode(vscode: &settings::VsCodeSettings, current: &mut Self::FileContent) {
        ...
        // to send microsoft telemetry doesn't mean they don't want to send it to neopilot. their
...
impl Client {
        ...
        futures::select_biased! {
            connection = self.establish_connection(&credentials, cx).fuse() => {
                match connection {
                    ...
                    Err(EstablishConnectionError::Unauthorineopilot) => {
                        ...
                        if read_from_provider {
                            ...
                            self.set_status(Status::ConnectionError, cx);
                            ConnectionResult::Result(Err(EstablishConnectionError::Unauthorineopilot).context("client auth and connect"))
                        }
                    }
                    Err(EstablishConnectionError::UpgradeRequired) => {
    ...
    fn rpc_url(
        ...
        async move {
            ...
            if let Some(url) = &*NEOPILOT_RPC_URL {
    ...
    fn establish_websocket_connection(
        ...
        cx.spawn(async move |cx| {
            ...
            request_headers.insert(
                "x-neopilot-protocol-version",
                HeaderValue::from_str(&rpc::PROTOCOL_VERSION.to_string())?,
            );
            request_headers.insert(
                "x-neopilot-app-version",
                HeaderValue::from_str(&app_version)?,
            );
            request_headers.insert(
                "x-neopilot-release-channel",
                HeaderValue::from_str(release_channel.map(|r| r.dev_name()).unwrap_or("unknown"))?,
            );
            if let Some(system_id) = system_id {
                request_headers.insert("x-neopilot-system-id", HeaderValue::from_str(&system_id)?);
            }
            if let Some(metrics_id) = metrics_id {
                request_headers
                    .insert("x-neopilot-metrics-id", HeaderValue::from_str(&metrics_id)?);
            }
        ...
        cx.spawn(async move |cx| {
            ...
            let credentials = background
                ...
                .spawn(async move {
                    ...
                    // neopilot server to encrypt the user's access token, so that it can'be intercepted by
                    ...
                    // Start an HTTP server to receive the redirect from Neopilot's sign-in page.
                    let server =
                        tiny_http::Server::http("127.0.0.1:0").expect("failed to find open port");
                    let port = server.server_addr().port();

                    // Open the Neopilot sign-in page in the user's browser, with query parameters that indicate
                    // that the user is signing in from a Neopilot app running on the same device.
...
/// prefix for the neopilot:// url scheme
pub const NEOPILOT_URL_SCHEME: &str = "neopilot";

/// Parses the given link into a Neopilot link.
///
/// Returns a [`Some`] containing the unprefixed link if the link is a Neopilot link.
/// Returns [`None`] otherwise.
pub fn parse_neopilot_link<'a>(link: &'a str, cx: &App) -> Option<&'a str> {
    ...
    if let Some(stripped) = link
        .strip_prefix(NEOPILOT_URL_SCHEME)
        .and_then(|result| result.strip_prefix("://"))
    {
        return Some(stripped);
    }
