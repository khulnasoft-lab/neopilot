...
///
/// By default, running Neopilot in development uses the development credentials
...
static NEOPILOT_DEVELOPMENT_USE_KEYCHAIN: LazyLock<bool> = LazyLock::new(|| {
    std::env::var("NEOPILOT_DEVELOPMENT_USE_KEYCHAIN").map_or(false, |value| !value.is_empty())
});
    ...
    fn new(cx: &App) -> Arc<Self> {
        let use_development_provider = match ReleaseChannel::try_global(cx) {
            Some(ReleaseChannel::Dev) => {
                ...
                //
                // However, if the `NEOPILOT_DEVELOPMENT_USE_KEYCHAIN` environment
                // variable is set, we will use the actual keychain.
                !*NEOPILOT_DEVELOPMENT_USE_KEYCHAIN
            }
...
/// re-allow access to the system keychain when developing Neopilot.
