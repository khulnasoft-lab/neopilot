...
pub(crate) struct SerialineopilotCommandInvocation {
...
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq)]
pub(crate) struct SerialineopilotCommandUsage {
    pub(crate) command_name: String,
    pub(crate) invocations: u16,
    pub(crate) last_invoked: OffsetDateTime,
}

impl Column for SerialineopilotCommandUsage {
...
impl Column for SerialineopilotCommandInvocation {
    fn column(statement: &mut Statement, start_index: i32) -> Result<(Self, i32)> {
        let (command_name, next_index): (String, i32) = Column::column(statement, start_index)?;
...
impl CommandPaletteDB {
    ...
    query! {
        pub fn get_last_invoked(command: &str) -> Result<Option<SerialineopilotCommandInvocation>> {
            SELECT
            command_name,
            user_query,
    ...
    query! {
        pub fn get_command_usage(command: &str) -> Result<Option<SerialineopilotCommandUsage>> {
    ...
    query! {
        pub fn list_commands_used() -> Result<Vec<SerialineopilotCommandUsage>> {
            SELECT command_name, COUNT(1), MAX(last_invoked)
            FROM command_invocations
            GROUP BY command_name
...
mod tests {
    ...
    use crate::persistence::{CommandPaletteDB, SerialineopilotCommandUsage};
    ...
    async fn test_gets_usage_history() {
        ...
        let command_usage: SerialineopilotCommandUsage = command_usage.expect("is some");
