...
use neopilot_actions::{OpenNeopilotUrl, command_palette::Toggle};
...
///
/// This improves the likelihood of a match by either humanineopilot name or keymap-style name.
...
impl CommandPaletteDelegate {
    ...
    fn matches_updated(
        ...
        if parse_neopilot_link(&query, cx).is_some() {
            intercept_results = vec![CommandInterceptResult {
                action: OpenNeopilotUrl { url: query.clone() }.boxed_clone(),
                string: query.clone(),
                positions: vec![],
            }]
        }
...
mod tests {
    ...
    #[gpui::test]
    async fn test_normalineopilot_matches(cx: &mut TestAppContext) {
        let app_state = init_test(cx);
        let project = Project::test(app_state.fs.clone(), [], cx).await;
        let (workspace, cx) =
            cx.add_window_view(|window, cx| Workspace::test_new(project.clone(), window, cx));
