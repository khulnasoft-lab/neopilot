...
pub use connection_pool::{ConnectionPool, NeopilotVersion};
...
impl Server {
        ...
        self.app_state.executor.spawn_detached(
            async move {
                ...
                if let Some((room_ids, channel_ids)) = app_state
                    .db
                    .stale_server_resource_ids(&app_state.config.neopilot_environment, server_id)
                    .await
                    .trace_err()
                {
                ...
                app_state
                    .db
                    .delete_stale_servers(&app_state.config.neopilot_environment, server_id)
                    .await
                    .trace_err();
            }
            .instrument(span),
    ...
    pub fn handle_connection(
        ...
        address: String,
        principal: Principal,
        neopilot_version: NeopilotVersion,
        geoip_country_code: Option<String>,
        system_id: Option<String>,
        ...
        async move {
            ...
            let user_agent = format!("Neopilot Server/{}", env!("CARGO_PKG_VERSION"));
            ...
            if let Err(error) = this.send_initial_client_update(connection_id, neopilot_version, send_connection_id, &session).await {
        ...
        match &session.principal {
            Principal::User(user) | Principal::Impersonated { user, admin: _ } => {
...

                {
                    ...
                    pool.add_connection(connection_id, user.id, user.admin, neopilot_version);
                    self.peer.send(
                        connection_id,
                ...
                if should_auto_subscribe_to_channels(neopilot_version) {
...
impl Header for ProtocolVersion {
    fn name() -> &'static HeaderName {
        static NEOPILOT_PROTOCOL_VERSION: OnceLock<HeaderName> = OnceLock::new();
        NEOPILOT_PROTOCOL_VERSION
            .get_or_init(|| HeaderName::from_static("x-neopilot-protocol-version"))
    }
...
impl Header for AppVersionHeader {
    fn name() -> &'static HeaderName {
        static NEOPILOT_APP_VERSION: OnceLock<HeaderName> = OnceLock::new();
        NEOPILOT_APP_VERSION.get_or_init(|| HeaderName::from_static("x-neopilot-app-version"))
    }
...
pub async fn handle_websocket_request(
    ...
    let Some(version) = app_version_header.map(|header| NeopilotVersion(header.0.0)) else {
        return (
            StatusCode::UPGRADE_REQUIRED,
...
fn should_auto_subscribe_to_channels(version: NeopilotVersion) -> bool {
    version.0.minor() < 139
}

async fn current_plan(db: &Arc<Database>, user_id: UserId, is_staff: bool) -> Result<proto::Plan> {
    if is_staff {
        return Ok(proto::Plan::NeopilotPro);
    }
    ...
    let plan = if let Some(subscription_kind) = subscription_kind {
        match subscription_kind {
            SubscriptionKind::NeopilotPro => proto::Plan::NeopilotPro,
            SubscriptionKind::NeopilotProTrial => proto::Plan::NeopilotProTrial,
            SubscriptionKind::NeopilotFree => proto::Plan::Free,
        }
    } else {
        proto::Plan::Free
    };
...
async fn make_update_user_plan_message(
    ...
    let account_too_young = !matches!(plan, proto::Plan::NeopilotPro)
    ...
    Ok(proto::UpdateUserPlan {
        ...
        usage: usage.map(|usage| {
            let plan = match plan {
                proto::Plan::Free => neopilot_llm_client::Plan::NeopilotFree,
                proto::Plan::NeopilotPro => neopilot_llm_client::Plan::NeopilotPro,
                proto::Plan::NeopilotProTrial => neopilot_llm_client::Plan::NeopilotProTrial,
            };

            let model_requests_limit = match plan.model_requests_limit() {
                neopilot_llm_client::UsageLimit::Limited(limit) => {
                    let limit = if plan == neopilot_llm_client::Plan::NeopilotProTrial
                        && feature_flags
                            .iter()
...
async fn join_channel_internal(
    ...
    let joined_room = {
        ...
        // If neopilot quits without leaving the room, and the user re-opens neopilot before the
    ...
    let billing_subscription =
        if let Some(billing_subscription) = db.get_active_billing_subscription(user.id).await? {
            ...
            db.create_billing_subscription(&db::CreateBillingSubscriptionParams {
                ...
                kind: Some(SubscriptionKind::NeopilotFree),
