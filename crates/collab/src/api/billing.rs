        ...
        subscriptions: subscriptions
            ...
            .map(|subscription| BillingSubscriptionJson {
                ...
                name: match subscription.kind {
                    Some(SubscriptionKind::NeopilotPro) => "Neopilot Pro".to_string(),
                    Some(SubscriptionKind::NeopilotProTrial) => "Neopilot Pro (Trial)".to_string(),
                    Some(SubscriptionKind::NeopilotFree) => "Neopilot Free".to_string(),
                    None => "Neopilot LLM Usage".to_string(),
                },
                status: subscription.stripe_subscription_status,
                trial_end_at: if subscription.kind == Some(SubscriptionKind::NeopilotProTrial) {
                    maybe!({
                ...
                is_cancelable: subscription.kind != Some(SubscriptionKind::NeopilotFree)
...
async fn create_billing_subscription(
    ...
    if let Some(existing_subscription) = app.db.get_active_billing_subscription(user.id).await? {
        let is_checkout_allowed = body.product == ProductCode::NeopilotProTrial
            && existing_subscription.kind == Some(SubscriptionKind::NeopilotFree);
    ...
    let success_url = format!(
        ...
        app.config.neopilot_dot_dev_url()
    );

    let checkout_session_url = match body.product {
        ProductCode::NeopilotPro => {
            stripe_billing
                .checkout_with_neopilot_pro(customer_id, &user.github_login, &success_url)
                .await?
        }
        ProductCode::NeopilotProTrial => {
            if let Some(existing_billing_customer) = &existing_billing_customer {
            ...
            stripe_billing
                .checkout_with_neopilot_pro_trial(
                    customer_id,
                    &user.github_login,
                    feature_flags,
                    &success_url,
        ...
        ProductCode::NeopilotFree => {
            stripe_billing
                .checkout_with_neopilot_free(customer_id, &user.github_login, &success_url)
                .await?
        }
    };
...
enum ManageSubscriptionIntent {
    ...
    UpdatePaymentMethod,
    /// The user intends to upgrade to Neopilot Pro.
    UpgradeToPro,
...
async fn manage_billing_subscription(
    ...
    let flow = match body.intent {
        ...
        ManageSubscriptionIntent::UpgradeToPro => {
            let neopilot_pro_price_id = stripe_billing.neopilot_pro_price_id().await?;
            let neopilot_free_price_id = stripe_billing.neopilot_free_price_id().await?;

            let stripe_subscription =
                Subscription::retrieve(&stripe_client, &subscription_id, &[]).await?;

            let is_on_neopilot_pro_trial = stripe_subscription.status
                == SubscriptionStatus::Trialing
                && stripe_subscription.items.data.iter().any(|item| {
                    item.price
                        .as_ref()
                        .map_or(false, |price| price.id == neopilot_pro_price_id)
                });
            if is_on_neopilot_pro_trial {
                ...
                // If the user is already on a Neopilot Pro trial and wants to upgrade to Pro, we just need to end their trial early.
                Subscription::update(
                    &stripe_client,
            ...
            let subscription_item_to_update = stripe_subscription
                ...
                .find_map(|item| {
                    ...
                    if price.id == neopilot_free_price_id {
                        Some(item.id.clone())
                    } else {
                        None
                    }
                    ...
                    CreateBillingPortalSessionFlowDataSubscriptionUpdateConfirm {
                        ...
                        items: vec![
                            CreateBillingPortalSessionFlowDataSubscriptionUpdateConfirmItems {
                                ...
                                price: Some(neopilot_pro_price_id.to_string()),
                                quantity: Some(1),
                            },
                        ],
                        discounts: None,
        ...
        ManageSubscriptionIntent::Cancel => {
            if subscription.kind == Some(SubscriptionKind::NeopilotFree) {
                return Err(Error::http(
                    StatusCode::BAD_REQUEST,
            ...
            Some(CreateBillingPortalSessionFlowData {
                ...
                after_completion: Some(CreateBillingPortalSessionFlowDataAfterCompletion {
                    type_: stripe::CreateBillingPortalSessionFlowDataAfterCompletionType::Redirect,
                    redirect: Some(CreateBillingPortalSessionFlowDataAfterCompletionRedirect {
                        return_url: format!("{}/account", app.config.neopilot_dot_dev_url()),
                    }),
                    ..Default::default()
                }),
    ...
    let return_url = format!("{}/account", app.config.neopilot_dot_dev_url());
...
async fn sync_subscription(
    ...
    if let Some(SubscriptionKind::NeopilotProTrial) = subscription_kind {
        ...
        .await?
    {
        ...
        if let Some(existing_subscription) = app
            ...
            .await?
        {
            if existing_subscription.kind == Some(SubscriptionKind::NeopilotFree)
                && subscription_kind == Some(SubscriptionKind::NeopilotProTrial)
            {
    ...
    if let Some(stripe_billing) = app.stripe_billing.as_ref() {
            ...
            || subscription.status == SubscriptionStatus::Paused
        {
            ...
            if !already_has_active_billing_subscription {
                ...
                stripe_billing
                    .subscribe_to_neopilot_free(stripe_customer_id)
                    .await?;
            }
        }
    }
...
async fn get_current_usage(
    ...
    let plan = subscription
        .kind
        .map(Into::into)
        .unwrap_or(neopilot_llm_client::Plan::NeopilotFree);

    let model_requests_limit = match plan.model_requests_limit() {
        neopilot_llm_client::UsageLimit::Limited(limit) => {
            let limit = if plan == neopilot_llm_client::Plan::NeopilotProTrial && has_extended_trial
            {
                1_000
            } else {
                limit
        ...
        neopilot_llm_client::UsageLimit::Unlimited => None,
    };

    let edit_predictions_limit = match plan.edit_predictions_limit() {
        neopilot_llm_client::UsageLimit::Limited(limit) => Some(limit),
        neopilot_llm_client::UsageLimit::Unlimited => None,
    };
...
async fn sync_model_request_usage_with_stripe(
    ...
    let billing_subscriptions = app
        .db
        .get_active_neopilot_pro_billing_subscriptions(user_ids)
        .await?;
