...
const CRASH_REPORTS_BUCKET: &str = "neopilot-crash-reports";
...
pub struct NeopilotChecksumHeader(Vec<u8>);

impl Header for NeopilotChecksumHeader {
    fn name() -> &'static HeaderName {
        static NEOPILOT_CHECKSUM_HEADER: OnceLock<HeaderName> = OnceLock::new();
        NEOPILOT_CHECKSUM_HEADER.get_or_init(|| HeaderName::from_static("x-neopilot-checksum"))
    }
...
pub async fn post_crash(
    ...
    if bundle_id == "dev.neopilot.Neopilot-Dev" {
    ...
    let recent_panic_on: Option<i64> = headers
        .get("x-neopilot-panicked-on")
        .and_then(|h| h.to_str().ok())
        .and_then(|s| s.parse().ok());

    let installation_id = headers
        .get("x-neopilot-installation-id")
        .and_then(|h| h.to_str().ok())
        .map(|s| s.to_string())
        .unwrap_or_default();
...
pub async fn post_hang(
    Extension(app): Extension<Arc<AppState>>,
    TypedHeader(NeopilotChecksumHeader(checksum)): TypedHeader<NeopilotChecksumHeader>,
    body: Bytes,
) -> Result<()> {
...
pub fn calculate_json_checksum(app: Arc<AppState>, json: &impl AsRef<[u8]>) -> Option<Vec<u8>> {
    let checksum_seed = app.config.neopilot_client_checksum_seed.as_ref()?;
...
fn for_snowflake(
    ...
    body.events.into_iter().filter_map(move |event| {
        ...
        // dictionary. See https://app.amplitude.com/data/neopilot/Neopilot/sources/detail/production/falcon%3A159998
