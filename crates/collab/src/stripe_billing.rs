...
impl StripeBilling {
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        log::info!("StripeBilling: initialineopilot");

        Ok(())
    }

    pub async fn neopilot_pro_price_id(&self) -> Result<PriceId> {
        self.find_price_id_by_lookup_key("neopilot-pro").await
    }

    pub async fn neopilot_free_price_id(&self) -> Result<PriceId> {
        self.find_price_id_by_lookup_key("neopilot-free").await
    }
    ...
    pub async fn determine_subscription_kind(
        ...
        let neopilot_pro_price_id = self.neopilot_pro_price_id().await.ok()?;
        let neopilot_free_price_id = self.neopilot_free_price_id().await.ok()?;

        subscription.items.data.iter().find_map(|item| {
            let price = item.price.as_ref()?;

            if price.id == neopilot_pro_price_id {
                Some(if subscription.status == SubscriptionStatus::Trialing {
                    SubscriptionKind::NeopilotProTrial
                } else {
                    SubscriptionKind::NeopilotPro
                })
            } else if price.id == neopilot_free_price_id {
                Some(SubscriptionKind::NeopilotFree)
            } else {
                None
            }
        })
    ...
    pub async fn checkout_with_neopilot_pro(
        &self,
        customer_id: stripe::CustomerId,
        github_login: &str,
        success_url: &str,
    ) -> Result<String> {
        let neopilot_pro_price_id = self.neopilot_pro_price_id().await?;
    ...
    pub async fn checkout_with_neopilot_pro_trial(
        &self,
        customer_id: stripe::CustomerId,
        github_login: &str,
    ...
    pub async fn subscribe_to_neopilot_free(
        &self,
        customer_id: stripe::CustomerId,
    ) -> Result<stripe::Subscription> {
        let neopilot_free_price_id = self.neopilot_free_price_id().await?;
        ...
        params.items = Some(vec![stripe::CreateSubscriptionItems {
            price: Some(neopilot_free_price_id.to_string()),
            quantity: Some(1),
            ..Default::default()
        }]);
    ...
    pub async fn checkout_with_neopilot_free(
        &self,
        customer_id: stripe::CustomerId,
        github_login: &str,
        success_url: &str,
