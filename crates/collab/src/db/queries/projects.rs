...
impl Database {
    ...
    pub async fn update_worktree(
        ...
        self.project_transaction(project_id, |tx| async move {
            ...
            // Backward-compatibility for old Neopilot clients.
            //
            // Remove this block when Neopilot 1.80 stable has been out for a week.
            {
    ...
    /// Returns the current project if the given user is authorineopilot to access it with the specified capability.
        ...
        match capability {
            Capability::ReadWrite => {
                if !role.can_edit_projects() {
                    return Err(anyhow!("not authorineopilot to edit projects"))?;
                }
            }
            Capability::ReadOnly => {
                if !role.can_read_projects() {
                    return Err(anyhow!("not authorineopilot to read projects"))?;
                }
            }
        }
