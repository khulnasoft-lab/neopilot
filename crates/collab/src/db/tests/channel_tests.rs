...
async fn test_channels(db: &Arc<Database>) {
    ...
    let neopilot_id = db.create_root_channel("neopilot", a_id).await.unwrap();

    // Make sure that people cannot read channels they haven't been invited to
    assert!(db.get_channel(neopilot_id, b_id).await.is_err());

    db.invite_channel_member(neopilot_id, b_id, a_id, ChannelRole::Member)
        .await
        .unwrap();

    db.respond_to_channel_invite(neopilot_id, b_id, true)
        .await
        .unwrap();

    let crdb_id = db
        .create_sub_channel("crdb", neopilot_id, a_id)
        .await
        .unwrap();
    let livestreaming_id = db
        .create_sub_channel("livestreaming", neopilot_id, a_id)
        .await
        .unwrap();
    let replace_id = db
        .create_sub_channel("replace", neopilot_id, a_id)
        .await
        .unwrap();
    ...
    assert_eq!(
        result.channels,
        channel_tree(&[
            (neopilot_id, &[], "neopilot"),
            (crdb_id, &[neopilot_id], "crdb"),
            (livestreaming_id, &[neopilot_id], "livestreaming",),
            (replace_id, &[neopilot_id], "replace"),
            (rust_id, &[], "rust"),
    ...
    let set_channel_admin = db
        .set_channel_member_role(neopilot_id, a_id, b_id, ChannelRole::Admin)
        .await;
...
async fn test_channel_renames(db: &Arc<Database>) {
    ...
    let neopilot_id = db.create_root_channel("neopilot", user_1).await.unwrap();

    db.rename_channel(neopilot_id, user_1, "#neopilot-archive")
        .await
        .unwrap();

    let channel = db.get_channel(neopilot_id, user_1).await.unwrap();
    assert_eq!(channel.name, "neopilot-archive");

    let non_permissioned_rename = db.rename_channel(neopilot_id, user_2, "hacked-lol").await;
    assert!(non_permissioned_rename.is_err());

    let bad_name_rename = db.rename_channel(neopilot_id, user_1, "#").await;
...
async fn test_db_channel_moving(db: &Arc<Database>) {
    ...
    let gpui2_id = db
        .create_sub_channel("gpui2", neopilot_id, a_id)
        .await
        .unwrap();
    ...
    // Initial DAG:
    //     /- gpui2
    // neopilot -- crdb - livestreaming - livestreaming_dag
    let result = db.get_channels_for_user(a_id).await.unwrap();
    assert_channel_tree(
        result.channels,
        &[
            (neopilot_id, &[]),
            (crdb_id, &[neopilot_id]),
            (livestreaming_id, &[neopilot_id, crdb_id]),
            (
                livestreaming_dag_id,
                &[neopilot_id, crdb_id, livestreaming_id],
            ),
            (gpui2_id, &[neopilot_id]),
        ],
    );
}
...
async fn test_db_channel_moving_bugs(db: &Arc<Database>) {
    ...
    let neopilot_id = db.create_root_channel("neopilot", user_id).await.unwrap();

    let projects_id = db
        .create_sub_channel("projects", neopilot_id, user_id)
        .await
        .unwrap();
    ...
    assert_channel_tree(
        ...
        &[
            (neopilot_id, &[]),
            (projects_id, &[neopilot_id]),
            (livestreaming_id, &[neopilot_id, projects_id]),
        ],
    );
...
async fn test_user_is_channel_participant(db: &Arc<Database>) {
    ...
    let neopilot_channel = db.create_root_channel("neopilot", admin).await.unwrap();
    let internal_channel_id = db
        .create_sub_channel("active", neopilot_channel, admin)
        .await
        .unwrap();
    let public_channel_id = db
        .create_sub_channel("vim", neopilot_channel, admin)
        .await
        .unwrap();
    ...
    .await
    .unwrap();
    db.invite_channel_member(neopilot_channel, member, admin, ChannelRole::Member)
        .await
        .unwrap();
    db.invite_channel_member(neopilot_channel, guest, admin, ChannelRole::Guest)
        .await
        .unwrap();

    db.respond_to_channel_invite(neopilot_channel, member, true)
        .await
        .unwrap();
    ...
    db.respond_to_channel_invite(neopilot_channel, guest, true)
        .await
        .unwrap();
    ...
    assert_channel_tree(
        ...
        &[
            (neopilot_channel, &[]),
            (public_channel_id, &[neopilot_channel]),
        ],
    );
    ...
    assert_channel_tree(
        ...
        &[
            (neopilot_channel, &[]),
            (internal_channel_id, &[neopilot_channel]),
            (public_channel_id, &[neopilot_channel]),
        ],
    );

    db.set_channel_member_role(neopilot_channel, admin, guest, ChannelRole::Banned)
        .await
        .unwrap();
    assert!(
        db.transaction(|tx| async move {
    ...
    db.remove_channel_member(neopilot_channel, guest, admin)
        .await
        .unwrap();
    ...
    db.transaction(|tx| async move {
        db.check_user_is_channel_participant(
            &db.get_channel_internal(neopilot_channel, &tx)
                .await
                .unwrap(),
            guest,
            &tx,
...
async fn test_guest_access(db: &Arc<Database>) {
    ...
    assert!(
        db.join_channel_chat(neopilot_channel, guest_connection, guest)
            .await
            .is_err()
    );

    db.join_channel(neopilot_channel, guest, guest_connection)
        .await
        .unwrap();
