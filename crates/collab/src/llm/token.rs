...
impl LlmTokenClaims {
    pub fn create(
        ...
        let plan = if is_staff {
            ...
            subscription
                .kind
                .map_or(Plan::NeopilotFree, |kind| match kind {
                    SubscriptionKind::NeopilotFree => Plan::NeopilotFree,
                    SubscriptionKind::NeopilotPro => Plan::NeopilotPro,
                    SubscriptionKind::NeopilotProTrial => Plan::NeopilotProTrial,
                })
        };
        let subscription_period =
            billing_subscription::Model::current_period(Some(subscription), is_staff)
                .map(|(start, end)| (start.naive_utc(), end.naive_utc()))
                .context("A plan is required to use Neopilot's hosted models or edit predictions. Visit https://neopilot.dev/account to get started.")?;
