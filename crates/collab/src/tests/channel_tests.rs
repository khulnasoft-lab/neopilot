...
async fn test_channel_room(
    ...
    active_call_a
        .update(cx_a, |active_call, cx| {
            active_call.join_channel(neopilot_id, cx)
        })
        .await
        .unwrap();
    ...
    cx_a.read(|cx| {
        client_a.channel_store().read_with(cx, |channels, _| {
            assert_participants_eq(
                channels.channel_participants(neopilot_id),
                &[client_a.user_id().unwrap()],
            );
        })
    });
    ...
    cx_a.read(|cx| {
        client_a.channel_store().read_with(cx, |channels, _| {
            assert_participants_eq(channels.channel_participants(neopilot_id), &[]);
        })
    });
...
async fn test_channel_jumping(executor: BackgroundExecutor, cx_a: &mut TestAppContext) {
    ...
    let neopilot_id = server
        .make_channel("neopilot", None, (&client_a, cx_a), &mut [])
        .await;
    let rust_id = server
...
async fn test_channel_link_notifications(
    ...
    let channels = server
        .make_channel_tree(&[("neopilot", None)], (&client_a, cx_a))
        .await;
    let neopilot_channel = channels[0];
    ...
    try_join_all(client_a.channel_store().update(cx_a, |channel_store, cx| {
        [
            ...
            channel_store.invite_member(neopilot_channel, user_b, proto::ChannelRole::Member, cx),
            channel_store.invite_member(neopilot_channel, user_c, proto::ChannelRole::Guest, cx),
        ]
    }))
    .await
    .unwrap();
    ...
    client_b
        .channel_store()
        .update(cx_b, |channel_store, cx| {
            channel_store.respond_to_channel_invite(neopilot_channel, true, cx)
        })
        .await
        .unwrap();
    ...
    // we have an admin (a), member (b) and guest (c) all part of the neopilot channel.
    ...
    let active_channel = client_a
        .channel_store()
        .update(cx_a, |channel_store, cx| {
            channel_store.create_channel("active", Some(neopilot_channel), cx)
        })
        .await
        .unwrap();
    ...
    assert_channels_list_shape(
        client_a.channel_store(),
        cx_a,
        &[(neopilot_channel, 0), (active_channel, 1)],
    );
    assert_channels_list_shape(
        client_b.channel_store(),
        cx_b,
    ...
    assert_channels_list_shape(client_c.channel_store(), cx_c, &[(neopilot_channel, 0)]);

    let vim_channel = client_a
        .channel_store()
        .update(cx_a, |channel_store, cx| {
            channel_store.create_channel("vim", Some(neopilot_channel), cx)
        })
        .await
        .unwrap();
    ...
    assert_channels_list_shape(
        client_a.channel_store(),
        cx_a,
        &[(neopilot_channel, 0), (active_channel, 1), (vim_channel, 1)],
    );
    assert_channels_list_shape(
        client_b.channel_store(),
        cx_b,
    ...
    assert_channels_list_shape(
        ...
        cx_c,
        &[(neopilot_channel, 0), (vim_channel, 1)],
    );

    let helix_channel = client_a
        .channel_store()
        .update(cx_a, |channel_store, cx| {
            channel_store.create_channel("helix", Some(neopilot_channel), cx)
        })
        .await
        .unwrap();
    ...
    assert_channels_list_shape(
        client_c.channel_store(),
        cx_c,
        &[(neopilot_channel, 0), (vim_channel, 1), (helix_channel, 2)],
    );
}
...
async fn test_channel_membership_notifications(
    ...
    try_join_all(client_a.channel_store().update(cx_a, |channel_store, cx| {
        [
            ...
            channel_store.set_channel_visibility(vim_channel, proto::ChannelVisibility::Public, cx),
            channel_store.invite_member(neopilot_channel, user_b, proto::ChannelRole::Admin, cx),
    ...
    // we have an admin (a), and a guest (b) with access to all of neopilot, and membership in vim.
    assert_channels(
        client_b.channel_store(),
        cx_b,
        &[
    ...
    client_b.channel_store().update(cx_b, |channel_store, _| {
        channel_store.is_channel_admin(neopilot_channel)
    });
