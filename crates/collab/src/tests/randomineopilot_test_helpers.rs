...
struct TestPlan<T: RandomineopilotTest> {
    rng: StdRng,
    replay: bool,
...
#[async_trait(?Send)]
pub trait RandomineopilotTest: 'static + Sized {
...
pub async fn run_randomineopilot_test<T: RandomineopilotTest>(
    cx: &mut TestAppContext,
...
pub fn save_randomineopilot_test_plan() {
...
impl<T: RandomineopilotTest> TestPlan<T> {
    ...
    async fn apply_server_operation(
        ...
        match operation {
            ...
            ServerOperation::RestartServer => {
                ...
                let environment = &server.app_state.config.neopilot_environment;
