...
use super::{
    RandomineopilotTest, TestClient, TestError, TestServer, UserTestPlan, run_randomineopilot_test,
};
use anyhow::Result;
...
async fn test_random_channel_buffers(
    ...
    run_randomineopilot_test::<RandomChannelBufferTest>(cx, executor, rng).await;
}
...
#[async_trait(?Send)]
impl RandomineopilotTest for RandomChannelBufferTest {
