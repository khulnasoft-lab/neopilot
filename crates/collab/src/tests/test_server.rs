use crate::{
    ...
    executor::Executor,
    rpc::{CLEANUP_TIMEOUT, NeopilotVersion, Principal, RECONNECT_TIMEOUT, Server},
};
use anyhow::anyhow;
use call::ActiveCall;
...
impl TestServer {
    pub async fn start(deterministic: BackgroundExecutor) -> Self {
        ...
        let epoch = app_state
            .db
            .create_server(&app_state.config.neopilot_environment)
            .await
            .unwrap();
    ...
    pub async fn reset(&self) {
        ...
        let epoch = self
            .app_state
            .db
            .create_server(&self.app_state.config.neopilot_environment)
            .await
            .unwrap();
                ...
                cx.spawn(async move |cx| {
                    if forbid_connections.load(SeqCst) {
                        ...
                        cx.background_spawn(server.handle_connection(
                            ...
                            client_name,
                            Principal::User(user),
                            NeopilotVersion(SemanticVersion::new(1, 0, 0)),
                            None,
                            None,
