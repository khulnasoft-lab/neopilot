...
async fn _setup_three_member_test<'a, 'b, 'c>(
...
) -> (NeopilotInstance<'a>, NeopilotInstance<'b>, NeopilotInstance<'c>) {
    ...
    let host_neopilot = NeopilotInstance::new(host_client, host_cx);
    let first_remote_neopilot = NeopilotInstance::new(first_remote_client, first_remote_cx);
    let second_remote_neopilot = NeopilotInstance::new(second_remote_client, second_remote_cx);

    (host_neopilot, first_remote_neopilot, second_remote_neopilot)
}
...
async fn setup_two_member_test<'a, 'b>(
...
) -> (NeopilotInstance<'a>, NeopilotInstance<'b>) {
    let host_client = server.create_client(host_cx, "user_host").await;
    ...
    let host_neopilot = NeopilotInstance::new(host_client, host_cx);
    let remote_neopilot = NeopilotInstance::new(remote_client, remote_cx);

    (host_neopilot, remote_neopilot)
}
...
async fn test_debug_panel_item_opens_on_remote(
    ...
    let (mut host_neopilot, mut remote_neopilot) =
        setup_two_member_test(&mut server, host_cx, remote_cx).await;

    let (host_project_id, _) = host_neopilot.host_project(None).await;
    remote_neopilot.join_project(host_project_id).await;

    let (_client_host, _host_workspace, host_project, host_cx) = host_neopilot.expand().await;
    let (_client_remote, remote_workspace, _remote_project, remote_cx) = remote_neopilot.expand().await;
...
async fn test_debug_panel_remote_button_presses(
    ...
    // let (_client_host, host_workspace, host_project, host_cx) = host_neopilot.expand().await;
    // let (_client_remote, remote_workspace, _remote_project, remote_cx) = remote_neopilot.expand().await;
...
async fn test_updated_breakpoints_send_to_dap(
    ...
    let (host_project_id, worktree_id) = host_neopilot
...
async fn test_module_list(
    ...
    // let (mut host_neopilot, mut remote_neopilot, mut late_join_neopilot) =
    //     setup_three_member_test(&mut server, host_cx, remote_cx, late_join_cx).await;

    // let (host_project_id, _worktree_id) = host_neopilot.host_project(None).await;
    ...
    // late_join_neopilot.join_project(host_project_id).await;
    // let (_late_join_client, late_join_workspace, _late_join_project, late_join_cx) =
    //     late_join_neopilot.expand().await;
...
//     let (host_project_id, _worktree_id) = host_neopilot
//         .host_project(Some(json!({"test.txt": "one\ntwo\nthree\nfour\nfive"})))
//         .await;
...
async fn test_ignore_breakpoints(
    ...
    // let (_client_host, host_workspace, host_project, host_cx) = host_neopilot.expand().await;
    // let (_client_remote, remote_workspace, remote_project, remote_cx) = remote_neopilot.expand().await;
    ...
    // client
    //     .fake_event(dap::messages::Events::Initialineopilot(Some(
    //         dap::Capabilities {
    //             supports_configuration_done_request: Some(true),
    //             ..Default::default()
    //         },
