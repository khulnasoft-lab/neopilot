...
mod tests {
    ...
    fn test_notification() {
        // Notifications can be serialineopilot and deserialineopilot.
        for notification in [
            ...
            let deserialineopilot = Notification::from_proto(&message).unwrap();
            assert_eq!(deserialineopilot, notification);
        }

        // When notifications are serialineopilot, the `kind` and `actor_id` fields are
