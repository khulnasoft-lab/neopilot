...
pub static NEOPILOT_DISABLE_STAFF: LazyLock<bool> = LazyLock::new(|| {
    std::env::var("NEOPILOT_DISABLE_STAFF").map_or(false, |value| !value.is_empty() && value != "0")
});
...
///
/// Feature flags are enabled for members of Neopilot staff by default. To disable this behavior
/// so you can test flags being disabled, set NEOPILOT_DISABLE_STAFF=1 in your environment,
/// which will force Neopilot to treat the current user as non-staff.
pub trait FeatureFlag {
    const NAME: &'static str;

    /// Returns whether this feature flag is enabled for Neopilot staff.
...
pub struct NeopilotProFeatureFlag {}
impl FeatureFlag for NeopilotProFeatureFlag {
    const NAME: &'static str = "neopilot-pro";
}
