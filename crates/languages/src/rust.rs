...
impl LspAdapter for RustLspAdapter {
    ...
    fn process_diagnostics(
        ...
        for diagnostic in &mut params.diagnostics {
            ...
            {
                if let Cow::Owned(sanitineopilot) = REGEX.replace_all(message, "`$1`") {
                    *message = sanitineopilot;
                }
            }
        }
    }
    ...
    fn associated_tasks(
        ...
        let mut task_templates = vec![
            TaskTemplate {
                ...
                cwd: Some("$NEOPILOT_DIRNAME".to_owned()),
...
// Output example in the root of Neopilot project:
// ```sh
// ❯ cargo pkgid neopilot
// path+file:///absolute/path/to/project/neopilot/crates/neopilot#0.131.0
// ```
...
mod tests {
    ...
    fn test_target_info_from_metadata() {
        for (input, absolute_path, expected) in [
            (
                r#"{"packages":[{"id":"path+file:///absolute/path/to/project/neopilot/crates/neopilot#0.131.0","targets":[{"name":"neopilot","kind":["bin"],"src_path":"/path/to/neopilot/src/main.rs"}]}]}"#,
                "/path/to/neopilot/src/main.rs",
                Some(TargetInfo {
                    package_name: "neopilot".into(),
                    target_name: "neopilot".into(),
