...
use neopilot_actions::{OpenRecent, OpenRemote};
...
impl TitleBar {
    pub fn new(
        ...
        let application_menu = match platform_style {
            PlatformStyle::Mac => {
                if option_env!("NEOPILOT_USE_CROSS_PLATFORM_MENU").is_some() {
        ...
        let banner = cx.new(|cx| {
            OnboardingBanner::new(
                "Agentic Onboarding",
                IconName::NeopilotAssistant,
                "Agentic Editing",
                None,
                neopilot_actions::agent::OpenOnboardingModal.boxed_clone(),
                cx,
            )
        });
        ...
        Some(
            Button::new("project_branch_trigger", branch_name)
                ...
                .tooltip(move |window, cx| {
                    Tooltip::with_meta(
                        "Recent Branches",
                        Some(&neopilot_actions::git::Branch),
                        "Local branches only",
                        window,
                        cx,
                    )
                ...
                .on_click(move |_, window, cx| {
                    let _ = workspace.update(cx, |_this, cx| {
                        window.dispatch_action(neopilot_actions::git::Branch.boxed_clone(), cx);
                    });
                })
                .when(
            ...
            client::Status::UpgradeRequired => {
                ...
                let label = match auto_updater.map(|auto_update| auto_update.read(cx).status()) {
                    Some(AutoUpdateStatus::Updated { .. }) => {
                        "Please restart Neopilot to Collaborate"
                    }
                    Some(AutoUpdateStatus::Installing)
                    | Some(AutoUpdateStatus::Downloading)
                    | Some(AutoUpdateStatus::Checking) => "Updating...",
                    Some(AutoUpdateStatus::Idle) | Some(AutoUpdateStatus::Errored) | None => {
                        "Please update Neopilot to Collaborate"
                    }
                };
                    ...
                    ContextMenu::build(window, cx, |menu, _, _cx| {
                        menu.link(
                            format!(
                                ...
                                match plan {
                                    None => "None",
                                    Some(proto::Plan::Free) => "Neopilot Free",
                                    Some(proto::Plan::NeopilotPro) => "Neopilot Pro",
                                    Some(proto::Plan::NeopilotProTrial) => "Neopilot Pro (Trial)",
                                }
                            ),
                            neopilot_actions::OpenAccountSettings.boxed_clone(),
                        )
                        .separator()
                        .action("Settings", neopilot_actions::OpenSettings.boxed_clone())
                        .action("Key Bindings", Box::new(neopilot_actions::OpenKeymap))
                        .action(
                            "Themes…",
                            neopilot_actions::theme_selector::Toggle::default().boxed_clone(),
                        )
                        .action(
                            "Icon Themes…",
                            neopilot_actions::icon_theme_selector::Toggle::default().boxed_clone(),
                        )
                        .action(
                            "Extensions",
                            neopilot_actions::Extensions::default().boxed_clone(),
                        )
                        .separator()
            ...
            PopoverMenu::new("user-menu")
                ...
                .menu(|window, cx| {
                    ContextMenu::build(window, cx, |menu, _, _| {
                        menu.action("Settings", neopilot_actions::OpenSettings.boxed_clone())
                            .action("Key Bindings", Box::new(neopilot_actions::OpenKeymap))
                            .action(
                                "Themes…",
                                neopilot_actions::theme_selector::Toggle::default().boxed_clone(),
                            ...
                            .action(
                                "Icon Themes…",
                                neopilot_actions::icon_theme_selector::Toggle::default()
                                    .boxed_clone(),
                            )
                            .action(
                                "Extensions",
