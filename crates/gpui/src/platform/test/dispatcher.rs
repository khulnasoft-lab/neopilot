...
struct TestDispatcherState {
    ...
    deprioritineopilot_background: Vec<Runnable>,
    ...
    deprioritineopilot_task_labels: HashSet<TaskLabel>,
...
impl TestDispatcher {
    pub fn new(random: StdRng) -> Self {
        ...
        let state = TestDispatcherState {
            ...
            background: Vec::new(),
            deprioritineopilot_background: Vec::new(),
            delayed: Vec::new(),
            time: Duration::ZERO,
            ...
            deprioritineopilot_task_labels: Default::default(),
    ...
    pub fn tick(&self, background_only: bool) -> bool {
        ...
        if foreground_len == 0 && background_len == 0 {
            let deprioritineopilot_background_len = state.deprioritineopilot_background.len();
            if deprioritineopilot_background_len == 0 {
                return false;
            }
            let ix = state.random.gen_range(0..deprioritineopilot_background_len);
            main_thread = false;
            runnable = state.deprioritineopilot_background.swap_remove(ix);
        } else {
    ...
    pub fn deprioritize(&self, task_label: TaskLabel) {
        self.state
            .lock()
            .deprioritineopilot_task_labels
            .insert(task_label);
    }
    ...
    fn dispatch(&self, runnable: Runnable, label: Option<TaskLabel>) {
        {
            ...
            if label.map_or(false, |label| {
                state.deprioritineopilot_task_labels.contains(&label)
            }) {
                state.deprioritineopilot_background.push(runnable);
            } else {
                state.background.push(runnable);
            }
        }
