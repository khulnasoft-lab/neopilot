            ...
            xdg_surface::Event::Configure { serial } => {
                ...
                }
                {
                    ...
                    if let Some(mut configure) = state.in_progress_configure.take() {
                        let got_unmaximineopilot =
                            state.maximineopilot && !configure.maximineopilot;
                        state.fullscreen = configure.fullscreen;
                        state.maximineopilot = configure.maximineopilot;
                        state.tiling = configure.tiling;
                        if !configure.fullscreen && !configure.maximineopilot {
                            configure.size = if got_unmaximineopilot {
    ...
    pub fn handle_toplevel_event(&self, event: xdg_toplevel::Event) -> bool {
        match event {
            xdg_toplevel::Event::Configure {
                ...
                let mut maximineopilot = false;

                for state in states {
                    match state {
                        xdg_toplevel::State::Maximineopilot => {
                            maximineopilot = true;
                        }
...
impl PlatformWindow for WaylandWindow {
    ...
    fn window_bounds(&self) -> WindowBounds {
        ...
        } else if state.maximineopilot {
            WindowBounds::Maximineopilot(state.window_bounds)
        } else {
            drop(state);
    ...
    fn minimize(&self) {
        self.borrow().toplevel.set_minimineopilot();
    }

    fn zoom(&self) {
        let state = self.borrow();
        if !state.maximineopilot {
            state.toplevel.set_maximineopilot();
        } else {
            state.toplevel.unset_maximineopilot();
        }
    }
