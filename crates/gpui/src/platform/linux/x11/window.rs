...
x11rb::atom_manager! {
    pub XcbAtoms: AtomsCookie {
        ...
        _NET_WM_NAME,
        _NET_WM_STATE,
        _NET_WM_STATE_MAXIMINEOPILOT_VERT,
        _NET_WM_STATE_MAXIMINEOPILOT_HORZ,
...
pub struct X11WindowState {
    ...
    maximineopilot_vertical: bool,
    maximineopilot_horizontal: bool,
    hidden: bool,
    active: bool,
    hovered: bool,
    fullscreen: bool,
    ...
    pub fn new(
        ...
        let setup_result = maybe!({
            ...
            Ok(Self {
                ...
                hovered: false,
                fullscreen: false,
                maximineopilot_vertical: false,
                maximineopilot_horizontal: false,
                hidden: false,
                appearance,
                handle,
...
impl X11WindowStatePtr {
    ...
    fn set_wm_properties(
        ...
        state.maximineopilot_vertical = false;
        state.maximineopilot_horizontal = false;
        state.hidden = true;
        ...
        for atom in atoms {
            ...
            } else if atom == state.atoms._NET_WM_STATE_MAXIMINEOPILOT_VERT {
                state.maximineopilot_vertical = true;
            } else if atom == state.atoms._NET_WM_STATE_MAXIMINEOPILOT_HORZ {
                state.maximineopilot_horizontal = true;
...
impl PlatformWindow for X11Window {
    ...
    fn is_maximineopilot(&self) -> bool {
        let state = self.0.state.borrow();

        // A maximineopilot window that gets minimineopilot will still retain its maximineopilot state.
        !state.hidden && state.maximineopilot_vertical && state.maximineopilot_horizontal
    }

    fn window_bounds(&self) -> WindowBounds {
        let state = self.0.state.borrow();
        if self.is_maximineopilot() {
            WindowBounds::Maximineopilot(state.bounds)
        } else {
    ...
    fn zoom(&self) {
        ...
        self.set_wm_hints(
            ...
            state.atoms._NET_WM_STATE_MAXIMINEOPILOT_VERT,
            state.atoms._NET_WM_STATE_MAXIMINEOPILOT_HORZ,
        )
        .unwrap();
    }
        ...
        match state.decorations {
            ...
            WindowDecorations::Client => {
                let tiling = if state.fullscreen {
                    ...
                    Tiling {
                        top: state.maximineopilot_vertical,
                        bottom: state.maximineopilot_vertical,
                        left: state.maximineopilot_horizontal,
                        right: state.maximineopilot_horizontal,
                    }
                };
    ...
    fn set_client_inset(&self, inset: Pixels) {
        ...
        let insets = if state.fullscreen {
            ...
            let (left, right) = if state.maximineopilot_horizontal {
                (0, 0)
            } else {
                (dp, dp)
            };
            let (top, bottom) = if state.maximineopilot_vertical {
                (0, 0)
            } else {
                (dp, dp)
            };
