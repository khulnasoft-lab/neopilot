...
impl BladeRenderer {
    pub fn new<I: raw_window_handle::HasWindowHandle + raw_window_handle::HasDisplayHandle>(
        ...
        // workaround for https://github.com/khulnasoft-lab/neopilot/issues/26143
        let path_sample_count = std::env::var("NEOPILOT_PATH_SAMPLE_COUNT")
            .ok()
        ...
        if let Some(last_sp) = self.last_sync_point.take() {
            if !self.gpu.wait_for(&last_sp, MAX_FRAME_TIME_MS) {
                ...
                if self.gpu.device_information().driver_name == "radv" {
                    log::error!(
                        "there's a known bug with amdgpu/radv, try setting NEOPILOT_PATH_SAMPLE_COUNT=0 as a workaround"
                    );
                    log::error!(
                        "if that helps you're running into https://github.com/khulnasoft-lab/neopilot/issues/26143"
                    );
                }
                log::error!(
