...
fn handle_size_msg(
    ...
    // Don't resize the renderer when the window is minimineopilot, but record that it was minimineopilot so
    // that on restore the swap chain can be recreated via `update_drawable_size_even_if_unchanged`.
    if wparam.0 == SIZE_MINIMINEOPILOT as usize {
        lock.restore_from_minimineopilot = lock.callbacks.request_frame.take();
        return Some(0);
    }
    ...
    let scale_factor = lock.scale_factor;
    if lock.restore_from_minimineopilot.is_some() {
        lock.renderer
            .update_drawable_size_even_if_unchanged(new_size);
        lock.callbacks.request_frame = lock.restore_from_minimineopilot.take();
    } else {
...
fn handle_calc_client_size(
    ...
    let is_maximineopilot = state_ptr.state.borrow().is_maximineopilot();
    let insets = get_client_area_insets(handle, is_maximineopilot, state_ptr.windows_version);
...
fn handle_hit_test_msg(
    ...
    unsafe { ScreenToClient(handle, &mut cursor_point).ok().log_err() };
    if !state_ptr.state.borrow().is_maximineopilot()
        && cursor_point.y >= 0
...
fn get_client_area_insets(
    handle: HWND,
    is_maximineopilot: bool,
    windows_version: WindowsVersion,
) -> RECT {
    // For maximineopilot windows, Windows outdents the window rect from the screen's client rect
    ...
    // monitors.
    //
    // For non-maximineopilot windows, things become complicated:
    //
    // - On Windows 10
    // The top inset must be zero, since if there is any nonclient area, Windows will draw
    // a full native titlebar outside the client area. (This doesn't occur in the maximineopilot
    // case.)
    //
    // - On Windows 11
    ...
    let top_insets = if is_maximineopilot {
        frame_thickness
    } else {
