...
pub struct WindowsWindowState {
    ...
    pub restore_from_minimineopilot: Option<Box<dyn FnMut(RequestFrameOptions)>>,
...
impl WindowsWindowState {
    fn new(
        ...
        let restore_from_minimineopilot = None;
    ...
    pub(crate) fn is_maximineopilot(&self) -> bool {
    ...
    // Calculate the bounds used for saving and whether the window is maximineopilot.
    fn calculate_window_bounds(&self) -> (Bounds<Pixels>, bool) {
        ...
        (
            ...
            placement.showCmd == SW_SHOWMAXIMINEOPILOT.0 as u32,
        )
    }

    fn window_bounds(&self) -> WindowBounds {
        let (bounds, maximineopilot) = self.calculate_window_bounds();

        if self.is_fullscreen() {
            WindowBounds::Fullscreen(self.fullscreen_restore_bounds)
        } else if maximineopilot {
...
impl WindowsWindowStatePtr {
    ...
    fn set_window_placement(&self) -> Result<()> {
        ...
        match open_status.state {
            WindowOpenState::Maximineopilot => unsafe {
...
impl PlatformWindow for WindowsWindow {
    ...
    fn is_maximineopilot(&self) -> bool {
        self.0.state.borrow().is_maximineopilot()
    }
    ...
    fn zoom(&self) {
        unsafe {
            if IsWindowVisible(self.0.hwnd).as_bool() {
                ...
                status.state = WindowOpenState::Maximineopilot;
            }
        }
    }
...
fn register_wnd_class(icon_handle: HICON) -> PCWSTR {
    const CLASS_NAME: PCWSTR = w!("Neopilot::Window");
