...
impl LineWrapper {
    ...
    pub(crate) fn is_word_char(c: char) -> bool {
        ...
        matches!(c, '-' | '_' | '.' | '\'' | '$' | '%' | '@' | '#' | '^' | '~' | ',') ||
        // Characters that used in URL, e.g. `https://github.com/khulnasoft-lab/neopilot?a=1&b=2` for better wrapping a long URL.
        matches!(c,  '/' | ':' | '?' | '&' | '=') ||
        // `⋯` character is special used in Neopilot, to keep this at the end of the line.
        matches!(c, '⋯')
    }
...
mod tests {
    ...
    fn build_wrapper() -> LineWrapper {
        ...
        let id = cx
            .text_system()
            .font_id(&font("Neopilot Plex Mono"))
            .unwrap();
    ...
    fn test_is_word_char() {
        ...
        // URL case
        assert_word("https://github.com/khulnasoft-lab/neopilot/");
