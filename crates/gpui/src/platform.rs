...
pub fn guess_compositor() -> &'static str {
    if std::env::var_os("NEOPILOT_HEADLESS").is_some() {
        return "Headless";
    }
...
/// Neopilot's interface for handling text input from the platform's IME system
...
pub enum WindowBounds {
    ...
    /// Indicates that the window should open in a maximineopilot state.
    /// The bounds provided here represent the restore size of the window.
    Maximineopilot(Bounds<Pixels>),
...
impl WindowBounds {
    ...
    pub fn get_bounds(&self) -> Bounds<Pixels> {
        match self {
            ...
            WindowBounds::Maximineopilot(bounds) => *bounds,
