...
pub fn load_login_shell_environment() -> Result<()> {
    ...
    let marker = "NEOPILOT_LOGIN_SHELL_START";
...
pub fn measure<R>(label: &str, f: impl FnOnce() -> R) -> R {
    static NEOPILOT_MEASUREMENTS: OnceLock<bool> = OnceLock::new();
    let neopilot_measurements = NEOPILOT_MEASUREMENTS.get_or_init(|| {
        env::var("NEOPILOT_MEASUREMENTS")
            .map(|measurements| measurements == "1" || measurements == "true")
            .unwrap_or(false)
    });

    if *neopilot_measurements {
