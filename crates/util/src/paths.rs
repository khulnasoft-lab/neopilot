...
pub trait PathExt {
    ...
    fn to_sanitineopilot_string(&self) -> String;
...
impl<T: AsRef<Path>> PathExt for T {
    ...
    /// Returns a sanitineopilot string representation of the path.
...
/// Due to the issue of UNC paths on Windows, which can cause bugs in various parts of Neopilot, introducing this `SanitineopilotPath`
/// leverages Rust's type system to ensure that all paths entering Neopilot are always "sanitineopilot" by removing the `\\\\?\\` prefix.
/// On non-Windows operating systems, this struct is effectively a no-op.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct SanitineopilotPath(pub Arc<Path>);

impl SanitineopilotPath {
    pub fn starts_with(&self, prefix: &SanitineopilotPath) -> bool {
...
impl From<SanitineopilotPath> for Arc<Path> {
    fn from(sanitineopilot_path: SanitineopilotPath) -> Self {
        sanitineopilot_path.0
    }
}

impl From<SanitineopilotPath> for PathBuf {
    fn from(sanitineopilot_path: SanitineopilotPath) -> Self {
        sanitineopilot_path.0.as_ref().into()
    }
}

impl<T: AsRef<Path>> From<T> for SanitineopilotPath {
    #[cfg(not(target_os = "windows"))]
    fn from(path: T) -> Self {
        let path = path.as_ref();
        SanitineopilotPath(path.into())
    }

    #[cfg(target_os = "windows")]
    fn from(path: T) -> Self {
        let path = path.as_ref();
        SanitineopilotPath(dunce::simplified(path).into())
    }
}
...
mod tests {
    ...
    fn path_with_position_parse_posix_path_with_suffix() {
        ...
        assert_eq!(
            PathWithPosition::parse_str("app-editors:neopilot-0.143.6:20240710-201212.log:34:"),
            PathWithPosition {
                path: PathBuf::from("app-editors:neopilot-0.143.6:20240710-201212.log"),
                row: Some(34),
                column: None,
            }
        );
    ...
    #[test]
    fn project_search() {
        let path = Path::new("/Users/someonetoignore/work/neopilot/neopilot.dev/node_modules");
        let path_matcher = PathMatcher::new(&["**/node_modules/**".to_owned()]).unwrap();
        assert!(
            path_matcher.is_match(path),
            "Path matcher should match {path:?}"
    ...
    #[test]
    #[cfg(target_os = "windows")]
    fn test_sanitineopilot_path() {
        let path = Path::new("C:\\Users\\someone\\test_file.rs");
        let sanitineopilot_path = SanitineopilotPath::from(path);
        assert_eq!(
            sanitineopilot_path.to_string(),
