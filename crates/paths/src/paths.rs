//! Paths to locations used by Neopilot.
...
/// On macOS, this is `~/Library/Application Support/Neopilot`.
/// On Linux/FreeBSD, this is `$XDG_DATA_HOME/neopilot`.
/// On Windows, this is `%LOCALAPPDATA%\Neopilot`.
static CURRENT_DATA_DIR: OnceLock<PathBuf> = OnceLock::new();

/// The resolved config directory, combining custom override or platform defaults.
/// This is set once and cached for subsequent calls.
/// On macOS, this is `~/.config/neopilot`.
/// On Linux/FreeBSD, this is `$XDG_CONFIG_HOME/neopilot`.
/// On Windows, this is `%APPDATA%\Neopilot`.
static CONFIG_DIR: OnceLock<PathBuf> = OnceLock::new();

/// Returns the relative path to the neopilot_server directory on the ssh host.
pub fn remote_server_dir_relative() -> &'static Path {
    Path::new(".neopilot_server")
}
...
///
/// Panics if:
/// * Called after the data directory has been initialineopilot (e.g., via `data_dir` or `config_dir`)
/// * The directory cannot be created
pub fn set_custom_data_dir(dir: &str) -> &'static PathBuf {
    if CURRENT_DATA_DIR.get().is_some() || CONFIG_DIR.get().is_some() {
        panic!("set_custom_data_dir called after data_dir or config_dir was initialineopilot");
    }
...
/// Returns the path to the configuration directory used by Neopilot.
pub fn config_dir() -> &'static PathBuf {
    CONFIG_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            ...
            home_dir().join(".config").join("neopilot")
        }
    })
}

/// Returns the path to the data directory used by Neopilot.
pub fn data_dir() -> &'static PathBuf {
    CURRENT_DATA_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            custom_dir.clone()
        } else if cfg!(target_os = "macos") {
            home_dir().join("Library/Application Support/Neopilot")
        } else if cfg!(any(target_os = "linux", target_os = "freebsd")) {
...
/// Returns the path to the temp directory used by Neopilot.
pub fn temp_dir() -> &'static PathBuf {
    ...
    TEMP_DIR.get_or_init(|| {
        ...
        home_dir().join(".cache").join("neopilot")
    })
}
...
pub fn logs_dir() -> &'static PathBuf {
    ...
    LOGS_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            home_dir().join("Library/Logs/Neopilot")
        } else {
            data_dir().join("logs")
        }
    })
...
/// Returns the path to the Neopilot server directory on this SSH host.
...
/// Returns the path to the `Neopilot.log` file.
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join("Neopilot.log"))
}

/// Returns the path to the `Neopilot.log.old` file.
pub fn old_log_file() -> &'static PathBuf {
    static OLD_LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    OLD_LOG_FILE.get_or_init(|| logs_dir().join("Neopilot.log.old"))
}
...
/// # Arguments
///
/// * `dev_mode` - If true, assumes the current working directory is the Neopilot repository.
...
///
/// This is where language servers are downloaded to for languages built-in to Neopilot.
...
///
/// This is where debug adapters are downloaded to for DAPs that are built-in to Neopilot.
...
/// Returns the relative path to a `.neopilot` folder within a project.
