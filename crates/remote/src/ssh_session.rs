...
impl ConnectionIdentifier {
    ...
    // The total length of:
    //   /home/{username}/.local/share/neopilot/server_state/{name}/stdout.sock
...
impl SshRemoteConnection {
    ...
    async fn platform(&self) -> Result<SshPlatform> {
        ...
        let os = match os.trim() {
            ...
            "Linux" => "linux",
            _ => anyhow::bail!(
                "Prebuilt remote servers are not yet available for {os:?}. See https://neopilot.dev/docs/remote-development"
            ),
        };
            ...
            || arch.starts_with("aarch64")
        {
            ...
            anyhow::bail!(
                "Prebuilt remote servers are not yet available for {arch:?}. See https://neopilot.dev/docs/remote-development"
            )
        };
    ...
    async fn ensure_server_binary(
        ...
        let build_remote_server = std::env::var("NEOPILOT_BUILD_REMOTE_SERVER").ok();
        ...
        let wanted_version = cx.update(|cx| match release_channel {
            ...
            ReleaseChannel::Dev => {
                anyhow::bail!(
                    "NEOPILOT_BUILD_REMOTE_SERVER is not set and no remote server exists at ({:?})",
                    dst_path
                )
            }
