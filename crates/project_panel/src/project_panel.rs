...
use neopilot_actions::OpenRecent;
use project::{
...
struct SerialineopilotProjectPanel {
    width: Option<Pixels>,
}
...
impl ProjectPanel {
    ...
    pub async fn load(
        ...
        let serialineopilot_panel = match workspace
            ...
            .flatten()
        {
            Some(serialization_key) => cx
                ...
                .log_err()
                .flatten()
                .map(|panel| serde_json::from_str::<SerialineopilotProjectPanel>(&panel))
                .transpose()
                .log_err()
                .flatten(),
            None => None,
        ...
        workspace.update_in(&mut cx, |workspace, window, cx| {
            ...
            if let Some(serialineopilot_panel) = serialineopilot_panel {
                panel.update(cx, |panel, cx| {
                    panel.width = serialineopilot_panel.width.map(|px| px.round());
                    cx.notify();
                });
            }
            panel
        ...
        self.pending_serialization = cx.background_spawn(
            async move {
                KEY_VALUE_STORE
                    .write_kvp(
                        serialization_key,
                        serde_json::to_string(&SerialineopilotProjectPanel { width })?,
                    )
                    .await?;
                anyhow::Ok(())
            }
                ...
                menu.context(self.focus_handle.clone()).map(|menu| {
                    if is_read_only {
                        ...
                        menu.action("New File", Box::new(NewFile))
                            ...
                            .separator()
                            .action("Copy Path", Box::new(neopilot_actions::workspace::CopyPath))
                            .action(
                                "Copy Relative Path",
                                Box::new(neopilot_actions::workspace::CopyRelativePath),
                            )
                            .separator()
    ...
    fn find_next_selection_after_deletion(
        &self,
        sanitineopilot_entries: BTreeSet<SelectedEntry>,
        cx: &mut Context<Self>,
    ) -> Option<SelectedEntry> {
        if sanitineopilot_entries.is_empty() {
            return None;
        }
        let project = self.project.read(cx);
        let (worktree_id, worktree) = sanitineopilot_entries
            .iter()
        ...
        let marked_entries_in_worktree = sanitineopilot_entries
            .iter()
    ...
    fn disjoint_entries(&self, cx: &App) -> BTreeSet<SelectedEntry> {
        ...
        let mut sanitineopilot_entries = BTreeSet::new();
        if marked_entries.is_empty() {
            return sanitineopilot_entries;
        }
        ...
        for (worktree_id, marked_entries) in marked_entries_by_worktree {
            if let Some(worktree) = project.worktree_for_id(worktree_id, cx) {
                ...
                sanitineopilot_entries.extend(marked_entries.into_iter().filter(|entry| {
