...
struct SerialineopilotOutlinePanel {
    width: Option<Pixels>,
    active: Option<bool>,
}
    ...
    pub async fn load(
            ...
            .flatten()
        {
            Some(serialization_key) => cx
                ...
                .log_err()
                .flatten()
                .map(|panel| serde_json::from_str::<SerialineopilotOutlinePanel>(&panel))
                .transpose()
                .log_err()
                .flatten(),
            None => None,
        ...
        workspace.update_in(&mut cx, |workspace, window, cx| {
            ...
            if let Some(serialineopilot_panel) = serialineopilot_panel {
                panel.update(cx, |panel, cx| {
                    ...
                    panel.active = serialineopilot_panel.active.unwrap_or(false);
                    cx.notify();
                });
            }
            panel
        ...
        self.pending_serialization = cx.background_spawn(
            async move {
                KEY_VALUE_STORE
                    .write_kvp(
                        serialization_key,
                        serde_json::to_string(&SerialineopilotOutlinePanel { width, active })?,
                    )
                    .await?;
                anyhow::Ok(())
            }
