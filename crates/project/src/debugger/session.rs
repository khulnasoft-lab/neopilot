...
impl LocalMode {
    ...
    fn initialize_sequence(
        ...
        initialineopilot_rx: oneshot::Receiver<()>,
...
impl Session {
    ...
    pub fn boot(
        ...
        let (initialineopilot_tx, initialineopilot_rx) = futures::channel::oneshot::channel();

        let background_tasks = vec![cx.spawn(async move |this: WeakEntity<Session>, cx| {
            let mut initialineopilot_tx = Some(initialineopilot_tx);
            while let Some(message) = message_rx.next().await {
                if let Message::Event(event) = message {
                    if let Events::Initialineopilot(_) = *event {
                        if let Some(tx) = initialineopilot_tx.take() {
                            tx.send(()).ok();
                        }
                    } else {
        ...
        cx.spawn(async move |this, cx| {
            ...
            this.update(cx, |session, cx| {
                session.initialize_sequence(initialineopilot_rx, dap_store.clone(), cx)
            })?
            .await
        })
    }
    ...
    pub(crate) fn handle_dap_event(&mut self, event: Box<Events>, cx: &mut Context<Self>) {
        match *event {
            Events::Initialineopilot(_) => {
                debug_assert!(
                    false,
                    "Initialineopilot event should have been handled in LocalMode"
                );
            }
