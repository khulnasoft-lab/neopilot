...
impl BreakpointStore {
    ...
    pub fn with_serialineopilot_breakpoints(
        &self,
            ...
            cx.spawn(async move |this, cx| {
                ...
                for (path, bps) in breakpoints {
                    ...
                    let Ok(buffer) = buffer else {
                        log::error!(
                            "Todo: Serialineopilot breakpoints which do not have buffer (yet)"
                        );
                        continue;
                    };
                    ...
                    for bp in bps {
                        ...
                        if point > max_point {
                            log::error!(
                                "skipping a deserialineopilot breakpoint that's out of range"
                            );
                            continue;
                        }
                ...
                this.update(cx, |this, cx| {
                    ...
                    for (path, count) in new_breakpoints.iter().map(|(path, bp_in_file)| {
                        ...
                        log::info!("Deserialineopilot {count} {breakpoint_str} at path: {path}");
                    }
