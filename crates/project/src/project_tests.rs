...
async fn test_editorconfig_support(cx: &mut gpui::TestAppContext) {
    ...
    cx.update(|cx| {
        ...
        // .editorconfig overrides .neopilot/settings
...
async fn test_managing_project_specific_settings(cx: &mut gpui::TestAppContext) {
    ...
    let topmost_local_task_source_kind = TaskSourceKind::Worktree {
        id: worktree_id,
        directory_in_worktree: PathBuf::from(".neopilot"),
        id_base: "local worktree tasks from directory \".neopilot\"".into(),
    };
        ...
        vec![
            (
                TaskSourceKind::Worktree {
                    id: worktree_id,
                    directory_in_worktree: PathBuf::from(separator!("b/.neopilot")),
                    id_base: if cfg!(windows) {
                        "local worktree tasks from directory \"b\\\\.neopilot\"".into()
                    } else {
                        "local worktree tasks from directory \"b/.neopilot\"".into()
                    },
                },
...
async fn test_fallback_to_single_worktree_tasks(cx: &mut gpui::TestAppContext) {
        ...
        json!({
            ".neopilot": {
                "tasks.json": r#"[{
                    ...
                    "command": "echo $NEOPILOT_WORKTREE_ROOT"
                }]"#,
            },
            "a": {
                "a.rs": "fn a() {\n    A\n}"
    ...
    assert!(
        ...
        "A task can not be resolved with context with no NEOPILOT_WORKTREE_ROOT data"
    );
    ...
    assert_eq!(
        ...
        vec![(
            TaskSourceKind::Worktree {
                id: worktree_id,
                directory_in_worktree: PathBuf::from(separator!(".neopilot")),
                id_base: if cfg!(windows) {
                    "local worktree tasks from directory \".neopilot\"".into()
                } else {
                    "local worktree tasks from directory \".neopilot\"".into()
                },
            },
...
async fn test_multiple_language_server_hovers(cx: &mut gpui::TestAppContext) {
    ...
    for i in 0..language_server_names.len() {
        ...
        assert!(
            ...
            "Unexpected: initialineopilot server with the same name twice. Name: `{new_server_name}`"
        );
...
fn git_commit(msg: &'static str, repo: &git2::Repository) {
    ...
    let signature = Signature::now("test", "test@neopilot.dev").unwrap();
...
async fn test_find_project_path_abs(
    ...
    // Make sure the worktrees are fully initialineopilot
