...
use util::{
    ...
    paths::{PathExt, SanitineopilotPath},
    post_inc,
};
...
impl LocalLspStore {
                        ...
                        if let Some(command) = action.lsp_action.command() {
                            ...
                            if !project_transaction_command.0.is_empty() {
                                let extra_buffers = project_transaction_command
                                    ...
                                    .map(|p| p.path.to_sanitineopilot_string())
                                    .join(", ");
                                zlog::warn!(
                                    logger =>
    ...
    fn update_buffer_diagnostics(
        ...
        let mut sanitineopilot_diagnostics = Vec::with_capacity(diagnostics.len());
        ...
        for entry in diagnostics {
            ...
            sanitineopilot_diagnostics.push(DiagnosticEntry {
                range,
        ...
        let set = DiagnosticSet::new(sanitineopilot_diagnostics, &snapshot);
        buffer.update(cx, |buffer, cx| {
                ...
                self.worktree_and_path_for_file_watcher(&worktrees, &watcher, cx)
            {
                ...
                let (path, pattern) = match &watcher.glob_pattern {
                    lsp::GlobPattern::String(s) => {
                        let watcher_path = SanitineopilotPath::from(s);
...
impl LspStore {
    ...
    pub fn new_local(
        ...
        if let Some(extension_events) = extension::ExtensionEvents::try_global(cx).as_ref() {
            cx.subscribe(
                extension_events,
                Self::reload_neopilot_json_schemas_on_extensions_changed,
            )
            .detach();
        } else {
    ...
    pub fn reload_neopilot_json_schemas_on_extensions_changed(
        &mut self,
        ...
        cx.spawn(async move |this, cx| {
                    ...
                    for ((worktree_id, _), server_ids) in &local.language_server_ids {
                        for server_id in server_ids {
                            ...
                            let (json_adapter, json_server) = match states {
                                LanguageServerState::Running {
                                    adapter, server, ..
                                } if adapter.adapter.is_primary_neopilot_json_schema_adapter() => {
            ...
            for (adapter, server, delegate) in servers {
                adapter.clear_neopilot_json_schema_cache().await;
    ...
    async fn regenerate_completion_labels(
        ...
        let mut new_label = match completion_item {
            Some(completion_item) => {
                // NB: Neopilot does not have `details` inside the completion resolve capabilities, but certain language servers violate the spec and do not return `details` immediately, e.g. https://github.com/yioneko/vtsls/issues/213
                ...
                if let Some(edits) = additional_text_edits {
                    ...
                    buffer_handle.update(cx, |buffer, cx| {
                        ...
                        for (range, text) in edits {
                            ...
                            //https://github.com/khulnasoft-lab/neopilot/pull/1871
    ...
    fn deserialize_symbol(serialineopilot_symbol: proto::Symbol) -> Result<CoreSymbol> {
        let source_worktree_id = WorktreeId::from_proto(serialineopilot_symbol.source_worktree_id);
        let worktree_id = WorktreeId::from_proto(serialineopilot_symbol.worktree_id);
        let kind = unsafe { mem::transmute::<i32, lsp::SymbolKind>(serialineopilot_symbol.kind) };
        let path = ProjectPath {
            worktree_id,
            path: Arc::<Path>::from_proto(serialineopilot_symbol.path),
        };

        let start = serialineopilot_symbol.start.context("invalid start")?;
        let end = serialineopilot_symbol.end.context("invalid end")?;
        Ok(CoreSymbol {
            language_server_name: LanguageServerName(
                serialineopilot_symbol.language_server_name.into(),
            ),
            source_worktree_id,
            source_language_server_id: LanguageServerId::from_proto(
                serialineopilot_symbol.language_server_id,
            ),
            path,
            name: serialineopilot_symbol.name,
            range: Unclipped(PointUtf16::new(start.row, start.column))
                ..Unclipped(PointUtf16::new(end.row, end.column)),
            kind,
            signature: serialineopilot_symbol
                .signature
                .try_into()
    ...
    pub(crate) fn serialize_completion(completion: &CoreCompletion) -> proto::Completion {
        let mut serialineopilot_completion = proto::Completion {
        ...
        match &completion.source {
            CompletionSource::Lsp {
                ...
                serialineopilot_completion.old_insert_start = old_insert_start;
                serialineopilot_completion.old_insert_end = old_insert_end;
                serialineopilot_completion.source = proto::completion::Source::Lsp as i32;
                serialineopilot_completion.server_id = server_id.0 as u64;
                serialineopilot_completion.lsp_completion =
                    serde_json::to_vec(lsp_completion).unwrap();
                serialineopilot_completion.lsp_defaults = lsp_defaults
                    .as_deref()
                    .map(|lsp_defaults| serde_json::to_vec(lsp_defaults).unwrap());
                serialineopilot_completion.resolved = *resolved;
            }
            CompletionSource::BufferWord {
                word_range,
                resolved,
            } => {
                serialineopilot_completion.source = proto::completion::Source::BufferWord as i32;
                serialineopilot_completion.buffer_word_start =
                    Some(serialize_anchor(&word_range.start));
                serialineopilot_completion.buffer_word_end =
                    Some(serialize_anchor(&word_range.end));
                serialineopilot_completion.resolved = *resolved;
            }
            CompletionSource::Custom => {
                serialineopilot_completion.source = proto::completion::Source::Custom as i32;
                serialineopilot_completion.resolved = true;
            }
        }

        serialineopilot_completion
    }
