...
pub enum TaskSourceKind {
    ...
    UserInput,
    /// Tasks from the worktree's .neopilot/task.json
    Worktree {
        id: WorktreeId,
        directory_in_worktree: PathBuf,
        id_base: Cow<'static, str>,
    },
    /// ~/.config/neopilot/task.json - like global files with task definitions, applicable to any path
    AbsPath {
        id_base: Cow<'static, str>,
        abs_path: PathBuf,
    },
...
impl ContextProvider for BasicContextProvider {
    fn build_context(
        ...
        let current_file = buffer
            ...
            .map(|file| file.abs_path(cx).to_sanitineopilot_string());
        ...
        if let Some(worktree_path) = worktree_root_dir {
            task_variables.insert(
                ...
                worktree_path.to_sanitineopilot_string(),
            );
            if let Some(full_path) = current_file.as_ref() {
                ...
                if let Some(relative_path) = relative_path {
                    task_variables.insert(
                        ...
                        relative_path.to_sanitineopilot_string(),
                    );
                }
            }
        }
