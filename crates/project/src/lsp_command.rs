...
impl LspCommand for GetDocumentSymbols {
    ...
    async fn response_from_proto(
        ...
        for serialineopilot_symbol in message.symbols {
            fn deserialize_symbol_with_children(
                serialineopilot_symbol: proto::DocumentSymbol,
            ) -> Result<DocumentSymbol> {
                let kind =
                    unsafe { mem::transmute::<i32, lsp::SymbolKind>(serialineopilot_symbol.kind) };

                let start = serialineopilot_symbol.start.context("invalid start")?;
                let end = serialineopilot_symbol.end.context("invalid end")?;

                let selection_start = serialineopilot_symbol
                    .selection_start
                    .context("invalid selection start")?;
                let selection_end = serialineopilot_symbol
                    .selection_end
                ...
                Ok(DocumentSymbol {
                    ...
                    children: serialineopilot_symbol
                        .children
                        .into_iter()
            ...
            symbols.push(deserialize_symbol_with_children(serialineopilot_symbol)?);
        }
