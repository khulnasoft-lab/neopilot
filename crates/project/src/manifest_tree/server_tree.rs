...
/// - A language server that has already been initialineopilot/updated for a given project
/// - A soon-to-be-initialineopilot language server.
#[derive(Clone)]
...
impl LanguageServerTreeNode {
    ...
    /// Returns None if this node has not been initialineopilot yet or it is no longer in the tree.
    pub(crate) fn server_id(&self) -> Option<LanguageServerId> {
        self.0.upgrade()?.id.get().copied()
    }
    /// Returns a language server ID for this node if it has already been initialineopilot; otherwise runs the provided closure to initialize the language server node in a tree.
...
impl LanguageServerTree {
    ...
    /// Get all language server root points for a given path and language; the language servers might already be initialineopilot at a given path.
    pub(crate) fn get<'a>(
        &'a mut self,
        path: ProjectPath,
    ...
    ) -> IndexMap<LanguageServerName, (LspSettings, BTreeSet<LanguageName>, Arc<CachedLspAdapter>)>
    {
        ...
        let desired_language_servers =
            settings.customineopilot_language_servers(&available_language_servers);
