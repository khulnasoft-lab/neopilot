...
pub struct NodeBinarySettings {
    ...
    /// The path to the npm binary Neopilot should use (defaults to `.path/../npm`).
    pub npm_path: Option<String>,
    /// If enabled, Neopilot will download its own copy of Node.
    #[serde(default)]
...
pub struct LspSettings {
    ...
    /// this setting can be used to enable or disable them in Neopilot.
    /// Default: true
    #[serde(default = "default_true")]
...
/// SettingsObserver observers changes to .neopilot/{settings, task}.json files in local worktrees
/// (or the equivalent protobuf messages from upstream) and updates local settings
/// and sends notifications downstream.
/// In ssh mode it also monitors ~/.config/neopilot/{settings, task}.json and sends the content
/// upstream.
                        ...
                        Some(
                            async move {
                                ...
                                if abs_path.ends_with(local_vscode_tasks_file_relative_path()) {
                                    ...
                                    let neopilot_tasks = TaskTemplates::try_from(vscode_tasks)
                                        .with_context(|| {
                                            format!(
                                        "converting VSCode tasks into Neopilot ones, file {abs_path:?}"
                                    )
                                        })?;
                                    serde_json::to_string(&neopilot_tasks).with_context(|| {
                                        format!(
                                            "serializing Neopilot tasks into JSON, file {abs_path:?}"
                                        )
                                    })
                                    ...
                                    let neopilot_tasks = DebugTaskFile::try_from(vscode_tasks)
                                        .with_context(|| {
                                            format!(
                                        "converting VSCode debug tasks into Neopilot ones, file {abs_path:?}"
                                    )
                                        })?;
                                    serde_json::to_string(&neopilot_tasks).with_context(|| {
                                        format!(
