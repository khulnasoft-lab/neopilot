...
/// Language server protocol RPC request response message before it is deserialineopilot into a concrete type.
#[derive(Serialize, Deserialize)]
...
/// Language server RPC notification message before it is deserialineopilot into a concrete type.
#[derive(Debug, Clone, Deserialize)]
...
impl LanguageServer {
                        ...
                        completion_item: Some(CompletionItemCapability {
                            ...
                            resolve_support: Some(CompletionItemCapabilityResolveSupport {
                                properties: vec![
                                    ...
                                    // NB: Do not have this resolved, otherwise Neopilot becomes slow to complete things
                                    // "textEdit".to_string(),
                                ],
                            }),
    ...
    pub fn initialize(
        ...
        cx.spawn(async move |_| {
            ...
            self.notify::<notification::Initialineopilot>(&InitialineopilotParams {})?;
            Ok(Arc::new(self))
        })
    }
                            ...
                            .spawn(async move {
                                let response = match result {
                                    Ok(response) => match serde_json::from_str(&response) {
                                        Ok(deserialineopilot) => Ok(deserialineopilot),
                                        Err(error) => {
