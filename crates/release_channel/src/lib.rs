//! Provides constructs for the Neopilot app version and release channel.
...
pub static RELEASE_CHANNEL_NAME: LazyLock<String> = LazyLock::new(|| {
    if cfg!(debug_assertions) {
        env::var("NEOPILOT_RELEASE_CHANNEL").unwrap_or_else(|_| {
...
pub fn app_identifier() -> &'static str {
    match *RELEASE_CHANNEL {
        ReleaseChannel::Dev => "Neopilot-Editor-Dev",
        ReleaseChannel::Nightly => "Neopilot-Editor-Nightly",
        ReleaseChannel::Preview => "Neopilot-Editor-Preview",
        ReleaseChannel::Stable => "Neopilot-Editor-Stable",
    }
}

/// The Git commit SHA that Neopilot was built at.
...
/// The version of Neopilot.
pub struct AppVersion;

impl AppVersion {
    /// Load the app version from env.
    pub fn load(pkg_version: &str) -> SemanticVersion {
        if let Ok(from_env) = env::var("NEOPILOT_APP_VERSION") {
            from_env.parse().expect("invalid NEOPILOT_APP_VERSION")
        } else {
...
/// A Neopilot release channel.
#[derive(Debug, Copy, Clone, PartialEq, Eq, Default)]
pub enum ReleaseChannel {
    /// The development release channel.
    ///
    /// Used for local debug builds of Neopilot.
    #[default]
    Dev,
...
impl ReleaseChannel {
    ...
    pub fn display_name(&self) -> &'static str {
        match self {
            ReleaseChannel::Dev => "Neopilot Dev",
            ReleaseChannel::Nightly => "Neopilot Nightly",
            ReleaseChannel::Preview => "Neopilot Preview",
            ReleaseChannel::Stable => "Neopilot",
        }
    }
    ...
    /// and WM_CLASS on X11.
    /// This also has to match the bundle identifier for Neopilot on macOS.
    pub fn app_id(&self) -> &'static str {
        match self {
            ReleaseChannel::Dev => "dev.neopilot.Neopilot-Dev",
            ReleaseChannel::Nightly => "dev.neopilot.Neopilot-Nightly",
            ReleaseChannel::Preview => "dev.neopilot.Neopilot-Preview",
            ReleaseChannel::Stable => "dev.neopilot.Neopilot",
        }
    }
