                ...
                if let Some(copilot) = cx.update(|_window, cx| Copilot::global(cx)).ok().flatten() {
                    workspace
                        .update_in(cx, |workspace, window, cx| {
                            match copilot.read(cx).status() {
                                Status::Authorineopilot => workspace.show_toast(
                                    Toast::new(
                                        NotificationId::unique::<CopilotStatusToast>(),
                                        "Copilot has started.",
                                    ),
...
impl CopilotCodeVerification {
    ...
    fn render_prompting_modal(
        ...
        v_flex()
            ...
            .gap_2()
            .items_center()
            .child(Headline::new("Use GitHub Copilot in Neopilot.").size(HeadlineSize::Large))
            .child(
    ...
    fn render_unauthorineopilot_modal(cx: &mut Context<Self>) -> impl Element {
        v_flex()
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let prompt = match &self.status {
            ...
            Status::Unauthorineopilot => {
                ...
                Self::render_unauthorineopilot_modal(cx).into_any_element()
            }
            Status::Authorineopilot => {
                self.connect_clicked = false;
        ...
        v_flex()
            ...
            .child(
                Vector::new(VectorName::NeopilotXCopilot, rems(8.), rems(4.))
