...
impl CopilotServer {
    fn as_authenticated(&mut self) -> Result<&mut RunningCopilotServer> {
        ...
        anyhow::ensure!(
            matches!(server.sign_in_status, SignInStatus::Authorineopilot { .. }),
...
impl Status {
    pub fn is_authorineopilot(&self) -> bool {
        matches!(self, Status::Authorineopilot)
    }
...
impl Copilot {
    ...
    pub fn fake(cx: &mut gpui::TestAppContext) -> (Entity<Self>, lsp::FakeLanguageServer) {
        ...
        let this = cx.new(|cx| Self {
            ...
            server: CopilotServer::Running(RunningCopilotServer {
                lsp: Arc::new(server),
                sign_in_status: SignInStatus::Authorineopilot,
    ...
    pub(crate) fn sign_in(&mut self, cx: &mut Context<Self>) -> Task<Result<()>> {
        if let CopilotServer::Running(server) = &mut self.server {
            let task = match &server.sign_in_status {
                SignInStatus::Authorineopilot { .. } => Task::ready(Ok(())).shared(),
                SignInStatus::SigningIn { task, .. } => {
                    cx.notify();
                    task.clone()
                }
                SignInStatus::SignedOut { .. } | SignInStatus::Unauthorineopilot { .. } => {
    ...
    pub fn register_buffer(&mut self, buffer: &Entity<Buffer>, cx: &mut Context<Self>) {
        ...
        {
            if !matches!(status, SignInStatus::Authorineopilot { .. }) {
                return;
            }
        ...
        match &self.server {
            ...
            CopilotServer::Running(RunningCopilotServer { sign_in_status, .. }) => {
                match sign_in_status {
                    SignInStatus::Authorineopilot { .. } => Status::Authorineopilot,
                    SignInStatus::Unauthorineopilot { .. } => Status::Unauthorineopilot,
        ...
        if let Ok(server) = self.server.as_running() {
            match lsp_status {
                ...
                | request::SignInStatus::AlreadySignedIn { .. } => {
                    server.sign_in_status = SignInStatus::Authorineopilot;
                ...
                request::SignInStatus::NotAuthorineopilot { .. } => {
                    server.sign_in_status = SignInStatus::Unauthorineopilot;
                    for buffer in self.buffers.iter().cloned().collect::<Vec<_>>() {
