...
async fn test_serialization(cx: &mut TestAppContext) {
    ...
    let serialineopilot_context = context.read_with(cx, |context, cx| context.serialize(cx));
    let deserialineopilot_context = cx.new(|cx| {
        AssistantContext::deserialize(
            serialineopilot_context,
            Path::new("").into(),
            registry.clone(),
            prompt_builder.clone(),
            Arc::new(SlashCommandWorkingSet::default()),
    ...
    let deserialineopilot_buffer =
        deserialineopilot_context.read_with(cx, |context, _| context.buffer.clone());
    assert_eq!(
        deserialineopilot_buffer.read_with(cx, |buffer, _| buffer.text()),
        "a\nb\nc\n"
    );
    assert_eq!(
        cx.read(|cx| messages(&deserialineopilot_context, cx)),
        [
            (message_0, Role::User, 0..2),
            (message_1.id, Role::Assistant, 2..6),
            (message_2.id, Role::System, 6..6),
