...
use feature_flags::NeopilotProFeatureFlag;
...
const TRY_NEOPILOT_PRO_URL: &str = "https://neopilot.dev/pro";
            ...
            for (provider_id, provider_name, authenticate_task) in authenticate_all_providers {
                if let Err(err) = authenticate_task.await {
                    if matches!(err, AuthenticateError::CredentialsNotFound) {
                        ...
                        // language model selector is initialineopilot.
                        //
...
impl PickerDelegate for LanguageModelPickerDelegate {
    ...
    fn render_footer(
        ...
        let plan = proto::Plan::NeopilotPro;
        ...
        Some(
            h_flex()
                ...
                .gap_4()
                .justify_between()
                .when(cx.has_flag::<NeopilotProFeatureFlag>(), |this| {
                    this.child(match plan {
                        Plan::NeopilotPro => Button::new("neopilot-pro", "Neopilot Pro")
                            .icon(IconName::NeopilotAssistant)
                            .icon_size(IconSize::Small)
                            .icon_color(Color::Muted)
                            .icon_position(IconPosition::Start)
                            .on_click(|_, window, cx| {
                                window.dispatch_action(
                                    Box::new(neopilot_actions::OpenAccountSettings),
                                    cx,
                                )
                            }),
                        Plan::Free | Plan::NeopilotProTrial => Button::new(
                            "try-pro",
                            if plan == Plan::NeopilotProTrial {
                                "Upgrade to Pro"
                            } else {
                                "Try Pro"
                            },
                        )
                        .on_click(|_, _, cx| cx.open_url(TRY_NEOPILOT_PRO_URL)),
                    })
                })
                .child(
                    Button::new("configure", "Configure")
                        ...
                        .on_click(|_, window, cx| {
                            window.dispatch_action(
                                neopilot_actions::agent::OpenConfiguration.boxed_clone(),
                                cx,
                            );
                        }),
                )
...
mod tests {
    ...
    fn test_exact_match(cx: &mut TestAppContext) {
        let models = create_models(vec![
            ("neopilot", "Claude 3.7 Sonnet"),
            ("neopilot", "Claude 3.7 Sonnet Thinking"),
            ("neopilot", "gpt-4.1"),
            ("neopilot", "gpt-4.1-nano"),
    ...
    fn test_fuzzy_match(cx: &mut TestAppContext) {
        ...
        // In the case below, `neopilot/gpt-4.1` and `openai/gpt-4.1` have identical
        // similarity scores, but `neopilot/gpt-4.1` was higher in the models list,
        ...
        assert_models_eq(results, vec!["neopilot/gpt-4.1-nano"]);
    }

    #[gpui::test]
    fn test_exclude_recommended_models(_cx: &mut TestAppContext) {
        let recommended_models = create_models(vec![("neopilot", "claude")]);
        let all_models = create_models(vec![
            ("neopilot", "claude"), // Should be filtered out from "other"
            ("neopilot", "gemini"),
            ("copilot", "o3"),
        ]);
        ...
        assert_models_eq(actual_other_models, vec!["neopilot/gemini", "copilot/o3"]);
    }
}
