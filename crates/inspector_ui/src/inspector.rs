...
pub fn init(app_state: Arc<AppState>, cx: &mut App) {
    cx.on_action(|_: &neopilot_actions::dev::ToggleInspector, cx| {
...
fn render_inspector_id(inspector_id: &InspectorElementId, cx: &App) -> Div {
    ...
    let source_location_string = source_location_string
        .strip_prefix(env!("NEOPILOT_REPO_DIR"))
    ...
    v_flex()
        ...
        .child(
            div()
                ...
                .tooltip(Tooltip::text("Click to open by running neopilot cli"))
                .on_click(move |_, _window, cx| {
                    cx.background_spawn(open_neopilot_source_location(source_location))
...
async fn open_neopilot_source_location(
    location: &'static std::panic::Location<'static>,
) -> anyhow::Result<()> {
    let mut path = Path::new(env!("NEOPILOT_REPO_DIR")).to_path_buf();
    ...
    let output = new_smol_command("neopilot")
        .arg(&path_arg)
        .output()
        .await
        .with_context(|| format!("running neopilot to open {path_arg} failed"))?;

    if !output.status.success() {
        Err(anyhow!(
            "running neopilot to open {path_arg} failed with stderr: {}",
