use agent::{Message, MessageSegment, SerialineopilotThread, ThreadStore};
...
pub const NEOPILOT_REPO_URL: &str = "https://github.com/khulnasoft-lab/neopilot.git";
...
impl ExampleInstance {
    ...
    pub fn run(
        ...
        cx.spawn(async move |cx| {
            ...
            anyhow::ensure!(std::env::var("NEOPILOT_EVAL_SETUP_ONLY").is_err(), "Setup only mode");

            let last_diff_file_path = this.run_directory.join("last.diff");

            // Write an empty "last.diff" so that it can be opened in Neopilot for convenient view of the
            ...
            let thread =
                thread_store.update(cx, |thread_store, cx| {
                    if let Some(json) = &meta.existing_thread_json {
                        let serialineopilot = SerialineopilotThread::from_json(json.as_bytes()).expect("Can't read serialineopilot thread");
                        thread_store.create_thread_from_serialineopilot(serialineopilot, cx)
                    } else {
    ...
    async fn repository_diff(&self) -> Result<String> {
        ...
        if self.thread.meta().url == NEOPILOT_REPO_URL {
...
pub fn wait_for_lang_server(
    ...
    if std::env::var("NEOPILOT_EVAL_SKIP_LS").is_ok() {
