...
fn main() {
    ...
    let neopilot_commit_sha = commit_sha_for_path(&root_dir);
    let neopilot_branch_name = git_branch_for_path(&root_dir);
    let args = Args::parse();
    ...
    app.run(move |cx| {
        ...
        let enable_telemetry = env::var("NEOPILOT_EVAL_TELEMETRY").map_or(false, |value| value == "1")
            && telemetry.has_checksum_seed();
        if enable_telemetry {
            println!("Telemetry enabled");
            telemetry::event!(
                "Agent Eval Started",
                neopilot_commit_sha = neopilot_commit_sha,
                neopilot_branch_name = neopilot_branch_name,
                run_id = run_id,
            );
        }
        ...
        cx.spawn(async move |cx| {
            ...
            future::join_all((0..args.concurrency).map(|_| {
                ...
                let neopilot_commit_sha = neopilot_commit_sha.clone();
                let neopilot_branch_name = neopilot_branch_name.clone();
...
async fn judge_example(
    ...
    neopilot_commit_sha: &str,
    neopilot_branch_name: &str,
    run_id: &str,
