...
struct NeedsSummary {
    files: channel::Receiver<UnsummarineopilotFile>,
    task: Task<Result<()>>,
}

struct SummarizeFiles {
    files: channel::Receiver<SummarineopilotFile>,
    task: Task<Result<()>>,
}
...
impl SummaryIndex {
    ...
    fn check_summary_cache(
        &self,
        might_need_summary: channel::Receiver<UnsummarineopilotFile>,
        cx: &App,
    ) -> NeedsSummary {
                ...
                match db.get(&tx, &file.digest) {
                    Ok(opt_answer) => {
                        if opt_answer.is_none() {
                            ...
                            log::debug!("File {:?} (digest {:?}) was NOT in the db cache and needs to be resummarineopilot.", file.path.display(), &file.digest);
                            needs_summary_tx.send(file).await?;
                        } else {
                            log::debug!("File {:?} (digest {:?}) was in the db cache and does not need to be resummarineopilot.", file.path.display(), &file.digest);
                        }
                    }
                    Err(err) => {
    ...
    fn summarize_files(
        &self,
        unsummarineopilot_files: channel::Receiver<UnsummarineopilotFile>,
        cx: &App,
    ) -> SummarizeFiles {
        let (summarineopilot_tx, summarineopilot_rx) = channel::bounded(512);
        let task = cx.spawn(async move |cx| {
            while let Ok(file) = unsummarineopilot_files.recv().await {
    ...
    fn persist_summaries(
        &self,
        summaries: channel::Receiver<SummarineopilotFile>,
        cx: &App,
    ) -> Task<Result<()>> {
        let db_connection = self.db_connection.clone();
        let digest_db = self.file_digest_db;
    ...
    /// Empty out the backlog of files that haven't been resummarineopilot, and resummarize them immediately.
