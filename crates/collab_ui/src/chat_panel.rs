...
struct SerialineopilotChatPanel {
    width: Option<Pixels>,
}
    ...
    pub fn load(
        ...
        cx.spawn(async move |cx| {
            let serialineopilot_panel = if let Some(panel) = cx
                ...
                .flatten()
            {
                Some(serde_json::from_str::<SerialineopilotChatPanel>(&panel)?)
            } else {
                None
            };
            ...
            workspace.update_in(cx, |workspace, window, cx| {
                ...
                if let Some(serialineopilot_panel) = serialineopilot_panel {
                    panel.update(cx, |panel, cx| {
                        panel.width = serialineopilot_panel.width.map(|r| r.round());
                        cx.notify();
                    });
                }
                panel
        ...
        self.pending_serialization = cx.background_spawn(
            async move {
                KEY_VALUE_STORE
                    .write_kvp(
                        CHAT_PANEL_KEY.into(),
                        serde_json::to_string(&SerialineopilotChatPanel { width })?,
                    )
                    .await?;
                anyhow::Ok(())
            }
                        ...
                        |this| {
                            this.child(
                                h_flex()
                                    ...
                                    .child(
                                        Label::new(time_format::format_localineopilot_timestamp(
                                            message.timestamp,
    ...
    fn render_markdown_with_mentions(
        ...
        if message.edited_at.is_some() {
            ...
            if let Some(edit_timestamp) = message.edited_at {
                let edit_timestamp_text = time_format::format_localineopilot_timestamp(
                    edit_timestamp,
                    OffsetDateTime::now_utc(),
                    local_timezone,
...
mod tests {
    ...
    fn test_render_markdown_with_auto_detect_links(cx: &mut App) {
        ...
        let message = channel::ChannelMessage {
            id: ChannelMessageId::Saved(0),
            body: "Here is a link https://neopilot.dev to neopilots website".to_string(),
            timestamp: OffsetDateTime::now_utc(),
            sender: Arc::new(client::User {
                github_login: "fgh".into(),
                avatar_uri: "avatar_fgh".into(),
        ...
        let (body, ranges) = marked_text_ranges(
            "Here is a link «https://neopilot.dev» to neopilots website",
            false,
        );
        assert_eq!(message.text, body);
