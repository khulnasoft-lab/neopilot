...
struct SerialineopilotNotificationPanel {
    width: Option<Pixels>,
}
...
impl NotificationPanel {
    ...
    pub fn load(
        ...
        cx.spawn(async move |cx| {
            let serialineopilot_panel = if let Some(panel) = cx
                ...
                .flatten()
            {
                Some(serde_json::from_str::<SerialineopilotNotificationPanel>(
                    &panel,
                )?)
            } else {
                None
            ...
            async move {
                KEY_VALUE_STORE
                    .write_kvp(
                        ...
                        serde_json::to_string(&SerialineopilotNotificationPanel { width })?,
                    )
                    .await?;
                anyhow::Ok(())
            }
    ...
    fn render_notification(
        ...
        let relative_timestamp = time_format::format_localineopilot_timestamp(
            timestamp,
            now,
            self.local_timezone,
        ...
        let absolute_timestamp = time_format::format_localineopilot_timestamp(
            timestamp,
            now,
            self.local_timezone,
