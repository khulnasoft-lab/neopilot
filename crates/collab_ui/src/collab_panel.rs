...
struct SerialineopilotCollabPanel {
    width: Option<Pixels>,
    ...
    pub async fn load(
            ...
            .flatten()
        {
            Some(serialization_key) => cx
                ...
                .log_err()
                .flatten()
                .map(|panel| serde_json::from_str::<SerialineopilotCollabPanel>(&panel))
                .transpose()
                .log_err()
                .flatten(),
            None => None,
        ...
        workspace.update_in(&mut cx, |workspace, window, cx| {
            ...
            if let Some(serialineopilot_panel) = serialineopilot_panel {
                panel.update(cx, |panel, cx| {
                    ...
                    panel.collapsed_channels = serialineopilot_panel
                        .collapsed_channels
        ...
        self.pending_serialization = cx.background_spawn(
            async move {
                KEY_VALUE_STORE
                    .write_kvp(
                        serialization_key,
                        serde_json::to_string(&SerialineopilotCollabPanel {
                            width,
                    ...
                    window.handler_for(&this, move |_, window, cx| {
                        ActiveCall::global(cx)
                            ...
                            .detach_and_prompt_err("Failed to grant write access", window, cx, |e, _, _| {
                                match e.error_code() {
                                    ErrorCode::NeedsCla => Some("This user has not yet signed the CLA at https://neopilot.dev/cla.".into()),
                                    _ => None,
                                }
                            })
                    }),
