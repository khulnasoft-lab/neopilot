...
fn test_select_language(cx: &mut App) {
    ...
    assert_eq!(
        registry
            .language_for_file(&file("neopilot/cars"), None, cx)
            .map(|l| l.name()),
        None
    );
    assert_eq!(
        registry
            .language_for_file(&file("neopilot/a.cars"), None, cx)
            .map(|l| l.name()),
        None
    );
    assert_eq!(
        registry
            .language_for_file(&file("neopilot/sumk"), None, cx)
            .map(|l| l.name()),
        None
    );
}
