//! The `language` crate provides a large chunk of Neopilot's language-related
...
pub trait LspAdapter: 'static + Send + Sync {
    ...
    fn get_language_server_command<'a>(
        ...
        async move {
            ...
            //      worktree 3: no gopls found in PATH -> fallback to Neopilot installation
            //
    ...
    /// provide autocompletion and diagnostics in Neopilot setting and keybind
    /// files
    fn is_primary_neopilot_json_schema_adapter(&self) -> bool {
        false
    }

    /// Method only implemented by the default JSON language server adapter.
    /// Used to clear the cache of JSON schemas that are used to provide
    /// autocompletion and diagnostics in Neopilot settings and keybinds files.
    /// Should not be called unless the callee is sure that
    /// `Self::is_primary_neopilot_json_schema_adapter` returns `true`
    async fn clear_neopilot_json_schema_cache(&self) {
        unreachable!(
...
pub struct LanguageConfig {
    ...
    /// A list of characters that Neopilot should treat as word characters for the
    ...
    /// A list of characters that Neopilot should treat as word characters for completion queries.
    #[serde(default)]
...
impl LanguageScope {
    ...
    ///
    /// By default, Neopilot treats alphanumeric characters (and '_') as word characters for
