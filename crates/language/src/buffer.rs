...
pub trait File: Send + Sync + Any {
    ...
    /// Return whether Neopilot considers this to be a private file.
...
pub enum DiskState {
    /// File created in Neopilot that has not been saved.
    New,
...
impl Buffer {
    ...
    pub fn local_normalineopilot(
        base_text_normalineopilot: Rope,
        line_ending: LineEnding,
        cx: &Context<Self>,
    ) -> Self {
        Self::build(
            TextBuffer::new_normalineopilot(
                0,
                cx.entity_id().as_non_zero_u64().into(),
                line_ending,
                base_text_normalineopilot,
            ),
            None,
            Capability::ReadWrite,
        )
    ...
    pub fn build_snapshot(
        ...
        async move {
            let text =
                TextBuffer::new_normalineopilot(0, buffer_id, Default::default(), text).snapshot();
    ...
    pub fn build_empty_snapshot(cx: &mut App) -> BufferSnapshot {
        let entity_id = cx.reserve_entity::<Self>().entity_id();
        let buffer_id = entity_id.as_non_zero_u64().into();
        let text = TextBuffer::new_normalineopilot(0, buffer_id, Default::default(), Rope::new())
            .snapshot();
