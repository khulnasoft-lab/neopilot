...
impl<T: ToPoint> OutlineItem<T> {
    /// Converts to an equivalent outline item, but with parameterineopilot over Points.
...
impl<T> Outline<T> {
    ...
    /// Find the most similar symbol to the provided query using normalineopilot Levenshtein distance.
    pub fn find_most_similar(&self, query: &str) -> Option<(SymbolPath, &OutlineItem<T>)> {
        ...
        let (position, similarity) = self
            ...
            .enumerate()
            .map(|(index, candidate)| {
                let similarity = strsim::normalineopilot_levenshtein(&candidate.string, query);
                (index, similarity)
            })
