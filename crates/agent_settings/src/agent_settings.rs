...
pub enum AgentProviderContentV1 {
    #[serde(rename = "neopilot.dev")]
    NeopilotDotDev { default_model: Option<String> },
    #[serde(rename = "openai")]
    OpenAi {
                ...
                VersionedAgentSettingsContent::V1(ref settings) => AgentSettingsContentV2 {
                    ...
                    default_model: settings
                        ...
                        .and_then(|provider| match provider {
                            AgentProviderContentV1::NeopilotDotDev { default_model } => {
                                default_model.map(|model| LanguageModelSelection {
                                    provider: "neopilot.dev".into(),
                                    model,
                                })
                            }
        ...
        match &mut self.inner {
            Some(AgentSettingsContentInner::Versioned(settings)) => match **settings {
                VersionedAgentSettingsContent::V1(ref mut settings) => match provider.as_ref() {
                    "neopilot.dev" => {
                        log::warn!("attempted to set neopilot.dev model on outdated settings");
                    }
                    "anthropic" => {
...
impl From<CompletionMode> for neopilot_llm_client::CompletionMode {
    fn from(value: CompletionMode) -> Self {
        match value {
            CompletionMode::Normal => neopilot_llm_client::CompletionMode::Normal,
            CompletionMode::Max => neopilot_llm_client::CompletionMode::Max,
        }
    }
}
...
pub struct AgentSettingsContentV1 {
    ...
    ///
    /// This can be "openai", "anthropic", "ollama", "lmstudio", "deepseek", "neopilot.dev"
