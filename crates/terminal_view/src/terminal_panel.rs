...
use crate::{
    ...
    persistence::{
        SerialineopilotItems, SerialineopilotTerminalPanel, deserialize_terminal_panel,
...
use neopilot_actions::assistant::InlineAssist;
                            ...
                            .menu(move |window, cx| {
                                ...
                                let menu = ContextMenu::build(window, cx, |menu, _, _| {
                                    menu.context(focus_handle.clone())
                                        ...
                                        .action(
                                            "Spawn task",
                                            neopilot_actions::Spawn::modal().boxed_clone(),
                                        )
                                });
    ...
    pub async fn load(
            ...
            .flatten()
        {
            Some((database_id, serialization_key)) => {
                if let Some(serialineopilot_panel) = cx
                    .background_spawn(async move { KEY_VALUE_STORE.read_kvp(&serialization_key) })
                    .await
                    .log_err()
                    .flatten()
                    .map(|panel| serde_json::from_str::<SerialineopilotTerminalPanel>(&panel))
                    .transpose()
                    .log_err()
                    .flatten()
                {
                    if let Ok(serialineopilot) = workspace
                        .update_in(&mut cx, |workspace, window, cx| {
                            deserialize_terminal_panel(
                                workspace.weak_handle(),
                                workspace.project().clone(),
                                database_id,
                                serialineopilot_panel,
                                window,
                                cx,
                            )
                        })?
                        .await
                    {
                        terminal_panel = Some(serialineopilot);
                    }
                }
            }
            _ => {}
        ...
        self.pending_serialization = cx.spawn(async move |terminal_panel, cx| {
            ...
            let items = terminal_panel
                .update(cx, |terminal_panel, cx| {
                    SerialineopilotItems::WithSplits(serialize_pane_group(
            ...
            cx.background_spawn(
                async move {
                    KEY_VALUE_STORE
                        .write_kvp(
                            serialization_key,
                            serde_json::to_string(&SerialineopilotTerminalPanel {
                                items,
                                active_item_id: None,
                                height,
                                width,
...
impl Render for InlineAssistTabBarButton {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        IconButton::new("terminal_inline_assistant", IconName::NeopilotAssistant)
            .icon_size(IconSize::Small)
