...
pub(crate) fn serialize_pane_group(
    ...
    build_serialineopilot_pane_group(&pane_group.root, active_pane, cx)
}
...
fn build_serialineopilot_pane_group(
    ...
    match pane_group {
        Member::Axis(PaneAxis {
            ...
            children: members
                .iter()
                .map(|member| build_serialineopilot_pane_group(member, active_pane, cx))
                .collect::<Vec<_>>(),
            flexes: Some(flexes.lock().clone()),
        },
        Member::Pane(pane_handle) => SerialineopilotPaneGroup::Pane(serialize_pane(
            pane_handle,
...
fn serialize_pane(pane: &Entity<Pane>, active: bool, cx: &mut App) -> SerialineopilotPane {
...
pub(crate) fn deserialize_terminal_panel(
    ...
    serialineopilot_panel: SerialineopilotTerminalPanel,
    window: &mut Window,
    cx: &mut App,
    ...
    window.spawn(cx, async move |cx| {
        let terminal_panel = workspace.update_in(cx, |workspace, window, cx| {
            cx.new(|cx| {
                ...
                panel.height = serialineopilot_panel.height.map(|h| h.round());
                panel.width = serialineopilot_panel.width.map(|w| w.round());
                panel
            })
        })?;
        match &serialineopilot_panel.items {
            SerialineopilotItems::NoSplits(item_ids) => {
                ...
                .await;
                let active_item = serialineopilot_panel.active_item_id;
            ...
            SerialineopilotItems::WithSplits(serialineopilot_pane_group) => {
                let center_pane = deserialize_pane_group(
                    workspace,
                    project,
                    terminal_panel.clone(),
                    database_id,
                    serialineopilot_pane_group,
                    cx,
                )
                .await;
...
async fn deserialize_pane_group(
    ...
    serialineopilot: &SerialineopilotPaneGroup,
    cx: &mut AsyncWindowContext,
) -> Option<(Member, Option<Entity<Pane>>)> {
    match serialineopilot {
        SerialineopilotPaneGroup::Group {
            axis,
            flexes,
            children,
        ...
        SerialineopilotPaneGroup::Pane(serialineopilot_pane) => {
            ...
            let new_items = deserialize_terminal_views(
                ...
                project.clone(),
                workspace.clone(),
                serialineopilot_pane.children.as_slice(),
                cx,
            )
            .await;
            ...
            let pane = panel
                ...
                .log_err()?;
            let active_item = serialineopilot_pane.active_item;
...
async fn deserialize_terminal_views(
    ...
    let mut deserialineopilot_items = item_ids
        .iter()
        .map(|item_id| {
            cx.update(|window, cx| {
    ...
    while let Some(item) = deserialineopilot_items.next().await {
...
pub(crate) struct SerialineopilotTerminalPanel {
    pub items: SerialineopilotItems,
...
#[derive(Debug, Serialize, Deserialize)]
#[serde(untagged)]
pub(crate) enum SerialineopilotItems {
    // The data stored before terminal splits were introduced.
    NoSplits(Vec<u64>),
    WithSplits(SerialineopilotPaneGroup),
}
...
#[derive(Debug, Serialize, Deserialize)]
pub(crate) struct SerialineopilotPane {
    pub active: bool,
...
#[derive(Debug)]
pub(crate) struct SerialineopilotAxis(pub Axis);

impl Serialize for SerialineopilotAxis {
...
impl<'de> Deserialize<'de> for SerialineopilotAxis {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        match s.as_str() {
            "horizontal" => Ok(SerialineopilotAxis(Axis::Horizontal)),
            "vertical" => Ok(SerialineopilotAxis(Axis::Vertical)),
