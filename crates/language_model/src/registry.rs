...
impl ConfiguredModel {
    ...
    pub fn is_provided_by_neopilot(&self) -> bool {
        self.provider.id().0 == crate::NEOPILOT_CLOUD_PROVIDER_ID
    }
}
...
impl LanguageModelRegistry {
    ...
    pub fn providers(&self) -> Vec<Arc<dyn LanguageModelProvider>> {
        let neopilot_provider_id = LanguageModelProviderId("neopilot.dev".into());
        let mut providers = Vec::with_capacity(self.providers.len());
        if let Some(provider) = self.providers.get(&neopilot_provider_id) {
            providers.push(provider.clone());
        }
        providers.extend(self.providers.values().filter_map(|p| {
            if p.id() != neopilot_provider_id {
                Some(p.clone())
            } else {
                None
            }
    ...
    pub fn default_model(&self) -> Option<ConfiguredModel> {
        ...
        if std::env::var("NEOPILOT_SIMULATE_NO_LLM_PROVIDER").is_ok() {
            return None;
        }
