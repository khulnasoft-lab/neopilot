...
use crate::{
    ...
    persistence::{self, DebuggerPaneItem, SerialineopilotLayout},
};
...
use task::{
    BuildTaskDefinition, DebugScenario, NeopilotDebugConfig, ShellBuilder, SpawnInTerminal,
...
impl RunningState {
    ...
    pub(crate) fn new(
        ...
        project: Entity<Project>,
        workspace: WeakEntity<Workspace>,
        serialineopilot_pane_layout: Option<SerialineopilotLayout>,
        dock_axis: Axis,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> Self {
        ...
        let panes = if let Some(root) =
            serialineopilot_pane_layout.and_then(|serialineopilot_layout| {
                persistence::deserialize_pane_layout(
                    serialineopilot_layout.panes,
                    dock_axis != serialineopilot_layout.dock_axis,
                    &workspace,
                    &project,
                    &stack_frame_list,
                    &variable_list,
    ...
    pub(crate) fn resolve_scenario(
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            if config_is_valid {
                ...
                let neopilot_config = NeopilotDebugConfig {
                    label: label.clone(),
                ...
                let scenario = dap_registry
                    .adapter(&adapter)
                    .ok_or_else(|| anyhow!("{}: is not a valid adapter name", &adapter))
                    .map(|adapter| adapter.config_from_neopilot_format(neopilot_config))??;
            ...
            self._schedule_serialize = Some(cx.spawn_in(window, async move |this, cx| {
                ...
                let Some((adapter_name, pane_layout)) = this
                    .read_with(cx, |this, cx| {
                        ...
                        (
                            adapter_name,
                            persistence::build_serialineopilot_layout(
                                &this.panes.root,
                                this.dock_axis,
                                cx,
                            ),
    ...
    #[cfg(test)]
    pub(crate) fn serialineopilot_layout(&self, cx: &App) -> SerialineopilotLayout {
        persistence::build_serialineopilot_layout(&self.panes.root, self.dock_axis, cx)
    }
