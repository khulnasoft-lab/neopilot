...
use task::{DebugScenario, LaunchRequest, NeopilotDebugConfig};
...
impl NewSessionModal {
    ...
    fn debug_scenario(&self, debugger: &str, cx: &App) -> Option<DebugScenario> {
        ...
        let session_scenario = NeopilotDebugConfig {
        ...
        cx.global::<DapRegistry>()
            ...
            .and_then(|adapter| adapter.config_from_neopilot_format(session_scenario).ok())
    }
...
pub(super) struct AttachMode {
    pub(super) definition: NeopilotDebugConfig,
...
impl AttachMode {
    pub(super) fn new(
        ...
        let definition = NeopilotDebugConfig {
