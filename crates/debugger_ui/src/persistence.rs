...
pub(crate) struct SerialineopilotLayout {
    pub(crate) panes: SerialineopilotPaneLayout,
    pub(crate) dock_axis: Axis,
}

#[derive(Debug, Serialize, Deserialize, Clone)]
pub(crate) enum SerialineopilotPaneLayout {
    Pane(SerialineopilotPane),
    Group {
        axis: Axis,
        flexes: Option<Vec<f32>>,
        children: Vec<SerialineopilotPaneLayout>,
    },
}
...
pub(crate) async fn serialize_pane_layout(
    ...
    pane_group: SerialineopilotLayout,
) -> anyhow::Result<()> {
    let serialineopilot_pane_group = serde_json::to_string(&pane_group)
...
pub(crate) fn build_serialineopilot_layout(
    pane_group: &Member,
    dock_axis: Axis,
    cx: &App,
) -> SerialineopilotLayout {
    SerialineopilotLayout {
        dock_axis,
        panes: build_serialineopilot_pane_layout(pane_group, cx),
    }
}

pub(crate) fn build_serialineopilot_pane_layout(
    pane_group: &Member,
    cx: &App,
) -> SerialineopilotPaneLayout {
    match pane_group {
        Member::Axis(PaneAxis {
            ...
            flexes,
            bounding_boxes: _,
        }) => SerialineopilotPaneLayout::Group {
            axis: *axis,
            children: members
                .iter()
                .map(|member| build_serialineopilot_pane_layout(member, cx))
                .collect::<Vec<_>>(),
            flexes: Some(flexes.lock().clone()),
        },
        Member::Pane(pane_handle) => {
            SerialineopilotPaneLayout::Pane(serialize_pane(pane_handle, cx))
        }
    }
}

fn serialize_pane(pane: &Entity<Pane>, cx: &App) -> SerialineopilotPane {
...
pub(crate) async fn get_serialineopilot_layout(
    adapter_name: impl AsRef<str>,
) -> Option<SerialineopilotLayout> {
    ...
    KEY_VALUE_STORE
        ...
        .log_err()
        .flatten()
        .and_then(|value| serde_json::from_str::<SerialineopilotLayout>(&value).ok())
}

pub(crate) fn deserialize_pane_layout(
    serialineopilot: SerialineopilotPaneLayout,
    should_invert: bool,
    ...
    match serialineopilot {
        ...
        SerialineopilotPaneLayout::Pane(serialineopilot_pane) => {
            ...
            let sub_views: Vec<_> = serialineopilot_pane
                .children
                .iter()
...
#[cfg(test)]
impl SerialineopilotPaneLayout {
    pub(crate) fn in_order(&self) -> Vec<SerialineopilotPaneLayout> {
        let mut panes = vec![];
    ...
    fn inner_in_order(&self, panes: &mut Vec<SerialineopilotPaneLayout>) {
        match self {
            SerialineopilotPaneLayout::Pane(_) => panes.push((*self).clone()),
            SerialineopilotPaneLayout::Group {
                axis: _,
                flexes: _,
                children,
