    ...
    pub(crate) fn save_scenario(
        ...
        self.workspace
            .update(cx, |workspace, cx| {
                ...
                let serialineopilot_scenario = serde_json::to_value(scenario);

                cx.spawn_in(window, async move |workspace, cx| {
                    let serialineopilot_scenario = serialineopilot_scenario?;
                    let fs =
                    ...
                    if !fs.is_file(path).await {
                        ...
                        values.push(serialineopilot_scenario);
                        fs.save(
                            path,
                    ...
                    workspace.update(cx, |workspace, cx| {
                        workspace
                            ...
                            .context(
                                "Couldn't get project path for .neopilot/debug.json in active worktree",
                            )
                    })?
                })
            })
...
async fn register_session_inner(
    ...
    .ok();
    let serialineopilot_layout = persistence::get_serialineopilot_layout(adapter_name).await;
