...
use crate::{
    ...
    persistence::SerialineopilotPaneLayout,
...
async fn test_invert_axis_on_panel_position_change(
    ...
    let pre_serialineopilot_layout = debug_panel
        .read_with(cx, |panel, cx| {
            panel
                ...
                .running_state()
                .read(cx)
                .serialineopilot_layout(cx)
        })
        .panes;

    let post_serialineopilot_layout = debug_panel
    ...
    let pre_panes = pre_serialineopilot_layout.in_order();
    let post_panes = post_serialineopilot_layout.in_order();
    ...
    for (pre, post) in zip(pre_panes, post_panes) {
        match (pre, post) {
            ...
            (
                SerialineopilotPaneLayout::Pane(pre_pane),
                SerialineopilotPaneLayout::Pane(post_pane),
            ) => {
