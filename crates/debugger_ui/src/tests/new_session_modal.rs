...
use task::{
    DebugRequest, DebugScenario, LaunchRequest, NeopilotDebugConfig, TaskContext, VariableName,
};
use util::path;
...
async fn test_debug_session_substitutes_variables_and_relativizes_paths(
    ...
    let test_cases: Vec<(&'static str, &'static str)> = vec![
        // Absolute path - should not be relativineopilot
        (
        ...
        // Path with $NEOPILOT_WORKTREE_ROOT - should be substituted without double appending
        (
            format!(
                "$NEOPILOT_WORKTREE_ROOT{0}src{0}program",
            ...
            move |client| {
                client.on_request::<dap::requests::Launch, _>({
                    ...
                    move |_, args| {
                        ...
                        let expected_other_field = if input_path.contains("$NEOPILOT_WORKTREE_ROOT")
                        {
                            input_path
                                .replace("$NEOPILOT_WORKTREE_ROOT", &path!("/test/worktree/path"))
                                .to_owned()
                        } else {
...
async fn test_save_debug_scenario_to_file(executor: BackgroundExecutor, cx: &mut TestAppContext) {
    ...
    let debug_json_content = fs
        .load(path!("/project/.neopilot/debug.json").as_ref())
        .await
...
async fn test_dap_adapter_config_conversion_and_validation(cx: &mut TestAppContext) {
    ...
    for adapter_name in adapter_names {
        ...
        let mut adapter_specific_config = neopilot_config.clone();
        adapter_specific_config.adapter = adapter_name.to_string().into();

        let debug_scenario = adapter
            .config_from_neopilot_format(adapter_specific_config)
            .unwrap_or_else(|_| {
                panic!(
                    "Adapter {} should successfully convert from Neopilot format",
                    adapter_name
                )
            });
