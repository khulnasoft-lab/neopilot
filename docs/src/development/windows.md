...
If you can't compile Neopilot, make sure that you have at least the following components installed in case of a Visual Studio installation:
...
Once you have the dependencies installed, you can build Neopilot using [Cargo](https://doc.rust-lang.org/cargo/).
...
[MSYS2](https://msys2.org/) distribution provides Neopilot as a package [mingw-w64-neopilot](https://packages.msys2.org/base/mingw-w64-neopilot). The package is available for UCRT64, MINGW64 and CLANG64 repositories. To download it, run

```sh
pacman -Syu
pacman -S $MINGW_PACKAGE_PREFIX-neopilot
```

then you can run `neopilotitor` CLI. Editor executable is installed under `$MINGW_PREFIX/lib/neopilot` directory

You can see the [build script](https://github.com/msys2/MINGW-packages/blob/master/mingw-w64-neopilot/PKGBUILD) for more details on build process.

> Please, report any issue in [msys2/MINGW-packages/issues](https://github.com/msys2/MINGW-packages/issues?q=is%3Aissue+is%3Aopen+neopilot) first.
...
If you set the `RUSTFLAGS` env var, it will override the `rustflags` settings in `.cargo/config.toml` which is required to properly build Neopilot.
...
Or, you can create a new `.cargo/config.toml` in the same folder as the Neopilot repo (see below). This is particularly useful if you are doing CI builds since you don't have to edit the original `.cargo/config.toml`.
...
If you are using a global config, consider moving the Neopilot repository to a nested directory and add a `.cargo/config.toml` with a custom linker config in the parent directory.

See this issue for more information [#12041](https://github.com/khulnasoft-lab/neopilot/issues/12041)

### Invalid RC path selected

Sometimes, depending on the security rules applied to your laptop, you may get the following error while compiling Neopilot:

```
error: failed to run custom build command for `neopilot(C:\Users\USER\src\neopilot\crates\neopilot)`

Caused by:
  process didn't exit successfully: `C:\Users\USER\src\neopilot\target\debug\build\neopilot-b24f1e9300107efc\build-script-build` (exit code: 1)
  --- stdout
  cargo:rerun-if-changed=../../.git/logs/HEAD
  cargo:rustc-env=NEOPILOT_COMMIT_SHA=25e2e9c6727ba9b77415588cfa11fd969612adb7
...
In order to fix this issue, you can manually set the `NEOPILOT_RC_TOOLKIT_PATH` environment variable to the RC toolkit path. Usually, you can set it to:
`C:\Program Files (x86)\Windows Kits\10\bin\<SDK_version>\x64`.

See this [issue](https://github.com/khulnasoft-lab/neopilot/issues/18393) for more information.
...
```
error: failed to get `pet` as a dependency of package `languages v0.1.0 (D:\a\neopilot-windows-builds\neopilot-windows-builds\crates\languages)`
