...
Edit Prediction is Neopilot's native mechanism for predicting the code you want to write through AI.
Each keystroke sends a new request to our [open source, open dataset Zeta model](https://huggingface.co/khulnasoft-lab/zeta) and it returns with individual or multi-line suggestions that can be quickly accepted by pressing `tab`.

## Configuring Zeta

Neopilot's Edit Prediction was initially introduced via a banner on the title bar.
Clicking on it would take you to a modal with a button ("Enable Edit Prediction") that sets `neopilot` as your `edit_prediction_provider`.

![Onboarding banner and modal](https://neopilot.dev/img/edit-prediction/docs.webp)

But, if you haven't come across the banner, Neopilot's Edit Prediction is the default edit prediction provider and you should see it right away in your status bar.

### Switching Modes {#switching-modes}

Neopilot's Edit Prediction comes with two different display modes:
...
![Edit Prediction status bar menu, with the modes toggle.](https://neopilot.dev/img/edit-prediction/status-bar-menu.webp)

### Conflict With Other `tab` Actions {#edit-predictions-conflict}

By default, when `tab` would normally perform a different action, Neopilot requires a modifier key to accept predictions:
...
When there's a [conflict with the `tab` key](#edit-predictions-conflict), Neopilot uses a different context to accept keybindings (`edit_prediction_conflict`). If you want to use a different one, you can insert this in your keymap:
...
You can also bind this action to keybind without a modifier. In that case, Neopilot will use the default modifier (`alt`) to preview the edit prediction.
...
Neopilot requires at least one keybinding for the {#action editor::AcceptEditPrediction} action in both the `Editor && edit_prediction` and `Editor && edit_prediction_conflict` contexts ([learn more above](#edit-predictions-keybinding)).
...
Alternatively, if you have Neopilot set as your provider, consider [using Subtle Mode](#switching-modes).
...
This hides every indication that there is a prediction available, regardless of [the display mode](#switching-modes) you're in (valid only if you have Neopilot as your provider).
...
You may also use the [Agent Panel](./agent-panel.md) or the [Inline Assistant](./inline-assistant.md) to interact with language models, see the [AI documentation](./overview.md) for more information on the other AI features in Neopilot.
