...
Neopilot’s plans offer hosted versions of major LLM’s, generally with higher rate limits than individual API keys.
We’re working hard to expand the models supported by Neopilot’s subscription offerings, so please check back often.
...
Non-Max Mode usage will use up to 25 tool calls per one prompt. If your prompt extends beyond 25 tool calls, Neopilot will ask if you’d like to continue, which will consume a second prompt.
...
Because of the increased cost to Neopilot, each subsequent request beyond the initial user prompt in Max Mode models is counted as a prompt for metering.
...
Each Agent thread in Neopilot maintains its own context window.
