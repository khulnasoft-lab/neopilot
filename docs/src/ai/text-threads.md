...
![Asking a question](https://neopilot.dev/img/assistant/ask-a-question.png)

As you type, the remaining tokens count for the selected model is updated.

Inserting text from an editor is as simple as highlighting the text and running `assistant: quote selection` ({#kb assistant::QuoteSelection}); Neopilot will wrap it in a fenced code block if it is code.

![Quoting a selection](https://neopilot.dev/img/assistant/quoting-a-selection.png)
...
![Receiving an answer](https://neopilot.dev/img/assistant/receiving-an-answer.png)
...
```plaintext
Title: Neopilot-Flavored Rust

## About Neopilot

/prompt Neopilot: Neopilot (a rule about what Neopilot is)

## Rust - Neopilot Style

/prompt Rust: Async - Async-std (neopilot doesn't use tokio)
/prompt Rust: Neopilot-style Crates (we have some unique conventions)
...
Neopilot uses rule templates to power internal assistant features, like the terminal assistant, or the content rules used in the inline assistant.

Neopilot has the following internal rule templates:
...
Neopilot allows you to override the default rules used for various assistant features by placing custom Handlebars (.hbs) templates in your `~/.config/neopilot/prompt_overrides` directory.

The following templates can be overridden:

1. [`content_prompt.hbs`](https://github.com/khulnasoft-lab/neopilot/tree/main/assets/prompts/content_prompt.hbs): Used for generating content in the editor.

2. [`terminal_assistant_prompt.hbs`](https://github.com/khulnasoft-lab/neopilot/tree/main/assets/prompts/terminal_assistant_prompt.hbs): Used for the terminal assistant feature.

> **Note:** Be sure you want to override these, as you'll miss out on iteration on our built-in features. This should be primarily used when developing Neopilot.

You can customize these templates to better suit your needs while maintaining the core structure and variables used by Neopilot. Neopilot will automatically reload your prompt overrides when they change on disk.

Consult Neopilot's [assets/prompts](https://github.com/khulnasoft-lab/neopilot/tree/main/assets/prompts) directory for current versions you can play with.
...
- `~/.config/neopilot/conversations` (macOS)
- `~/.local/share/neopilot/conversations` (Linux)
- `%LocalAppData%\Neopilot\conversations` (Windows)

You can access and load previous contexts by clicking on the history button in the top-left corner of the agent panel.

![Viewing assistant history](https://neopilot.dev/img/assistant/assistant-history.png)
