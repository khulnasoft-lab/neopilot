...
Rust support is available natively in Neopilot.
...
<!--
TBD: Polish Rust Docs. Neopilot is a good rust editor, good Rust docs make it look like we care about Rust (we do!)
...
You can configure which `rust-analyzer` binary Neopilot should use.

By default, Neopilot will try to find a `rust-analyzer` in your `$PATH` and try to use that. If that binary successfully executes `rust-analyzer --help`, it's used. Otherwise, Neopilot will fall back to installing its own `rust-analyzer` version and using that.

If you want to disable Neopilot looking for a `rust-analyzer` binary, you can set `ignore_system_version` to `true` in your `settings.json`:
...
If want rust-analyzer to provide diagnostics for a target other than you current platform (e.g. for windows when running on macOS) you can use the following Neopilot lsp settings:
...
Neopilot provides tasks using tree-sitter, but rust-analyzer has an LSP extension method for querying file-related tasks via LSP.
...
"diagnostics": {
  "cargo": {
    // When enabled, Neopilot disables rust-analyzer's check on save and starts to query
...
<!--
TBD: Is it possible to specify RUSTFLAGS? https://github.com/khulnasoft-lab/neopilot/issues/14334
-->
...
Rust-analyzer in Neopilot runs with the default parameters.
...
Which would mean that every time Neopilot saves, a `cargo check --workspace --all-targets` command is run, checking the entire project (workspace), lib, doc, test, bin, bench and [other targets](https://doc.rust-lang.org/cargo/reference/cargo-targets.html).

While that works fine on small projects, it does not scale well.

The alternatives would be to use [tasks](../tasks.md), as Neopilot already provides a `cargo check --workspace --all-targets` task and the ability to cmd/ctrl-click on the terminal output to navigate to the error, and limit or turn off the check on save feature entirely.
...
Here's a snippet for Neopilot settings.json (the language server will restart automatically after the `lsp.rust-analyzer` section is edited and saved):
