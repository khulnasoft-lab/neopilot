...
Ruby support is available through the [Ruby extension](https://github.com/neopilot-extensions/ruby).
...
There are multiple language servers available for Ruby. Neopilot supports the two following:
...
In addition to these two language servers, Neopilot also supports [rubocop](https://github.com/rubocop/rubocop) which is a static code analyzer and linter for Ruby. Under the hood, it's also used by Neopilot as a language server, but its functionality is complimentary to that of solargraph and ruby-lsp.
...
The [Ruby extension](https://github.com/neopilot-extensions/ruby) offers both `solargraph` and `ruby-lsp` language server support.
...
Solargraph has formatting and diagnostics disabled by default. We can tell Neopilot to enable them by adding the following to your `settings.json`:
...
Ruby LSP uses pull-based diagnostics which Neopilot doesn't support yet. We can tell Neopilot to disable it by adding the following to your `settings.json`:
...
LSP `settings` and `initialization_options` can also be project-specific. For example to use [standardrb/standard](https://github.com/standardrb/standard) as a formatter and linter for a particular project, add this to a `.neopilot/settings.json` inside your project repo:
...
Rubocop has unsafe autocorrection disabled by default. We can tell Neopilot to enable it by adding the following to your `settings.json`:
...
To run tests in your Ruby project, you can set up custom tasks in your local `.neopilot/tasks.json` configuration file. These tasks can be defined to work with different test frameworks like Minitest, RSpec, quickdraw, and tldr. Below are some examples of how to set up these tasks to run your tests from within your editor.
...
[
  {
    "label": "test $NEOPILOT_RELATIVE_FILE -n /$NEOPILOT_SYMBOL/",
    "command": "bin/rails test $NEOPILOT_RELATIVE_FILE -n /$NEOPILOT_SYMBOL/",
    "tags": ["ruby-test"]
  }
]
```
...
Plain minitest does not support running tests by line number, only by name, so we need to use `$NEOPILOT_SYMBOL` instead:

```json
[
  {
    "label": "-Itest $NEOPILOT_RELATIVE_FILE -n /$NEOPILOT_SYMBOL/",
    "command": "bundle exec ruby",
    "args": ["-Itest", "$NEOPILOT_RELATIVE_FILE", "-n /$NEOPILOT_SYMBOL/"],
    "tags": ["ruby-test"]
  }
]
```
...
[
  {
    "label": "test $NEOPILOT_RELATIVE_FILE:$NEOPILOT_ROW",
    "command": "bundle exec rspec",
    "args": ["\"$NEOPILOT_RELATIVE_FILE:$NEOPILOT_ROW\""],
    "tags": ["ruby-test"]
  }
]
```
