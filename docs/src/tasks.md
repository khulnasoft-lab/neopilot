...
Neopilot supports ways to spawn (and rerun) commands using its integrated terminal to output the results. These commands can read a limited subset of Neopilot state (such as a path to the file currently being edited or selected text).
...
- in the global `tasks.json` file; such tasks are available in all Neopilot projects you work on. This file is usually located in `~/.config/neopilot/tasks.json`. You can edit them by using the `neopilot: open tasks` action.
- in the worktree-specific (local) `.neopilot/tasks.json` file; such tasks are available only when working on a project with that worktree included. You can edit worktree-specific tasks by using the `neopilot: open project tasks` action.
...
Neopilot tasks act just like your shell; that also means that you can reference environmental variables via sh-esque `$VAR_NAME` syntax. A couple of additional environmental variables are set for your convenience.
...
- `NEOPILOT_COLUMN`: current line column
- `NEOPILOT_ROW`: current line row
- `NEOPILOT_FILE`: absolute path of the currently opened file (e.g. `/Users/my-user/path/to/project/src/main.rs`)
- `NEOPILOT_FILENAME`: filename of the currently opened file (e.g. `main.rs`)
- `NEOPILOT_DIRNAME`: absolute path of the currently opened file with file name stripped (e.g. `/Users/my-user/path/to/project/src`)
- `NEOPILOT_RELATIVE_FILE`: path of the currently opened file, relative to `NEOPILOT_WORKTREE_ROOT` (e.g. `src/main.rs`)
- `NEOPILOT_STEM`: stem (filename without extension) of the currently opened file (e.g. `main`)
- `NEOPILOT_SYMBOL`: currently selected symbol; should match the last symbol shown in a symbol breadcrumb (e.g. `mod tests > fn test_task_contexts`)
- `NEOPILOT_SELECTED_TEXT`: currently selected text
- `NEOPILOT_WORKTREE_ROOT`: absolute path to the root of the current worktree. (e.g. `/Users/my-user/path/to/project`)
- `NEOPILOT_CUSTOM_RUST_PACKAGE`: (Rust-specific) name of the parent package of $NEOPILOT_FILE source file.
...
```json
{
  ...
  "command": "echo $NEOPILOT_FILE"
}
```

You can also use verbose syntax that allows specifying a default if a given variable is not available: `${NEOPILOT_FILE:default_value}`
...
{
  "label": "start lazygit",
  "command": "lazygit -p $NEOPILOT_WORKTREE_ROOT"
}
```
...
Neopilot supports overriding the default action for inline runnable indicators via workspace-local and global `tasks.json` file with the following precedence hierarchy:
