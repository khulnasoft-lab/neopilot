...
Language support in Neopilot has several components:
...
Each language supported by Neopilot must be defined in a subdirectory inside the `languages` directory of your extension.
...
- `first_line_pattern` is a regular expression, that in addition to `path_suffixes` (above) or `file_types` in settings can be used to match files which should use this language. For example Neopilot uses this to identify Shell Scripts by matching the [shebangs lines](https://github.com/khulnasoft-lab/neopilot/blob/main/crates/languages/src/bash/config.toml) in the first line of a script.
...
Neopilot uses the syntax tree produced by the [Tree-sitter](https://tree-sitter.github.io) query language to implement
several features:
...
features in Neopilot, using [JSON syntax](https://www.json.org/json-en.html) as a guiding example.
...
The `textobjects.scm` file defines rules for navigating by text objects. This was added in Neopilot v0.165 and is currently used only in Vim mode.
...
For languages with closures, these typically should not count as functions in Neopilot. This is best-effort however, as languages like Javascript do not syntactically differentiate syntactically between closures and top-level function declarations.
...
The `@run` capture specifies where the run button should appear in the editor. Other captures, except those prefixed with an underscore, are exposed as environment variables with a prefix of `NEOPILOT_CUSTOM_$(capture_name)` when running the code.
...
Neopilot uses the [Language Server Protocol](https://microsoft.github.io/language-server-protocol/) to provide advanced language support.
