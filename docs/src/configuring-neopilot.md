# Configuring Neopilot

Neopilot is designed to be configured: we want to fit your workflow and preferences exactly. We provide default settings that are designed to be a comfortable starting point for as many people as possible, but we hope you will enjoy tweaking it to make it feel incredible.

In addition to the settings described here, you may also want to change your [theme](./themes.md), configure your [key bindings](./key-bindings.md), set up [tasks](./tasks.md) or install [extensions](https://github.com/khulnasoft-lab/extensions).

## Settings files

<!--
TBD: Settings files. Rewrite with "remote settings" in mind (e.g. `local settings` on the remote host).
Consider renaming `neopilot: Open Local Settings` to `neopilot: Open Project Settings`.
...
Your settings file can be opened with {#kb neopilot::OpenSettings}. By default it is located at `~/.config/neopilot/settings.json`, though if you have XDG_CONFIG_HOME in your environment on Linux it will be at `$XDG_CONFIG_HOME/neopilot/settings.json` instead.

This configuration is merged with any local configuration inside your projects. You can open the project settings by running {#action neopilot::OpenProjectSettings} from the command palette. This will create a `.neopilot` directory containing`.neopilot/settings.json`.
...
You can find the default settings for your current Neopilot by running {#action neopilot::OpenDefaultSettings} from the command palette.
...
```sh
ls ~/Library/Application\ Support/Neopilot/extensions/installed/
```

On Linux:

```sh
ls ~/.local/share/neopilot/extensions/installed
```
...
1. Restore all workspaces that were open when quitting Neopilot:
...
Neopilot supports all OpenType features that can be enabled or disabled for a given buffer or terminal font, as well as setting values for font features.
...
- Description: Files or globs of files that will be excluded by Neopilot entirely. They will be skipped during file scans, file searches, and not be displayed in the project file tree. Overrides `file_scan_inclusions`.
- Default:
...
- Description: Files or globs of files that will be included by Neopilot, even when ignored by git. This is useful for files that are not tracked by git, but are still important to your project. Note that globs that are overly broad can slow down Neopilot's file scanning. `file_scan_exclusions` takes precedence over these inclusions.
- Default:
...
- Setting: `file_types`
- Description: Configure how Neopilot selects a language for a file based on its filename or extension. Supports glob entries.
- Default:

```json
"file_types": {
  "JSONC": ["**/.neopilot/**/*.json", "**/neopilot/**/*.json", "**/Neopilot/**/*.json", "**/.vscode/**/*.json"],
  "Shell Script": [".env.*"]
}
```
...
3. Enable indent aware coloring ("rainbow indentation").
   The colors that are used for different indentation levels are defined in the theme (theme key: `accents`). They can be customineopilot by using theme overrides.
...
- Description: The icon theme setting can be specified in two forms - either as the name of an icon theme or as an object containing the `mode`, `dark`, and `light` icon themes for files/folders inside Neopilot.
- Setting: `icon_theme`
- Default: `Neopilot (Default)`
...
"icon_theme": {
  "mode": "system",
  "dark": "Neopilot (Default)",
  "light": "Neopilot (Default)"
},
```
...
The following languages have inlay hints preconfigured by Neopilot:

- [Go](https://docs.neopilot.dev/languages/go)
- [Rust](https://docs.neopilot.dev/languages/rust)
- [Svelte](https://docs.neopilot.dev/languages/svelte)
- [Typescript](https://docs.neopilot.dev/languages/typescript)

Use the `lsp` section for the server configuration. Examples are provided in the corresponding language documentation.

Hints are not instantly queried in Neopilot, two kinds of debounces are used, either may be set to 0 to be disabled.
...
- Description: Configure a network proxy for Neopilot.
- Setting: `proxy`
- Default: `null`
...
By default no proxy will be used, or Neopilot will attempt to retrieve proxy settings from environment variables, such as `http_proxy`, `HTTP_PROXY`, `https_proxy`, `HTTPS_PROXY`, `all_proxy`, `ALL_PROXY`.
...
- Description: Control what info is collected by Neopilot.
- Setting: `telemetry`
- Default:
...
- Description: Setting for sending anonymineopilot usage data, such what languages you're using Neopilot with.
- Setting: `metrics`
- Default: `true`
...
- Description: The theme setting can be specified in two forms - either as the name of a theme or as an object containing the `mode`, `dark`, and `light` themes for the Neopilot UI.
- Setting: `theme`
- Default: `One Dark`
...
- Description: The name of the dark Neopilot theme to use for the UI.
- Setting: `dark`
- Default: `One Dark`
...
- Description: The name of the light Neopilot theme to use for the UI.
- Setting: `light`
- Default: `One Light`
...
Neopilot supports all OpenType features that can be enabled or disabled for a given UI font, as well as setting values for font features.
...
// ~/.config/neopilot/settings.json
{
  ...
  "buffer_font_size": 18,
  "buffer_font_family": "Neopilot Plex Mono",
