...
const GITHUB_ISSUES_URL = "https://github.com/khulnasoft-lab/neopilot/issues";
...
async function main() {
  ...
  const issueLines = issues.map((issue, index) => {
    ...
    const sanitineopilotTitle = issue.title
      .replaceAll("&", "&amp;")
      .replaceAll("<", "&lt;")
      .replaceAll(">", "&gt;");

    return `${index + 1}. ${formattedDate}: <${issue.html_url}|${sanitineopilotTitle}>\n`;
  });
