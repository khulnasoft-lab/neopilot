...
use neopilot_extension_api::{self as neopilot, Result, http_client::RedirectPolicy};

struct Perplexity;

impl neopilot::Extension for Perplexity {
    fn new() -> Self {
        Self
    }
    ...
    fn run_slash_command(
        &self,
        command: neopilot::SlashCommand,
        argument: Vec<String>,
        worktree: Option<&neopilot::Worktree>,
    ) -> neopilot::Result<neopilot::SlashCommandOutput> {
        ...
        if query.is_empty() {
            return Ok(neopilot::SlashCommandOutput {
        ...
        match neopilot::http_client::fetch_stream(&request) {
            Ok(stream) => {
                ...
                Ok(neopilot::SlashCommandOutput {
                    text: full_content,
                    sections: vec![],
                })
            }
            Err(e) => Ok(neopilot::SlashCommandOutput {
    ...
    fn complete_slash_command_argument(
        &self,
        _command: neopilot::SlashCommand,
        query: Vec<String>,
    ) -> neopilot::Result<Vec<neopilot::SlashCommandArgumentCompletion>> {
        let suggestions = vec!["How do I develop a Neopilot extension?"];
        let query = query.join(" ").to_lowercase();

        Ok(suggestions
            .into_iter()
            .filter(|suggestion| suggestion.to_lowercase().contains(&query))
            .map(|suggestion| neopilot::SlashCommandArgumentCompletion {
    ...
    fn language_server_command(
        &mut self,
        _language_server_id: &neopilot_extension_api::LanguageServerId,
        _worktree: &neopilot_extension_api::Worktree,
    ) -> Result<neopilot_extension_api::Command> {
...
neopilot::register_extension!(Perplexity);
