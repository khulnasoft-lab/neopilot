...
use neopilot_extension_api::{self as neopilot, LanguageServerId, Result, serde_json::json};
use std::{env, fs};

const BINARY_NAME: &str = "vscode-html-language-server";
const SERVER_PATH: &str =
    "node_modules/@khulnasoft-lab/vscode-langservers-extracted/bin/vscode-html-language-server";
const PACKAGE_NAME: &str = "@khulnasoft-lab/vscode-langservers-extracted";
...
impl neopilot::Extension for HtmlExtension {
    fn new() -> Self {
        Self {
    ...
    fn language_server_command(
        ...
        Ok(neopilot::Command {
            ...
            args: vec![
                neopilot_ext::sanitize_windows_path(env::current_dir().unwrap())
                    .join(&server_path)
                    .to_string_lossy()
                    .to_string(),
                "--stdio".to_string(),
    ...
    fn language_server_workspace_configuration(
        ...
        worktree: &neopilot::Worktree,
    ) -> Result<Option<neopilot::serde_json::Value>> {
...
neopilot::register_extension!(HtmlExtension);
...
mod neopilot_ext {
    ...
    pub fn sanitize_windows_path(path: std::path::PathBuf) -> std::path::PathBuf {
        use neopilot_extension_api::{Os, current_platform};
